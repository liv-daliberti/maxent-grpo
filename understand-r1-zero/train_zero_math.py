...
from oat_zero_ext.listwise import (ListwiseControllerState,
...
class ZeroMathLearner(PPOLearner):
    ...
    def _listwise_learning_step(self, trajectory):
        ...
        if self.strategy.is_rank_0():
            logging.info(
                "listwise runtime config: tau=%s beta=%s q_temperature=%s "
        ...
        for _ in range(args.num_ppo_epochs):
            ...
            for mini_batch_inds in iter_grouped_minibatch_indices(
                ...
                active_group_mask = (~neutral_group_mask) & grouped_loss_masks.any(dim=1)
                ...
                if drgrpo_token_primary:
                    ...
                    logprobs_diff = new_logps - mb_behavior_logps.to(new_logps.dtype)
                    ratio = torch.exp(logprobs_diff)
                    pg_losses = -mb_advantage * ratio
                    pg_losses2 = -mb_advantage * torch.clamp(
                        ratio,
                    ...
                    pg_loss_max = torch.max(pg_losses, pg_losses2)
                    drgrpo_pg_loss = self.masked_aggregator(
                        pg_loss_max,
                        mb_response_masks,
                        axis=1,
                    )
                    drgrpo_pg_loss = (drgrpo_pg_loss * mb_loss_masks).mean()
                    ...
                    stats["zero_pg_loss_count"].append(
                        (pg_loss_max == 0).detach().sum().to(torch.float32)
                    )
                    stats["logprobs_diff_max"].append(
                        torch.amax(logprobs_diff.detach() * mb_response_masks).to(
                            torch.float32
                        )
                    )
                    stats["logprobs_diff_min"].append(
                        torch.amin(logprobs_diff.detach() * mb_response_masks).to(
                            torch.float32
                        )
                    )
                    drgrpo_row_adv_flat = mb_advantage.reshape(-1).to(
                        device=new_logps.device,
                        dtype=new_logps.dtype,
                    )
                ...
                if bool(args.maxent_use_clip_objective):
                    ...
                    if clip_coef > 0:
                        ...
                        if clip_mode == "sequence":
                            ...
                            if drgrpo_token_primary:
                                ...
                                infos["listwise_helpfulness_proxy"] = (
                                    ...
                                    / (
                                        torch.abs(infos["drgrpo_primary_loss"])
                                        + 1e-8
                                    )
                                )
                            ...
                            active_valid_group_entries = (
                                grouped_loss_masks & active_group_mask.unsqueeze(1)
                            )
                        ...
                        elif clip_mode == "token":
                            token_clip_adv_flat = flatten_prompt_major_tensor(
                                clip_adv
                            ).to(
                                device=new_logps.device,
                                dtype=new_logps.dtype,
                ...
                if local_grad_step == 1 and skip_zero_signal_update:
                    if drgrpo_token_primary:
                        logging.info(
                            ...
                            "any rank; continuing with the Dr.GRPO token update and "
                            "skipping only the sequence-level listwise auxiliary."
                        )
                    else:
                        logging.info(
                            "listwise minibatch has no active prompt groups across "
                ...
                if skip_listwise_backward:
                    ...
                    if token_surrogate_primary:
                        ...
                        if drgrpo_token_primary:
                            seq_coeffs_grouped = seq_coeffs_grouped * float(
                                sequence_aux_coef
                            )
                    ...
                    if drgrpo_token_primary:
                        token_clip_enabled = (
                            drgrpo_row_adv_flat is not None
                            and drgrpo_active_row_mask is not None
                            and drgrpo_active_row_count is not None
                        )
                        token_clip_adv_for_backward = drgrpo_row_adv_flat
                    ...
                    if (
                        ...
                        grad_probe_infos = self._probe_listwise_branch_gradient_metrics(
                            ...
                            clip_low=0.0 if clip_low is None else clip_low,
                            clip_high=0.0 if clip_high is None else clip_high,
                            sequence_aux_coef=sequence_aux_coef,
                            global_active_group_count=global_active_group_count,
                ...
                if (
                    ...
                    logging.info(
                        ...
                        (
                            "applied_drgrpo_only"
                            if skip_zero_signal_update and drgrpo_token_primary
                            else (
                                "skipped_zero_signal"
                ...
                with torch.no_grad():
                    ...
                    infos["listwise_active_group_frac"] = active_group_mask.to(
                        torch.float32
                    ).mean().detach()
                    infos["listwise_active_group_count_global"] = torch.tensor(
