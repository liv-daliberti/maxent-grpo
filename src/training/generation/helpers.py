...
wrappers that delegate to :mod:`training.generation.vllm` for the
...
if dist is None:
    ...
    dist = _DistFallback()  # type: ignore[assignment]
else:
    if not hasattr(dist, "is_available"):
        dist.is_available = lambda: False  # type: ignore[assignment]
    if not hasattr(dist, "is_initialized"):
        dist.is_initialized = lambda: False  # type: ignore[assignment]
    if not hasattr(dist, "get_world_size"):
        dist.get_world_size = lambda: 1  # type: ignore[assignment]
...
def _retry_incomplete_prompts_impl(
    ...
    while incomplete_indices and retry_round < retry_limit:
        ...
        if isinstance(retry_meta, list):
            meta_payload = retry_meta  # type: ignore[assignment]
