...
ds_zero = None  # type: ignore[assignment]
ZeroParamStatus = None  # type: ignore[assignment]
...
def _maybe_zero_gather_embedding(model: Optional[PreTrainedModel]):
    ...
    if weight is None:
        yield
        return
    with gather_ctx([weight], modifier_rank=None):  # type: ignore[operator]  # pylint: disable=not-callable
        yield
...
def _maybe_zero_gather_params(model: Optional[nn.Module], enabled: bool):
    ...
    with gather_ctx(gather_params, modifier_rank=0):  # type: ignore[operator]  # pylint: disable=not-callable
        yield
