"""Hash-bound AntMaze executor for the admitted v9 waypoint controller."""
...
MODEL_PATH = ROOT / "var/maze_runtime/controllers/ant_waypoint_v9.zip"
RECEIPT_PATH = (
    ROOT / "var/maze_runtime/controllers/ant_waypoint_v9.evaluation.json"
)
...
def controller_receipt_sha256() -> str:
    """Return the exact v9 receipt identity after validating admission."""
...
def controller_identity() -> dict[str, Any]:
    """Load and verify the terminal v9 receipt and model as one identity."""
    ...
    if (
        ...
        or receipt.get("decision") != "admitted_to_fresh_maze_route_gate_v9"
    ):
        raise RuntimeError("Ant v9 controller is not admitted")
    if receipt.get("seed") != 73009 or receipt.get("timesteps") != 5_000_000:
        raise RuntimeError("Ant v9 receipt differs from its frozen training identity")
    if (
        training_identity.get("job_id") != 30192731
        or training_identity.get("seed") != 73009
    ):
        raise RuntimeError("Ant v9 training identity is not the frozen job")
    ...
    if not isinstance(checks, dict) or not required_checks.issubset(checks) or not all(
        ...
        raise RuntimeError("Ant v9 receipt contains a failed controller check")
    if receipt.get("hashes", {}).get("model_sha256") != model_sha256:
        raise RuntimeError("Ant v9 receipt does not bind the controller model")
    if receipt.get("training_waypoint_distances") != [WAYPOINT_DISTANCE]:
        raise RuntimeError("Ant v9 receipt has a different waypoint distance")
    if receipt.get("waypoint_distance") != WAYPOINT_DISTANCE:
        raise RuntimeError("Ant v9 receipt has a different executor distance")
    if receipt.get("success_threshold") != WAYPOINT_SUCCESS_THRESHOLD:
        raise RuntimeError("Ant v9 receipt has a different waypoint threshold")
    ...
    if route_identity_path:
        ...
        if route_identity.get("schema_version") != (
            ...
            raise RuntimeError("Ant v9 route identity schema mismatch")
        if any(route_identity.get(key) != value for key, value in expected.items()):
            raise RuntimeError("Ant v9 route identity does not bind the controller")
    return receipt
...
def execute_ant_v9_raw(
    ...
    if spec.controller_sha256 != controller_receipt_sha256():
        raise ValueError("AntMaze controller hash differs from frozen v9 receipt")
...
def execute_ant_v9(candidate: str, raw_spec: Mapping[str, Any]):
    """Execute and validate one v9 program in the pinned AntMaze runtime."""

    execution = execute_ant_v9_raw(candidate, raw_spec)
