...
When ``GenerationContext.use_vllm`` is enabled, the main generator delegates to
this module to:

* Deduplicate prompts across ranks and expand them into the requested number of
  samples.
* Coordinate multi-round retries when the remote server streams partial groups.
* Backfill missing completions locally when vLLM fails to return the remaining
  samples within the configured retry budget.
* Broadcast and scatter grouped completions/logprobs back to each rank.

Separating this logic keeps :mod:`maxent_grpo.training.generation` focused on the
public HF-style API surface while documenting the more elaborate distributed
flow here for Sphinx.
"""
...
def _optional_import(module_name: str) -> Any:
    ...
    :returns: Imported module object or ``None`` when unavailable.
    :rtype: Any | None
    """
    try:
...
def _zero3_gather_factory(
    ...
    :param accelerator: Accelerator instance owning the DeepSpeed plugin.
    :type accelerator: accelerate.Accelerator
    :returns: Context manager factory used to gather ZeRO-3 partitions.
    ...
    def _factory(params: Sequence[Any]) -> Any:
        """Context manager that gathers ZeRO-3 partitions for ``params``."""
...
def _is_peft_model_safe(target: Any) -> bool:
    ...
    :param target: Model/module to inspect for PEFT adapters.
    :type target: Any
    :returns: ``True`` if the model safely exposes PEFT adapters.
    :rtype: bool
    """
...
class VLLMGenerationHelper:
    ...
    def _vllm_base_url(self, url: str) -> str:
        ...
        :param url: Raw server URL provided via configuration.
        :type url: str
        :returns: Canonical base URL pointing to the vLLM server.
        :rtype: str
        """
    ...
    def _ensure_vllm_client(self) -> bool:
        ...
        :returns: ``True`` if the client exists and communicator is initialized.
        :rtype: bool
        """
        ctx = self.ctx
    ...
    def _sync_model_params_to_vllm(self, model: Any) -> None:
        ...
        :param model: Model whose parameters should be synchronized.
        :type model: Any
        """
        fsdp_cls = getattr(getattr(torch, "distributed", None), "fsdp", None)
        fsdp_cls = (
            getattr(fsdp_cls, "FullyShardedDataParallel", None) if fsdp_cls else None
        )
    ...
    def _sync_fsdp_params(self, module: Any, fsdp_cls: type) -> None:
        ...
        for child_name, child in module.named_children():
            ...
            if isinstance(child, fsdp_cls):
                with fsdp_cls.summon_full_params(child, recurse=False, writeback=False):
...
def _gather_object_list(accelerator: Accelerator, value: List[Any]) -> List[List[Any]]:
    ...
    :param accelerator: Accelerator enabling gather helpers.
    :type accelerator: accelerate.Accelerator
    :param value: Local list payload to gather from each rank.
    :type value: list[Any]
    :returns: Gathered payloads ordered by global rank.
    :rtype: list[list[Any]]
    """
...
def _scatter_object(
    ...
    *,
    ...
    :param accelerator: Accelerator providing scatter utilities.
    :type accelerator: accelerate.Accelerator
    :param input_list: List of objects to scatter (only present on ``src`` rank).
    :type input_list: list[Any] | None
    :param src: Source rank performing the scatter.
    :type src: int
    :returns: Object assigned to the current rank.
    :rtype: Any
    """
