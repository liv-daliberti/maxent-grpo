...
GenerationLogprobGroups = Optional[
...
class VLLMLogprobResult:
    ...
    def to_trl_payload(self) -> Dict[str, Any]:
        """Return a dict compatible with TRL's refine_completions metadata."""
...
def _parse_nonstream_json(
    ...
    *,
    ...
    def _append_group(texts: List[str], logs: Optional[List[Optional[VLLMLogprobResult]]]) -> None:
...
def safe_generate(
    *,
    ...
    :raises RuntimeError: When the server returns a non‑200 response or after
        exhausting retries.
    """
    timeout = float(timeout if timeout is not None else os.environ.get("VLLM_TIMEOUT", 120.0))
    max_retries = int(max_retries if max_retries is not None else os.environ.get("VLLM_MAX_RETRIES", 3))
    backoff = float(backoff if backoff is not None else os.environ.get("VLLM_BACKOFF", 1.0))
    effective_request_id = request_id or (
        f"{request_id_prefix}-" if request_id_prefix else ""
    ) + uuid.uuid4().hex
    payload: Dict[str, Any] = {
        "prompts": prompts,
