...
from .run_logging import build_training_metrics_dict
from .run_training_pipeline import PreparedBatch
...
from .run_training_loss import LossOutputs
...
try:  # Optional dependency
    ...
    from wandb.errors import Error as WandbError  # type: ignore[attr-defined]
...
class LogStepArtifacts:
    """Helper container for logging state."""
...
def _summarize_reward_stats(
    ...
    return RewardLoggingView(
        ...
        frac_zero_std=_fraction_zero_std_groups(accelerator, reward_comp.advantage.grouped),
...
def _summarize_weight_stats(
    ...
    :type weight_stats: maxent_helpers.run_training_types.WeightStats
...
def _build_metrics_payload(
    ...
    :rtype: maxent_helpers.run_training_types.TrainingMetricsPayload
    """
    ...
    scalar_stats = TrainingScalarStats(
        ...
        vllm_latency_ms=float(ctx.generation.generation_stats.get("vllm_last_latency_ms", 0.0))
...
def log_local_step(
    ...
    _emit_metrics(ctx, metrics, state.global_step, log_to_wandb=False, tag="Local")
...
def _build_sample_table(
    ...
    for idx in range(max_rows):
        ...
        advantage_val = float(advantages[idx]) if idx < len(advantages) else float("nan")
...
def _log_sample_table(
    ...
    if wandb_run is None or not accelerator.is_main_process or wandb_mod is None:
        return
    pairs = prepared.reward_comp.pairs
