...
from .run_training_weighting import CONTROLLER_STATE_FILENAME, load_controller_state

if TYPE_CHECKING:
    from .run_training_loop import TrainingLoopState

LOG = logging.getLogger(__name__)
_checkpoint_log_once = {"config": False, "strategy": False, "steps": False}


def maybe_clear_stale_controller_state(accelerator: Accelerator, controller_cfg: ControllerPaths) -> None:
    ...
    :type controller_cfg: maxent_helpers.run_training_types.ControllerPaths
    """
    ...
    if accelerator.is_main_process:
        try:
            ...
            LOG.info("Removed stale controller state at %s due to overwrite_output_dir.", state_path)
        except OSError as exc:  # pragma: no cover - filesystem race
            LOG.warning("Failed to remove stale controller state %s: %s", state_path, exc)
    wait_for_all = getattr(accelerator, "wait_for_everyone", None)
    if callable(wait_for_all):
        wait_for_all()


def _load_controller_file(path: Optional[str], accelerator: Accelerator, weighting_cfg: WeightingSettings) -> bool:
    ...
    :type weighting_cfg: maxent_helpers.run_training_types.WeightingSettings
...
def load_controller_state_chain(
    ...
    if isinstance(resume_path, str) and resume_path:
        ...
        controller_loaded = _load_controller_file(resume_state_file, accelerator, weighting_cfg)
    if not controller_loaded:
        controller_loaded = _load_controller_file(controller_cfg.state_path, accelerator, weighting_cfg)
    return controller_loaded


def maybe_load_accelerator_state(resume_state_path: Optional[str], accelerator: Accelerator) -> None:
    ...
    if (
        ...
        try:
            ...
            LOG.warning("Failed to load accelerator state from %s: %s", resume_state_path, exc)


def maybe_checkpoint(logging_cfg: LoggingHandles, accelerator: Accelerator, global_step: int) -> None:
    """Checkpoint periodically while on the main process.

    :param logging_cfg: Logging handles containing checkpoint callbacks and
        scheduling knobs.
    :type logging_cfg: maxent_helpers.run_training_types.LoggingHandles
...
def check_stop_condition(schedule: OptimizationSchedule, state: "TrainingLoopState") -> None:
    ...
    :type schedule: maxent_helpers.run_training_types.OptimizationSchedule
    :param state: Mutable training loop state whose ``stop_training`` flag
        should be updated when the threshold is crossed.
    :type state: maxent_helpers.run_training_loop.TrainingLoopState
    """
    if schedule.total_training_steps > 0 and state.global_step >= schedule.total_training_steps:
