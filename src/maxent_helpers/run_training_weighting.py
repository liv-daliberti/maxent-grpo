...
def _split_ref_logprobs_per_token(
    ...
    for comps in grouped_completions:
        ...
        tok_slice = ref_stats.ref_tok_counts[offset : offset + comp_count].clamp(min=1.0)
...
def maybe_update_tau(
    ...
    if not isinstance(measured_entropy, (int, float)) or not math.isfinite(measured_entropy):
        return
    ...
    if prev_ema is None or not isinstance(prev_ema, (int, float)) or not math.isfinite(prev_ema):
...
def controller_state_dict(weighting_cfg: WeightingSettings) -> dict:
    ...
    return {
        ...
        "tau_entropy_ema": float(getattr(weighting_cfg, "_tau_entropy_ema", float("nan"))),
    }


def save_controller_state(path: Optional[str], weighting_cfg: WeightingSettings) -> None:
...
def load_controller_state(path: Optional[str], weighting_cfg: WeightingSettings) -> bool:
...
def compute_weight_stats(
    ...
    if not flat_weights:
        return None
    weight_entropy, entropy_min, entropy_max, entropy_adv_samples = collect_weight_entropy(
        weights_grouped
    )
    return WeightStats(
