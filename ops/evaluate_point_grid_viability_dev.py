#!/usr/bin/env python3
"""Development-only 0.5B viability for PointMaze grid macro-actions."""
...
def _prompt(spec: dict[str, Any], minimum: int, maximum: int) -> str:
    ...
    return (
        ...
        + "\n".join(rendered)
        + f"\nUse between {minimum} and {maximum} tokens. End exactly at G."
        "\n<|im_end|>\n<|im_start|>assistant\n"
    )
...
def main() -> None:
    ...
    outputs = []
    for row_index, (spec, bounds) in enumerate(zip(adapted_specs, length_bounds)):
    ...
    try:
        for row_index, (row, output, raw_spec, bounds) in enumerate(
            zip(rows, outputs, adapted_specs, length_bounds)
        ):
    ...
    payload = {
        "schema_version": "point-grid-action-development-viability-v1",
        ...
        "sampling": {
            ...
            "prompt_template": "qwen_point_grid_development",
        },
        "criteria": {
        ...
        "information_boundary": {
            ...
            "guided_decoder_knows_feasible_paths": False,
            "guided_decoder_knows_only_cardinal_syntax_and_length": True,
        },
    }
