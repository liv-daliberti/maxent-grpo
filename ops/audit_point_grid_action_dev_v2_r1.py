...
def _hash_selected_tree(root: Path, names: tuple[str, ...]) -> str:
...
def main() -> None:
    ...
    checks = {
        ...
        "execution_hash_matches_identity_bound_files": (
            receipt.get("execution_hash") == identity.get("execution_hash")
            == _hash_selected_tree(
                args.execution_root,
                (
                    "evaluate_point_grid_viability_dev_v2.py",
                    "evaluate_point_grid_viability_dev_v2.slurm",
                ),
            )
        ),
        "evaluator_file_hash_matches": (
            _sha256(args.execution_root / "evaluate_point_grid_viability_dev_v2.py")
            == identity.get("evaluator_sha256")
        ),
        "slurm_file_hash_matches": (
            _sha256(args.execution_root / "evaluate_point_grid_viability_dev_v2.slurm")
            == identity.get("slurm_sha256")
        ),
    ...
    try:
        for row_index, row in enumerate(rows):
            ...
            parse_point_grid_spec(raw_spec)
    ...
    payload = {
        "schema_version": "point-grid-action-development-audit-v2-r1",
        "supersedes_failed_audit_sha256": _sha256(
            Path("var/artifacts/point_grid_action_dev_v2_audit.json")
        ),
        "audit_correction": (
            "The original audit incorrectly required every wall-legal guided choice "
            "to end at the goal and hashed an audit-generated __pycache__. This "
            "revision checks action-space membership, replays the official verifier, "
            "and hashes the two identity-bound execution files."
        ),
        "execution_snapshot_extra_files": sorted(
            path.relative_to(args.execution_root).as_posix()
            for path in args.execution_root.rglob("*")
            if path.is_file()
