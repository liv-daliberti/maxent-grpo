#!/usr/bin/env python3
"""Run the frozen constrained AntMaze v13 development viability gate."""
...
from oat_drgrpo.ant_maze_interactive_process import AntMazeInteractiveProcess
from oat_drgrpo.maze_modebench import ANT_MAZE_VERIFIER, parse_maze_action_spec
...
def _spec(row):
    ...
    if parsed.verifier != ANT_MAZE_VERIFIER:
        raise ValueError("v13 viability received a non-AntMaze row")
    if tuple(parsed.action_tokens) != ANT_POLICY_ACTIONS:
        raise ValueError("v13 AntMaze action support changed")
    return value
...
def _output_path() -> Path:
    try:
        ...
        raise ValueError("AntMaze v13 evaluator requires --output") from error
...
def main() -> None:
    ...
    payload["schema_version"] = "ant-maze-interactive-viability-v13"
    payload["domain"] = "ant_maze"
    payload["decision"] = (
        "eligible_for_ant_v13_paired_online_smoke"
        if payload.get("status") == "pass"
        else "ant_maze_v13_stage_b_stopped"
    )
    payload["sampling"]["policy_interface"] = (
        "closed_loop_public_markov_compass_token_v13"
    )
