#!/usr/bin/env python3
"""Continue the maze-blind Ant controller on balanced 4-unit waypoints."""
...
CONTROLLER_VERSION = "v7"
DEFAULT_OUTPUT = ROOT / "var/maze_runtime/controllers/ant_waypoint_v7"
INITIAL_MODEL = ROOT / "var/maze_runtime/controllers/ant_waypoint_v6.zip"
...
TRAINING_DISTANCES = np.asarray((4.0,), dtype=np.float32)
...
INFORMATION_FIREWALL = (
    ...
    "Only the aggregate v6 receipt and exact v6 model are antecedents."
)
INITIALIZATION_DESCRIPTION = (
    "Exact failed-v6 waypoint weights with a reset optimizer; the normalized "
    "relative waypoint occupies the same two-coordinate input. Training "
    "cycles all eight headings at the operational four-unit distance."
)
TRAINING_SOURCE_PATH = Path(__file__).resolve()
EXTRA_EVALUATION_CHECKS: Callable[[dict], dict[str, bool]] | None = None
...
class AntWaypointEnv(gym.Wrapper):
    ...
    def reset(self, *, seed=None, options=None):
        ...
        heading_index = (self.rank + self.episode_index) % len(HEADINGS)
        self.episode_index += 1
        target_distance = WAYPOINT_DISTANCE
        self.target_xy = (
...
def main() -> None:
    ...
    evaluation = _evaluate(
        loaded,
        seed=args.seed + EVALUATION_SEED_OFFSET,
    ...
    if EXTRA_EVALUATION_CHECKS is not None:
        checks.update(EXTRA_EVALUATION_CHECKS(summary))
    status = "development_smoke" if args.development_smoke else (
        "pass" if all(checks.values()) else "fail"
    )
    receipt = {
        "schema_version": f"ant-waypoint-controller-{CONTROLLER_VERSION}-evaluation-v1",
        ...
        "decision": (
            ...
            if args.development_smoke
                ...
                f"admitted_to_fresh_maze_route_gate_{CONTROLLER_VERSION}"
                if status == "pass"
                else f"ant_waypoint_{CONTROLLER_VERSION}_ineligible"
            )
        ),
        "information_firewall": INFORMATION_FIREWALL,
        ...
        "hashes": {
            ...
            "training_source_sha256": _sha256(TRAINING_SOURCE_PATH),
        },
    }
    ...
    print(
        f"[ant-waypoint-{CONTROLLER_VERSION}] "
