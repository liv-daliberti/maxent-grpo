#!/usr/bin/env python3
"""Fail-closed K=16 qualification for horizontal-repair PointMaze v4."""
...
def main() -> None:
    ...
    if args.output.exists():
        raise FileExistsError("fresh PointMaze v4 qualification required")
    ...
    per_row_counts = {
        index: sum(
            int(attempt.get("row_index", -1)) == index
    ...
    checks = {
        ...
        "four_prompts_sixty_four_each": (
            ...
            and per_row_counts == {0: 64, 1: 64, 2: 64, 3: 64}
        ),
        ...
        "viability_sampling_contract": (
            viability.get("sampling", {}).get("seed") == 76550
        ...
        "balanced_harder_data_identity": (
            data.get("schema_version")
            == "point-maze-algorithm-repair-data-v4"
            and data.get("orientation_counts") == expected_counts
            and data.get("executable_task_overlap_count") == 0
            and data.get("horizontal_geometry_repair") is True
            and data.get("horizontal_family_size") == 13
            and data.get("horizontal_route_counts") == [13, 32, 16]
        ),
        "hash_chain": (
    ...
    payload = {
        "schema_version": (
            "point-maze-algorithm-repair-v4-qualification-v1"
        ),
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "status": status,
        "decision": (
            "eligible_for_point_maze_algorithm_repair_v4_pair"
            if status == "pass"
            else "point_maze_algorithm_repair_v4_stopped"
        ),
        "checks": checks,
        "errors": errors,
        "summary": {
