#!/usr/bin/env python3
"""Train the frozen command-conditioned Ant locomotion controller.

This auxiliary controller is trained only on the open-plane Ant-v5 task. It
never sees a maze map, route gate, ModeBench prompt, or MaxEnt/Dr.GRPO outcome.
The resulting policy is frozen and shared byte-for-byte by both 0.5B arms.
"""
...
DEFAULT_OUTPUT = ROOT / "var/maze_runtime/controllers/ant_heading_v1"
COMMANDS = np.asarray(
    [
        (0.0, 1.0),
...
class AntHeadingEnv(gym.Wrapper):
    """Open-plane Ant with an episode-fixed discrete heading command."""
    ...
    def _observation(self, observation) -> np.ndarray:
        return np.concatenate(
            [np.asarray(observation, dtype=np.float32), self.command]
...
def _factory(seed: int, rank: int, episode_steps: int) -> Callable[[], gym.Env]:
    def make() -> gym.Env:
        env = AntHeadingEnv(seed=seed + 10_000 * rank, episode_steps=episode_steps)
...
def main() -> None:
    ...
    receipt = {
        "schema_version": "ant-heading-controller-training-v1",
