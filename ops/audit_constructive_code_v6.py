#!/usr/bin/env python3
"""Fail-closed audit for ConstructiveCode v6 paired smoke or Stage B."""
...
def validate_run(
    *,
    ...
    expected_updates = (
        len(train.DEVELOPMENT_PROBLEMS) if mode == train.PAIRED_MODE else 48
    )
    expected_candidates = expected_updates * train.SAMPLES
    if (
        receipt.get("schema") != "constructive-code-v6-run-receipt-v1"
        ...
        or receipt.get("curation_manifest_sha256")
        != train.sha256_file(curation_manifest)
    ...
    evaluation_programs = len(train.EVALUATION_PROBLEMS) * (1 + 4 * 8)
    expected_eval_programs = (
        0 if mode == train.PAIRED_MODE else 49 * evaluation_programs
    )
    if (
    ...
    training_metrics = [
        row
        for row in metrics
        if row.get("schema") == "constructive-code-v6-training-metric-v1"
    ]
    evaluation_metrics = [
        row
        for row in metrics
        if row.get("schema")
        == "constructive-code-v6-stage-b-evaluation-metric-v1"
    ]
    if (
    ...
    for row in candidates:
        code = row.get("code")
        if (
            row.get("schema") != "constructive-code-v6-training-candidate-v1"
    ...
    if mode == train.STAGE_B_MODE:
        if (
            ...
            or any(row.get("programs") != evaluation_programs for row in evaluations)
            or any(
                not isinstance(row.get("tasks"), list)
                or len(row["tasks"]) != len(train.EVALUATION_PROBLEMS)
                or any(
...
def main() -> None:
    ...
    if (
        ...
        != train.sha256_file(args.curation_manifest)
    ...
    expected_second_replays = (
        2 * len(train.DEVELOPMENT_PROBLEMS) * train.SAMPLES
        if args.mode == train.PAIRED_MODE
        else 7680
    )
    if (
    ...
    payload = {
        "schema": (
            "constructive-code-v6-paired-smoke-audit-v1"
            if args.mode == train.PAIRED_MODE
            else "constructive-code-v6-stage-b-audit-v1"
        ),
        ...
        "curation_manifest_sha256": train.sha256_file(args.curation_manifest),
