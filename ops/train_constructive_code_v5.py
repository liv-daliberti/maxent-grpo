#!/usr/bin/env python3
"""Run one frozen ConstructiveCode v5 paired-smoke or Stage-B arm."""
...
TRAIN_PROBLEMS = ("327_B", "659_C", "1208_C", "1102_B")
...
def _replay_modules():
    ...
    replay_v5, base, materialize = v5._replay_modules()
    return v5, replay_v5, base, materialize
...
def _selected_suites(
    ...
    if (
        ...
        raise ValueError("ConstructiveCode v5 gate is not an exact pass")
    task_rows = gate.get("task_results")
    suite_rows = gate.get("suite_results")
    if not isinstance(task_rows, list) or len(task_rows) != 12:
        raise ValueError("ConstructiveCode v5 gate lacks 12 task results")
    if not isinstance(suite_rows, list):
        raise ValueError("ConstructiveCode v5 gate lacks suite results")
    task_status = {
    ...
    for problem_id, problem_key in problem_keys.items():
        if task_status.get(problem_key) != "pass":
            raise ValueError(f"v5 task did not pass: {problem_id}")
        if suite_status.get((problem_key, OVERLAY_SUITE)) == "pass":
            ...
            raise ValueError(f"v5 task has no admitted suite: {problem_id}")
    return selected
...
def load_frozen_tasks(
    *,
    ...
    if set(summaries) != set(materialize.V5_TASKS):
        raise ValueError("ConstructiveCode v5 task manifest drift")
    ...
    if any(problem_id not in summaries for problem_id in problem_ids):
        raise ValueError("ConstructiveCode task request left the v5 slate")
    ...
    for problem_id in problem_ids:
        ...
        if (
            ...
            raise ValueError(f"v5 task identity or split drift: {problem_id}")
        ...
        if not isinstance(language, Mapping) or (
            ...
            raise ValueError(f"v5 language contract drift: {problem_id}")
        ...
        if language.get(base.V1_LEDGER_COUNT_FIELD) != len(v1_hashes):
            raise ValueError(f"v5 logical v1 ledger drift: {problem_id}")
        replay_path = task_dir / "py3_replays.jsonl"
        if base._sha256_path(replay_path) != record.get("replays", {}).get(
            "jsonl_sha256"
        ):
            raise ValueError(f"v5 replay ledger drift: {problem_id}")
        ...
        if not isinstance(raw_limits, Mapping):
            raise ValueError(f"v5 limits missing: {problem_id}")
        ...
        if (
            ...
            raise ValueError(f"v5 limits malformed: {problem_id}")
        cpu_seconds = max(
        ...
        if not isinstance(suites, Mapping) or set(suites) != base.REQUIRED_SUITE_IDS:
            raise ValueError(f"v5 suite metadata drift: {problem_id}")
        ...
        if not isinstance(statement, str) or not statement.strip():
            raise ValueError(f"v5 public statement missing: {problem_id}")
    ...
    if tuple(tasks) != tuple(problem_ids) or tuple(public_rows) != tuple(problem_ids):
        raise RuntimeError("ConstructiveCode v5 task order drift")
...
def evaluation_coordinate(
    *,
    ...
    greedy_value = sum(row["greedy"] for row in task_summaries) / 4.0
    metric = {
        "schema": "constructive-code-v5-stage-b-evaluation-metric-v1",
        ...
        / 16.0,
        "evaluation_programs": 132,
        "evaluation_tasks": 4,
        "evaluation_draws_per_task": 4,
    ...
    ledger = {
        "schema": "constructive-code-v5-stage-b-evaluation-ledger-v1",
        "update": update,
        "seed": seed,
        "programs": 132,
        "tasks": ledger_tasks,
...
def main() -> None:
    ...
    if tuple(public_rows[key]["witness_family"] for key in training_ids) != (
        EXPECTED_FAMILIES
    ):
    ...
    total_updates = 4 if args.mode == PAIRED_MODE else 48
    training_attempts = 0
    ...
    for update in range(1, total_updates + 1):
        ...
        for attempt in attempts:
            append_jsonl(
                ...
                [
                    {
                        "schema": "constructive-code-v5-training-candidate-v1",
                        "mode": args.mode,
                        "arm": args.arm,
                        "seed": args.seed,
                        "update": update,
        ...
        metric = {
            "schema": "constructive-code-v5-training-metric-v1",
            "mode": args.mode,
            "arm": args.arm,
            "seed": args.seed,
            "trainer/global_step": update,
    ...
    payload = {
        "schema": "constructive-code-v5-run-receipt-v1",
        "status": "complete",
        "mode": args.mode,
        "arm": args.arm,
        "seed": args.seed,
        ...
        "counts": {
            ...
            * RESPONSE_TOKENS,
            ...
            "evaluation_programs": 0 if args.mode == PAIRED_MODE else 49 * 132,
        },
    ...
    print(
        "[constructive-v5-train] complete "
        f"mode={args.mode} arm={args.arm} seed={args.seed} "
