#!/usr/bin/env python3
"""Aggregate the three frozen AntMaze v9 node receipts."""
...
def main() -> None:
    ...
    if args.output.exists():
        raise FileExistsError(f"fresh Ant v9 cross-node aggregate required: {args.output}")
    ...
    payload = {
        "schema_version": "ant-maze-v9-cross-node-audit-v1",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "status": "pass" if passed else "fail",
        "decision": (
            "eligible_for_frozen_05b_viability_gate_v9"
            if passed else "ant_maze_v9_stopped"
        ),
    ...
    print(
        f"[ant-v9-cross-node-aggregate] status={payload['status']} "
