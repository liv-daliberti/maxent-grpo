...
GC3B = ROOT / "var/artifacts/gc3b_regression.json"
...
def main():
    fine, grid, gc3b = rows(FINE), rows(GRID), rows(GC3B)
    fig, axes = plt.subplots(2, 2, figsize=(5.5, 4.4), sharex=True)
    specs = [
        # (axes row, data builder, tau0?, adaptive?, row tag)
        (0, lambda o: sorted(series(fine, o, "fine") + series(grid, o, "grid")),
         True, True, "0.5B Countdown"),
        (1, lambda o: sorted(series(gc3b, o, "fine")), False, False,
         "3B graph coloring"),
    ]
    for row, build, want_tau0, want_adapt, tag in specs:
        for col, (outcome, ylab) in enumerate(
            (("pass@8", "$\\Delta$ pass@8 (pts)"),
             ("coverage@8", "$\\Delta$ coverage@8 (pts)"))
        ):
            ax = axes[row][col]
            pts = build(outcome)
            xs = [p[0] for p in pts]
            ...
            if want_tau0:
                z = next((r for r in fine if r["outcome"] == outcome
                          and r["treatment"] == "xdr_tau0"), None)
                if z:
                    zg, zl, zh = (100 * z["gamma"], 100 * z["ci_low"],
                                  100 * z["ci_high"])
                    ax.errorbar([2.6e-5], [zg], yerr=[[zg - zl], [zh - zg]],
                                fmt="s", ms=4.5, color=HUE, mfc="white",
                                ecolor=HUE, elinewidth=0.9, capsize=2.0,
                                zorder=4)
                    ax.annotate(r"$\tau{=}0$", (2.6e-5, zg),
                                textcoords="offset points", xytext=(0, 7),
                                ha="center", fontsize=7, color=HUE,
                                annotation_clip=False)
            if want_adapt:
                a = adapt_row(fine, outcome)
                if a:
                    ax.errorbar([3.6], [a[0]],
                                yerr=[[a[0] - a[1]], [a[2] - a[0]]],
                                fmt="D", ms=4.5, color=ADAPT, ecolor=ADAPT,
                                elinewidth=0.9, capsize=2.0, zorder=4)
                    ax.annotate("adaptive", (3.6, a[0]),
                                textcoords="offset points", xytext=(-1, 7),
                                ha="right", fontsize=7, color=ADAPT,
                                annotation_clip=False)
            ax.set_xscale("log")
            ...
            ax.set_ylabel(ylab)
            ax.set_title(f"{outcome} — {tag}", loc="left", fontsize=8)
            ax.grid(axis="y", color="#dddddd", lw=0.5, zorder=0)
            ax.spines[["top", "right"]].set_visible(False)
            ax.tick_params(length=2.5, width=0.7)
    handles = [
        plt.Line2D([], [], marker="o", ls="-", color=HUE, mfc=HUE, ms=4.5,
                   lw=1.4, label="same-round arms"),
        plt.Line2D([], [], marker="o", ls="-", color=HUE, mfc="white", ms=4.5,
                   lw=1.4, label="earlier-round arms"),
        plt.Line2D([], [], marker="s", ls="", color=HUE, mfc="white", ms=4.5,
                   label=r"$\tau{=}0$ (argmax set)"),
        plt.Line2D([], [], marker="D", ls="", color=ADAPT, ms=4.5,
                   label="mode-adaptive"),
    ]
    axes[0][0].legend(handles=handles, frameon=False, fontsize=6.5,
                      loc="upper right", handlelength=1.5, borderaxespad=0.1)
