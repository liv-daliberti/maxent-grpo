#!/usr/bin/env python3
"""Train the frozen train-only PointMaze finite-action warm start."""
...
def main() -> None:
    ...
    if (
        ...
        or args.gradient_accumulation != 7
        or (
            args.frozen_short_balanced_v6
            and (
                args.policy_interface != "velocity_state_v3"
                or args.warmup_steps != 7
            )
        )
    ):
        raise ValueError("PointMaze warm-start update schedule changed")
    ...
    if len(examples) != 644 or len(examples) != identity["example_count"]:
