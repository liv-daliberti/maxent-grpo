...
OUTPUT_ROOT="$ROOT_DIR/var/artifacts/e69_math_route_gate1_base_equation_v3"
if [[ -e "$OUTPUT_ROOT" ]]; then
  echo "E69 equation-v3 Gate 1 archive already exists: $OUTPUT_ROOT" >&2
  exit 1
fi
...
identity="$(
  sha256sum \
    ...
    src/oat_drgrpo/math_equation_route.py \
    ...
    paper/preregistration/e69_math_route_gate1_equation_v3_amendment_20260728.md \
...
job_id="$(
  sbatch \
    ...
    "$ROOT_DIR/ops/slurm/e69_math_equation_gate1_node302.slurm"
)"
printf 'submitted E69 equation-v3 Gate 1 job %s input_identity=%s\n' \
  "$job_id" "$identity"
