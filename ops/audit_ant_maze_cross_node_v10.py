#!/usr/bin/env python3
"""Replay the exact admitted AntMaze v10 slate on one cluster node."""
...
from oat_drgrpo.ant_maze_worker_v10 import (  # noqa: E402
...
def main() -> None:
    ...
    if args.output.exists():
        raise FileExistsError(f"fresh Ant v10 node receipt required: {args.output}")
    export = json.loads(args.specs.read_text(encoding="utf-8"))
    if export.get("schema_version") != EXPORT_SCHEMA:
        raise ValueError(f"Ant {VERSION} cross-node spec schema mismatch")
    if export.get("controller_receipt_sha256") != controller_receipt_sha256():
        raise ValueError("Ant v10 cross-node export binds the wrong controller")
    records = []
    for row in export["rows"]:
        ...
        for route in row["routes"]:
            ...
            for repetition in range(args.repetitions):
                ...
                error = None
                try:
                    execution = execute_ant_v10_raw(candidate, spec)
    ...
    print(
        f"[ant-{VERSION}-cross-node] replica={args.replica} host={payload['hostname']} "
