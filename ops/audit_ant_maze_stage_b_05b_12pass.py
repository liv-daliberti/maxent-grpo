#!/usr/bin/env python3
"""Fail-closed terminal audit for the ten AntMaze Stage-B paper cells."""
...
UPDATES = 48
EVAL_ROUNDS = tuple(range(0, UPDATES + 1))
MAP_IDS = (
...
def replay_cell(
    *,
    ...
    encoded_actions = [
        ...
        for label in ANT_POLICY_ACTIONS
    ]
    if any(
        len(ids) != 1 or tokenizer.decode(ids) != label
        for label, ids in zip(ANT_POLICY_ACTIONS, encoded_actions)
    ):
        return [f"{arm}/s{seed}: Ant action support is not exact one-token"], counts
    action_ids = tuple(int(ids[0]) for ids in encoded_actions)
    with AntMazeInteractiveProcess(worker_python=worker_python) as worker:
        for expected_update, payload in enumerate(replay_rows, start=1):
            ...
            if (
                payload.get("schema") != "ant-maze-stage-b-state-replay-v1"
            ...
            if (
                ...
                or spec.max_actions != 16
                or spec.action_repeat != 400
            ):
                errors.append(f"{label}: Ant action/controller contract mismatch")
                continue
...
def main() -> None:
    ...
    if parsed.output.exists():
        raise FileExistsError("fresh AntMaze Stage-B audit output required")
    ...
    checks["identity_contract"] = (
        ...
        and identity.get("train_prompt_count") == 4
        and identity.get("decision_horizon") == 16
        and identity.get("action_repeat") == 400
    )
    checks["submission_contract"] = (
        submission.get("schema") == "ant-maze-stage-b-05b-12pass-submission-v1"
    ...
    checks["model_snapshot"] = tree_hash(root / "var/models/ant_maze_interactive_warmstart_v13") == identity.get("model_tree_sha256")
    checks["data_snapshot"] = tree_hash(root / "var/data/ant_maze_modebench_v12") == identity.get("data_tree_sha256")
    ...
    checks["qualification_pass"] = (
        ...
        and qualification_payload.get("decision") == "eligible_for_ten_ant_maze_stage_b_jobs"
        and sha(qualification) == identity.get("qualification_audit_sha256")
    )

    from datasets import load_from_disk
    from transformers import AutoTokenizer
    train_rows = load_from_disk(str(root / "var/data/ant_maze_modebench_v12/train"))["train"].to_list()
    ...
    for row in manifest_rows:
        ...
        training = [item for item in metrics if item.get("schema") == "ant-maze-stage-b-training-metric-v1"]
        evaluations = [item for item in metrics if item.get("schema") == "ant-maze-stage-b-evaluation-v1"]
        ...
        for expected_update, item in enumerate(training, start=1):
            for key, expected in {
                "fixed_policy_slots": 256, "replay_decision_forward_slots": 256,
        ...
        for evaluation in evaluations:
            ...
            for map_id in MAP_IDS:
                if not finite(evaluation.get(f"eval/{map_id}/greedy")):
                    errors.append(f"{label}/eval{round_index}: missing {map_id} greedy")
                for draw in range(4):
                    for stem in ("mean8", "pass8", "distinct8"):
                        if not finite(evaluation.get(f"eval/{map_id}/{stem}_draw_{draw}")):
                            errors.append(f"{label}/eval{round_index}: missing {map_id}/{stem}/{draw}")
        if (
            receipt.get("schema") != "ant-maze-stage-b-05b-12pass-receipt-v1"
            ...
            or receipt.get("information_boundary", {}).get("fixed_v11_low_level_controller") is not True
        ):
    ...
    payload = {
        "schema": "ant-maze-stage-b-05b-12pass-audit-v1",
        "status": "pass" if not errors else "fail",
        "decision": "ant_maze_terminal_eligible" if not errors else "ant_maze_stage_b_stopped",
