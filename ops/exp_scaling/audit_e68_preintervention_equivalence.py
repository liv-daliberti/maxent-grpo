#!/usr/bin/env python3
"""Audit E66/E68 trajectory equivalence before the singleton actuator fires."""
...
MANIFESTS = {
    "graph_coloring": (
        ...
        ROOT
        / "var/artifacts/gce68_separated_support_actuator_05b_12ep_comparative_jobs.tsv",
    ),
    "countdown": (
        ROOT
        / "var/artifacts/cde66_same_plumbing_control_05b_12ep_comparative_jobs.tsv",
        ROOT
        / "var/artifacts/cde68_separated_support_actuator_05b_12ep_comparative_jobs.tsv",
    ),
    "python_factor": (
        ROOT
        / "var/artifacts/pye66_same_plumbing_control_05b_12ep_comparative_jobs.tsv",
        ROOT
        / "var/artifacts/pye68_separated_support_actuator_05b_12ep_comparative_jobs.tsv",
    ),
    "mathir": (
        ROOT
        / "var/artifacts/mie66_same_plumbing_control_05b_12ep_comparative_jobs.tsv",
        ROOT
        / "var/artifacts/mie68_separated_support_actuator_05b_12ep_comparative_jobs.tsv",
    ),
}
EXACT_KEYS = (
...
def main() -> None:
    missing_manifests = [
        ...
        for pair in MANIFESTS.values()
        for path in pair
        if not path.is_file()
    ]
    if missing_manifests:
        payload = {
            "schema": "e68_preintervention_equivalence_audit_v1",
        ...
        print(
            "[e68-equivalence] status=not_submitted "
            "pairs=0/12 ready=0/12 violations=0"
        )
        return
    violations: list[str] = []
    ...
    os.replace(temporary, OUT)
    print(
        f"[e68-equivalence] status={status} "
