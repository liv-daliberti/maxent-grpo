#!/usr/bin/env bash
# Freeze and atomically submit E67's 12-job prospective repair cohort.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e67_corrected_same_objective_actuator_ablation_05b.md"
...
GRAPH_PREFIX=gce67_corrected_same_objective_actuator_05b_12ep
COUNTDOWN_PREFIX=cde67_corrected_same_objective_actuator_05b_12ep
PYTHON_PREFIX=pye67_corrected_same_objective_actuator_05b_12ep
MATHIR_PREFIX=mie67_corrected_same_objective_actuator_05b_12ep
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing E67 prerequisite: $required" >&2
    exit 1
  fi
done
...
if [[ "$phase" == full ]]; then
  for fresh in \
    ...
    if [[ -e "$fresh" ]]; then
      echo "Fresh E67 artifact required; already exists: $fresh" >&2
      exit 1
    fi
  done
fi
...
freeze_execution() {
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E67 source snapshot hash mismatch" >&2
    exit 1
  fi

  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e67-ops-input.XXXXXX")"
  ...
  OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e67_corrected_actuator_ops_${EXECUTION_HASH}/ops"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E67 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
submit_domain() {
  ...
  case "$domain" in
    graph_coloring)
      ...
      export OAT_ZERO_TRAIN_NODELIST=node103
    ...
    countdown)
      ...
      export OAT_ZERO_TRAIN_NODELIST=node022,node026
...
if [[ "$phase" == config ]]; then
  ...
  echo "[e67] all four configurations passed"
  exit 0
fi
...
for manifest in \
  ...
  if [[ "${#manifest_jobs[@]}" -ne 3 ]]; then
    echo "E67 manifest $manifest has ${#manifest_jobs[@]} jobs; expected 3" >&2
    exit 1
  fi
...
payload = {
    "schema": "e67_corrected_same_objective_actuator_ablation_v1",
...
if [[ "${#job_ids[@]}" -ne 12 ]]; then
  echo "E67 expected 12 jobs; got ${#job_ids[@]}" >&2
  exit 1
fi
for job_id in "${job_ids[@]}"; do
  ...
  for required in \
    ...
    if [[ "$record" != *"$required"* ]]; then
      echo "E67 held-job audit failed for $job_id: missing $required" >&2
      exit 1
    fi
  done
done
...
released=1
trap - EXIT
echo "[e67] released 12 jobs: ${job_ids[*]}"
echo "[e67] identity=$IDENTITY"
