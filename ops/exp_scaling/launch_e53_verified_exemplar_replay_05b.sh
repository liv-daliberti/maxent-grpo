#!/usr/bin/env bash
# Launch E53's fresh 50-pass, three-domain target-free sentinel.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e53_verified_exemplar_replay_05b.md"
AUDITOR="$ROOT_DIR/ops/exp_scaling/audit_e53_sentinel.py"
...
IDENTITY="$ROOT_DIR/var/artifacts/e53_verified_replay_05b_sentinel_identity.json"
EXPECTED_JOBS=9
PROMPT_EPOCHS=50
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing E53 prerequisite: $required" >&2
    exit 1
  fi
done
if [[ "$phase" == sentinel && -e "$IDENTITY" ]]; then
  echo "Fresh E53 identity required; file exists: $IDENTITY" >&2
  exit 1
fi
...
freeze_execution() {
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E53 source snapshot hash mismatch" >&2
    exit 1
  fi

  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e53-ops-input.XXXXXX")"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E53 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
write_identity() {
  local protocol_hash launcher_hash auditor_hash
  ...
  "$PYTHON_BIN" - "$IDENTITY" "$protocol_hash" "$launcher_hash" \
    "$auditor_hash" "$SOURCE_HASH" "$EXECUTION_HASH" <<'PY'
import json
import os
import pathlib
import sys
...
payload = {
    "schema": "e53_verified_exemplar_replay_05b_sentinel_v1",
    ...
    "replay": {
        "loss": "KL_uniform_to_model_observed_exemplar_scores_v1",
        "base_alpha": 0.10,
        "capacity": 16,
...
submit_domain() {
  ...
  case "$domain" in
    ...
    python_factor)
      ...
      ;;
    *)
      echo "Unknown E53 domain: $domain" >&2
      exit 1
      ;;
  esac
...
if [[ "$phase" == config ]]; then
  ...
  echo "[e53] all three sentinel configurations passed; no jobs submitted"
  exit 0
fi
...
cleanup_partial_cohort() {
  ...
  if [[ "$COHORT_RELEASED" != "1" && "${#job_ids[@]}" -gt 0 ]]; then
    ...
    echo "[e53] cancelled incomplete held sentinel: ${job_ids[*]}" >&2
  fi
  exit "$status"
}
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX" "$PYTHON_PREFIX"; do
  ...
  if [[ "${#task_jobs[@]}" -ne 3 ]]; then
    echo "E53 ${prefix} has ${#task_jobs[@]} jobs; expected 3" >&2
    exit 1
  fi
  job_ids+=("${task_jobs[@]}")
...
if observed != expected or len(rows) != 3:
    raise SystemExit(
        f"E53 manifest arm mismatch: expected={sorted(expected)} "
...
if [[ "${#job_ids[@]}" -ne "$EXPECTED_JOBS" ]]; then
  echo "E53 held cohort has ${#job_ids[@]} jobs; expected $EXPECTED_JOBS" >&2
  exit 1
fi
scontrol release "${job_ids[@]}"
COHORT_RELEASED=1
trap - EXIT
echo "[e53] released held sentinel cohort: ${job_ids[*]}"
