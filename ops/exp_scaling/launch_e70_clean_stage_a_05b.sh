#!/usr/bin/env bash
# Configure or atomically submit E70 Stage A's 40-job matched cohort.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e70_clean_verified_maxent_vs_compute_matched_drgrpo_stage_a_05b.md"
...
IDENTITY="$ROOT_DIR/var/artifacts/e70_clean_stage_a_05b_identity.json"

GRAPH_PREFIX=gce70_clean_stage_a_05b_12pass
COUNTDOWN_PREFIX=cde70_clean_stage_a_05b_12pass
PYTHON_PREFIX=pye70_clean_stage_a_05b_12pass
MATHIR_PREFIX=mie70_clean_stage_a_05b_12pass
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing E70 Stage-A prerequisite: $required" >&2
    exit 1
  fi
done
...
for raw in sys.argv[1:]:
    ...
    if payload.get("status") != "pass" or payload.get("violations"):
        raise SystemExit(f"E70 Stage-A requires a clean smoke certificate: {path}")
PY
...
if [[ "$phase" == full ]]; then
  for fresh in \
    ...
    if [[ -e "$fresh" ]]; then
      echo "Fresh E70 Stage-A artifact required; already exists: $fresh" >&2
      exit 1
    fi
  done
fi
...
freeze_execution() {
  ...
  SOURCE_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e70_clean_stage_a_${SOURCE_HASH}/src"
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E70 Stage-A source snapshot hash mismatch" >&2
    exit 1
  fi

  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e70-stage-a-ops-input.XXXXXX")"
  ...
  OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e70_clean_stage_a_ops_${EXECUTION_HASH}/ops"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E70 Stage-A execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
payload = {
    "schema": "e70_clean_stage_a_05b_v1",
...
submit_domain() {
  ...
  case "$domain" in
    ...
    mathir)
      ...
      ;;
    *)
      echo "Unknown E70 Stage-A domain: $domain" >&2
      exit 1
      ;;
  esac
...
if [[ "$phase" == config ]]; then
  ...
  echo "[e70] all four five-seed 12-pass configurations passed"
  exit 0
fi
...
cleanup_held() {
  ...
  if [[ "$released" != "1" && "${#job_ids[@]}" -gt 0 ]]; then
    ...
    echo "[e70] cancelled incomplete held cohort: ${job_ids[*]}" >&2
  fi
  exit "$status"
}
...
for manifest in \
  ...
  if [[ "${#manifest_jobs[@]}" -ne 10 ]]; then
    echo "E70 Stage-A manifest $manifest has ${#manifest_jobs[@]} jobs; expected 10" >&2
    exit 1
  fi
  job_ids+=("${manifest_jobs[@]}")
done
if [[ "${#job_ids[@]}" -ne 40 ]]; then
  echo "E70 Stage-A cohort has ${#job_ids[@]} jobs; expected 40" >&2
  exit 1
fi
...
for job_id in "${job_ids[@]}"; do
  ...
  for required in \
    ...
    if [[ "$job_record" != *"$required"* ]]; then
      echo "E70 Stage-A held-job audit failed for $job_id: missing $required" >&2
      exit 1
    fi
  done
  if [[ "$job_record" == *'OAT_ZERO_VARIANT=verified_first_global_replay_canonical'* ]]; then
    for required in \
      ...
      if [[ "$job_record" != *"$required"* ]]; then
        echo "E70 Stage-A treatment audit failed for $job_id: missing $required" >&2
        exit 1
      fi
    done
    ...
    for required in \
      ...
      if [[ "$job_record" != *"$required"* ]]; then
        echo "E70 Stage-A baseline audit failed for $job_id: missing $required" >&2
        exit 1
      fi
    done
  else
    echo "E70 Stage-A job $job_id is neither frozen arm" >&2
    exit 1
  fi
done
...
released=1
trap - EXIT
echo "[e70] released 40 matched jobs: ${job_ids[*]}"
echo "[e70] identity=$IDENTITY"
