#!/usr/bin/env bash
# Launch E49T's matched three-epoch natural-menu hard-MATH toy.
set -euo pipefail
...
PROTOCOL="${E49T_TOY_PROTOCOL:-$ROOT_DIR/paper/preregistration/e49t_menu_inferred_math_haarnoja_05b.md}"
CALIBRATION_DIR="$ROOT_DIR/var/artifacts/e49t_route_confusion_calibration_v1"
CALIBRATION="$CALIBRATION_DIR/result.json"
COHORT_IDENTITY="$CALIBRATION_DIR/frozen_identity.json"
DECLARATION_CALIBRATION_DIR="$ROOT_DIR/var/artifacts/e49t_declaration_mismatch_calibration_v1"
DECLARATION_CALIBRATION="$DECLARATION_CALIBRATION_DIR/result.json"
DECLARATION_IDENTITY="$DECLARATION_CALIBRATION_DIR/frozen_identity.json"
DECLARATION_PROTOCOL="$ROOT_DIR/paper/preregistration/e49t_declaration_mismatch_calibration_20260726.md"
DATA_ROOT="${E49T_TOY_DATA_ROOT:-$ROOT_DIR/var/data/e49t_natural_menu_math_toy}"
DATA_MANIFEST="$DATA_ROOT/MATERIALIZATION_MANIFEST.json"
ENDPOINT_RECORD="${E49T_QWEN72_ENDPOINT_RECORD:-$ROOT_DIR/var/artifacts/e49t_qwen72_node302_v1/qwen72_endpoint.json}"
...
PREFIX="${E49T_TOY_PREFIX:-e49t_natural_menu_math_toy_05b_3ep_v1}"
DATA_SCHEMA="${E49T_TOY_DATA_SCHEMA:-e49t_natural_menu_materialization_v1}"
EXPECTED_TRAIN_MULTI="${E49T_TOY_TRAIN_MULTI:-10}"
EXPECTED_EVAL_MULTI="${E49T_TOY_EVAL_MULTI:-10}"
MANIFEST="$ROOT_DIR/var/artifacts/${PREFIX}_comparative_jobs.tsv"
IDENTITY="$ROOT_DIR/var/artifacts/${PREFIX}_identity.json"
SOURCE_ROOT_OVERRIDE="${E49T_SOURCE_ROOT_OVERRIDE:-}"
CANONICALIZER_SOURCE_OVERRIDE="${E49T_CANONICALIZER_SOURCE_OVERRIDE:-}"
OPS_ROOT_OVERRIDE="${E49T_OPS_ROOT_OVERRIDE:-}"
EXTRA_GATE_RESULT="${E49T_EXTRA_GATE_RESULT:-}"
...
for required in \
  "$PYTHON_BIN" "$PROTOCOL" "$CALIBRATION" "$COHORT_IDENTITY" \
  "$DECLARATION_CALIBRATION" "$DECLARATION_IDENTITY" \
  "$DECLARATION_PROTOCOL" \
  "$CANONICALIZER_SOURCE" \
  "$DATA_MANIFEST" "$ENDPOINT_RECORD" "$MODEL_ROOT/model.safetensors" \
  "$DATA_ROOT/train/dataset_dict.json" "$DATA_ROOT/eval/dataset_dict.json"; do
  if [[ ! -e "$required" ]]; then
    echo "E49T frozen prerequisite is missing: $required" >&2
    exit 1
  fi
done
if [[ -n "$EXTRA_GATE_RESULT" && ! -f "$EXTRA_GATE_RESULT" ]]; then
  echo "matched toy extra gate is missing: $EXTRA_GATE_RESULT" >&2
  exit 1
fi

"$PYTHON_BIN" - "$CALIBRATION" "$COHORT_IDENTITY" "$ENDPOINT_RECORD" \
  "$CANONICALIZER_SOURCE" <<'PY'
import hashlib
import json
import pathlib
import sys
calibration_path, cohort_path, endpoint_path, source_path = map(
...
if (
    ...
    raise SystemExit("E49T route-confusion calibration has not passed")
def sha(path):
    return hashlib.sha256(path.read_bytes()).hexdigest()
identities = calibration.get("identities", {})
if identities.get("cohort_identity_sha256") != sha(cohort_path):
    raise SystemExit("E49T calibration cohort identity mismatch")
if identities.get("endpoint_record_sha256") != sha(endpoint_path):
    raise SystemExit("E49T calibration endpoint identity mismatch")
if identities.get("canonicalizer_sha256") != sha(source_path):
    raise SystemExit("E49T calibration canonicalizer source identity mismatch")
PY

"$PYTHON_BIN" - "$DECLARATION_CALIBRATION" "$DECLARATION_IDENTITY" \
  "$ENDPOINT_RECORD" "$CANONICALIZER_SOURCE" <<'PY'
import hashlib
import json
import pathlib
import sys
result_path, identity_path, endpoint_path, source_path = map(
    pathlib.Path, sys.argv[1:]
)
result = json.loads(result_path.read_text(encoding="utf-8"))
if (
    result.get("schema") != "e49t_declaration_mismatch_result_v1"
    or result.get("pass") is not True
    or not all(result.get("checks", {}).values())
):
    raise SystemExit("E49T declaration-mismatch calibration has not passed")
def sha(path):
    return hashlib.sha256(path.read_bytes()).hexdigest()
identities = result.get("identities", {})
if identities.get("cohort_identity_sha256") != sha(identity_path):
    raise SystemExit("E49T declaration cohort identity mismatch")
if identities.get("endpoint_record_sha256") != sha(endpoint_path):
    raise SystemExit("E49T declaration endpoint identity mismatch")
if identities.get("canonicalizer_sha256") != sha(source_path):
    raise SystemExit("E49T declaration canonicalizer source identity mismatch")
PY
...
if (
    manifest.get("schema") != sys.argv[2]
    or manifest.get("row_counts", {}).get("train") != 50
    or manifest.get("row_counts", {}).get("eval") != 50
    or manifest.get("multi_support_counts", {}).get("train") != int(sys.argv[3])
    or manifest.get("multi_support_counts", {}).get("eval") != int(sys.argv[4])
):
    raise SystemExit("invalid finite-menu toy data manifest")
...
if [[ "$TRAIN_DATA_HASH" != "$EXPECTED_TRAIN_HASH" ]] \
  ...
  echo "E49T data identity mismatch" >&2
  exit 1
fi
...
if any(record.get(key) != value for key, value in expected.items()):
    raise SystemExit("unexpected E49T judge endpoint")
...
if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
  echo "E49T source snapshot mismatch" >&2
  exit 1
fi
...
if [[ -n "$OPS_ROOT_OVERRIDE" ]]; then
  ...
  OPS_INPUT="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e49t-ops.XXXXXX")"
  ...
  OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e49t_natural_menu_ops_${OPS_HASH}/ops"
...
if [[ "$mode" == run && -e "$MANIFEST" ]]; then
  echo "Fresh E49T launch required; manifest already exists: $MANIFEST" >&2
  exit 1
fi
...
export OAT_ZERO_EVAL_PROMPT_INTERVAL=50
export OAT_ZERO_EVAL_STEPS=50
...
export OAT_ZERO_SAVE_STEPS=50
export OAT_ZERO_SAVE_FROM=50
export OAT_ZERO_MAX_SAVE_NUM=3
export OAT_ZERO_RESUME_STEPS=50
export OAT_ZERO_RESUME_FROM=50
...
DECLARATION_PROTOCOL_HASH="$(sha256sum "$DECLARATION_PROTOCOL" | cut -d' ' -f1)"
...
"$PYTHON_BIN" - "$IDENTITY" "$SOURCE_HASH" "$OPS_HASH" "$ENDPOINT" \
  ...
  "$CALIBRATION" "$COHORT_IDENTITY" "$DATA_MANIFEST" "$ENDPOINT_RECORD" \
  "$LAUNCHER_HASH" "$DECLARATION_CALIBRATION" "$DECLARATION_IDENTITY" \
  "$DECLARATION_PROTOCOL_HASH" "$PREFIX" "$EXPECTED_TRAIN_MULTI" \
  "$EXPECTED_EVAL_MULTI" "${E49T_WRAPPER_HASH:-}" \
  "$EXTRA_GATE_RESULT" <<'PY'
import hashlib
import json
import os
import pathlib
...
payload = {
    ...
    "calibration_result_sha256": sha256(sys.argv[9]),
    "calibration_cohort_identity_sha256": sha256(sys.argv[10]),
    "data_manifest_sha256": sha256(sys.argv[11]),
    "judge_endpoint_record_sha256": sha256(sys.argv[12]),
    "launcher_sha256": sys.argv[13],
    "wrapper_sha256": sys.argv[20] or None,
    "extra_gate_result_sha256": (
        sha256(sys.argv[21]) if sys.argv[21] else None
    ),
    "declaration_mismatch_result_sha256": sha256(sys.argv[14]),
    "declaration_mismatch_identity_sha256": sha256(sys.argv[15]),
    "declaration_mismatch_protocol_sha256": sys.argv[16],
...
echo "[$PREFIX] submitted matched three-epoch toy cohort"
