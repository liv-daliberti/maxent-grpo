#!/usr/bin/env bash
# Launch the E24 canonical graph-coloring 7B four-GPU empty-shard fix cohort.
set -euo pipefail
...
STAMP=gce24_canonical_maxent_7b_v4_4xa100_evalsync_fix
...
# OAT counts these in learner updates. With rollout batch four, 48 prompts
# correspond to 12 updates. Keep sync/checkpoint aligned with evaluation.
export OAT_ZERO_SYNC_PARAMS_EVERY=12
export OAT_ZERO_SAVE_STEPS=12
export OAT_ZERO_SAVE_FROM=12
...
if [[ "$PHASE" == config ]]; then
  ...
  bash "$SUBMITTER"
  echo "[e24-v4-evalsync-fix] canonical graph-coloring-7B configuration passed; no jobs submitted"
  exit 0
fi
...
for job_id in "${job_ids[@]}"; do
  ...
  for required in \
    ...
    'OAT_ZERO_SYNC_PARAMS_EVERY=12' \
    'OAT_ZERO_SAVE_STEPS=12' \
    'OAT_ZERO_SAVE_FROM=12'; do
...
case "$release_mode" in
  held)
    echo "[e24-v4-evalsync-fix] audited cohort remains held for canary release: ${job_ids[*]}"
    ;;
  all)
    scontrol release "${job_ids[@]}"
    echo "[e24-v4-evalsync-fix] released complete canonical graph-coloring-7B cohort: ${job_ids[*]}"
    ;;
  *)
...
echo "[e24-v4-evalsync-fix] manifest=$MANIFEST"
