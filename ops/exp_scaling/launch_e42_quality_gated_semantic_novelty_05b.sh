#!/usr/bin/env bash
# Launch E42's treatment-only, quality-gated semantic novelty cohort.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e42_quality_gated_semantic_novelty_05b.md"
...
IDENTITY="$ROOT_DIR/var/artifacts/e42_quality_gated_semantic_novelty_05b_v1_identity.json"
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing frozen E42 prerequisite: $required" >&2
    exit 1
  fi
done
if ! grep -q '^\*\*Status: FROZEN BEFORE LAUNCH' "$PROTOCOL"; then
  echo "E42 protocol is not frozen" >&2
  exit 1
fi
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "E42 MATH materialization is incomplete: $required" >&2
    exit 1
  fi
done
...
if ! grep -q 'semantic_shannon_separate_advantage' \
   ...
   ! grep -q 'semantic_shannon_quality_gated_advantage' \
     "$ROOT_DIR/src/oat_drgrpo/learner/grpo.py" || \
   ! grep -q 'OAT_ZERO_SEMANTIC_SHANNON_QUALITY_GATED_ADVANTAGE' \
     "$ROOT_DIR/ops/train.sh" || \
   ! grep -q 'submit_arm quality_gated_semantic_novelty quality_gated_semantic_novelty' \
  ...
  echo "Current source/execution stack lacks the frozen E42 quality-gated semantic-novelty mechanism" >&2
  exit 1
fi
...
if (
    ...
    raise SystemExit("E37 identity does not match the frozen E42 comparator")
...
if (
    ...
    raise SystemExit("E38 identity does not match the frozen E42 comparator")
...
if (
    ...
    raise SystemExit("E39 identity does not match the frozen E42 MATH comparator")
...
if data_hash != e39.get("data", {}).get("materialization_manifest_sha256"):
    raise SystemExit("E42 MATH materialization identity differs from frozen E39")
...
if (
    ...
    raise SystemExit("E41 identity does not match the frozen E42 parent treatment")
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX" "$MATH_PREFIX"; do
  ...
  if [[ "$phase" == full && -e "$manifest" ]]; then
    echo "Fresh E42 prefix required; manifest already exists: $manifest" >&2
    exit 1
  fi
done
if [[ "$phase" == full && -e "$IDENTITY" ]]; then
  echo "Fresh E42 identity required; artifact already exists: $IDENTITY" >&2
  exit 1
fi
...
freeze_execution() {
  ...
  SOURCE_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e42_quality_gated_semantic_novelty_05b_${SOURCE_HASH}/src"
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E42 source snapshot hash mismatch" >&2
    exit 1
  fi

  local ops_staging ops_input
  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e42-ops-input.XXXXXX")"
  ...
  OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e42_quality_gated_semantic_novelty_05b_ops_${EXECUTION_HASH}/ops"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E42 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
export OAT_ZERO_ONLY_ARMS=quality_gated_semantic_novelty
...
export OAT_ZERO_INCLUDE_QUALITY_GATED_SEMANTIC_NOVELTY_ARM=1
...
MODEBENCH_NODELIST="${OAT_ZERO_E42_MODEBENCH_TRAIN_NODELIST:-node105,node202,node203,node204}"
MODEBENCH_GRES="${OAT_ZERO_E42_MODEBENCH_TRAIN_GRES:-gpu:a5000:1}"
MODEBENCH_CPUS="${OAT_ZERO_E42_MODEBENCH_TRAIN_CPUS_PER_TASK:-4}"
MODEBENCH_MEMORY="${OAT_ZERO_E42_MODEBENCH_TRAIN_MEMORY:-32G}"
MATH_NODELIST="${OAT_ZERO_E42_MATH_TRAIN_NODELIST:-node105,node202,node203,node204}"
MATH_GRES="${OAT_ZERO_E42_MATH_TRAIN_GRES:-gpu:a5000:1}"
MATH_CPUS="${OAT_ZERO_E42_MATH_TRAIN_CPUS_PER_TASK:-8}"
MATH_MEMORY="${OAT_ZERO_E42_MATH_TRAIN_MEMORY:-64G}"
TRAIN_PARTITION="${OAT_ZERO_E42_TRAIN_PARTITION:-all}"
TRAIN_ACCOUNT="${OAT_ZERO_E42_TRAIN_ACCOUNT:-mltheory}"
TRAIN_TIME_LIMIT="${OAT_ZERO_E42_TRAIN_TIME_LIMIT:-24:00:00}"
...
write_identity() {
  ...
  local e41_countdown_manifest_hash e41_math_manifest_hash math_data_hash
  ...
  "$PYTHON_BIN" - "$IDENTITY" "$protocol_hash" "$launcher_hash" \
    ...
    "$e41_countdown_manifest_hash" "$e41_math_manifest_hash" <<'PY'
import json
import os
import pathlib
import sys
...
payload = {
    "schema": "e42_quality_gated_semantic_novelty_05b_v1",
    ...
    "tasks": {
        "graph_coloring": {
            "prompt_pool": 192,
            "prefix": "gce42_quality_gated_semantic_novelty_05b_v1",
        },
        "countdown": {
            "prompt_pool": 384,
            "prefix": "cde42_quality_gated_semantic_novelty_05b_v1",
        },
        "math12k_384": {
            "prompt_pool": 384,
            "eval_rows": 500,
            "prefix": "mte42_math12k_384_quality_gated_semantic_novelty_05b_v1",
            "materialization_manifest_sha256": sys.argv[14],
        },
    },
    "new_arms": ["quality_gated_semantic_novelty"],
    "quality_gated_semantic_novelty": {
        ...
        "effective_transform": (
            "eligible*min(max(raw_advantage,0),0.05)"
        ),
        "application": "centered_task_advantage_plus_capped_positive_novelty",
...
if [[ "$phase" == config ]]; then
  ...
  submit_task math
  echo "[e42] all three treatment-only configurations passed; no jobs submitted"
  exit 0
fi
...
cleanup_partial_cohort() {
  ...
  if [[ "$COHORT_RELEASED" != "1" ]]; then
    ...
    if [[ "${#cleanup_ids[@]}" -gt 0 ]]; then
      ...
      echo "[e42] cancelled incomplete held cohort: ${cleanup_ids[*]}" >&2
    fi
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX" "$MATH_PREFIX"; do
  ...
  if [[ "${#task_jobs[@]}" -ne "$EXPECTED_JOBS_PER_TASK" ]]; then
    echo "E42 ${prefix} has ${#task_jobs[@]} jobs; expected $EXPECTED_JOBS_PER_TASK" >&2
    exit 1
  fi
  job_ids+=("${task_jobs[@]}")
done
if [[ "${#job_ids[@]}" -ne "$EXPECTED_JOBS" ]]; then
  echo "E42 cohort has ${#job_ids[@]} jobs; expected $EXPECTED_JOBS" >&2
  exit 1
fi
...
for manifest_name in sys.argv[1:]:
    ...
    if len(rows) != 3 or observed != expected:
        raise SystemExit(
            f"E42 held manifest mismatch: {manifest_name} "
...
for job_id in "${job_ids[@]}"; do
  if ! scontrol update JobId="$job_id" Partition="$TRAIN_PARTITION"; then
    echo "E42 could not normalize held job $job_id to $TRAIN_PARTITION" >&2
    exit 1
  fi
done
...
for spec in \
  ...
  for job_id in "${task_jobs[@]}"; do
    ...
    for required in \
      ...
      "RUN_STAMP=${run_stamp}" \
      "OAT_ZERO_SEED=${seed}" \
      'OAT_ZERO_VARIANT=quality_gated_semantic_novelty' \
      ...
      'OAT_ZERO_SEMANTIC_SHANNON_QUALITY_GATED_ADVANTAGE=1' \
      ...
      if [[ "$job_record" != *"$required"* ]]; then
        echo "E42 held-job audit failed for $job_id: missing $required" >&2
        exit 1
      fi
    done
    if [[ "$arm" != quality_gated_semantic_novelty ]]; then
      echo "E42 manifest has unexpected arm for $job_id: $arm" >&2
      exit 1
    fi
    if [[ "$job_record" != *"Partition=${TRAIN_PARTITION}"* ]] || \
      ...
      echo "E42 held job $job_id does not attest the frozen placement" >&2
      exit 1
    fi
  done
done
...
COHORT_RELEASED=1
trap - EXIT
echo "[e42] released ${#job_ids[@]} quality-gated semantic novelty jobs: ${job_ids[*]}"
echo "[e42] identity=$IDENTITY math_data=$MATH_DATA_IDENTITY"
