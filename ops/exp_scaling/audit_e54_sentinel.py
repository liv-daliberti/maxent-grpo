#!/usr/bin/env python3
"""Fail-closed audit for E54 uniform verified-likelihood sentinels."""
...
SPEC = importlib.util.spec_from_file_location("e54_bound_e53_audit", BASE_PATH)
...
DOMAINS = {
    "countdown": {
        ...
        "treatment_prefix": "cde54_verified_likelihood_05b_50ep_sentinel_allcs",
    },
    "graph_coloring": {
        "pool": 192,
        "control_prefix": "gce53_verified_replay_05b_50ep_sentinel",
        "treatment_prefix": "gce54_verified_likelihood_05b_50ep_sentinel",
    },
    "python_factor": {
        "pool": 384,
        "control_prefix": "pye53_verified_replay_05b_50ep_sentinel_allcs",
        "treatment_prefix": "pye54_verified_likelihood_05b_50ep_sentinel_allcs",
    },
}
IDENTITY_PATH = (
    ROOT / "var/artifacts/e54_uniform_verified_likelihood_identity.json"
)
E53_IDENTITY_PATH = (
    ROOT / "var/artifacts/e53_verified_replay_05b_sentinel_identity.json"
)
PROTOCOL_PATH = (
    ROOT / "paper/preregistration/e54_uniform_verified_likelihood_05b.md"
)
LAUNCHER_PATH = (
    ROOT
    / "ops/exp_scaling/launch_e54_uniform_verified_likelihood_05b.sh"
)
APPROVAL_PATH = ROOT / "var/artifacts/e54_sentinel_stage_a_approval.json"
...
def approval_binding() -> tuple[dict[str, Any], list[str]]:
    ...
    try:
        ...
        return {}, [f"E54 identity is unavailable or invalid: {error}"]
    if (
        not isinstance(identity, dict)
        or identity.get("schema")
        != "e54_uniform_verified_likelihood_05b_sentinel_v1"
    ):
        return {}, ["E54 identity has an incompatible schema"]
    ...
    source_root = (
        ...
        / f"e54_verified_likelihood_{source_hash}"
        / "src"
    )
    ops_root = (
        ROOT
        / "var/artifacts/source_snapshots"
        / f"e54_verified_likelihood_ops_{execution_hash}"
        / "ops"
    )
    try:
        observed = {
        ...
        return {}, [f"E54 approval evidence is unavailable: {error}"]
    for key in (
        "protocol_sha256",
        "launcher_sha256",
        "auditor_sha256",
...
def _check_e54_replay(
    ...
    *,
    ...
    if missing:
        return (
            reference,
            violations + [f"step {step}: missing/nonfinite E54 replay {missing}"],
            {},
            True,
        )
    ...
    observed = value["train/canonical_replay_observed_normalized_entropy"]
    ema = value["train/canonical_replay_entropy_ema"]
    multiplier = value["train/canonical_replay_inverse_multiplier"]
    alpha_before = value["train/canonical_replay_alpha_before"]
    ...
    if (
        value["train/canonical_replay_eligible_groups"] != groups
        or value["train/canonical_replay_retained_modes"] != modes
    ):
    ...
    if not math.isclose(
        weighted,
        actuator * alpha_used * ESTIMATOR_SCALE,
        rel_tol=2e-5,
        abs_tol=1e-8,
    ):
    ...
    if value["train/canonical_replay_projection_active"] != 0 or skipped != 0:
...
BASE._check_replay_controller = _check_e54_replay
...
def audit(data_root: Path) -> dict[str, Any]:
    ...
    for domain, config in DOMAINS.items():
        ...
        domains[domain] = {
            "runs": {"e53_grpo": control, "e54": treatment},
...
def main() -> int:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=ROOT / "var/artifacts/e54_sentinel_audit_latest.json",
    )
