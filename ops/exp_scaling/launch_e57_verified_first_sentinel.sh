#!/usr/bin/env bash
# Configure or atomically submit E57's smoke-bound three-domain sentinel.
set -euo pipefail
...
AUDITOR="$ROOT_DIR/ops/exp_scaling/audit_e57_sentinel.py"
HELPER_AUDITOR="$ROOT_DIR/ops/exp_scaling/audit_e56_sentinel.py"
PYTHON_SMOKE_IDENTITY="$ROOT_DIR/var/artifacts/e57_verified_first_split_python_smoke_identity.json"
GRAPH_SMOKE_IDENTITY="$ROOT_DIR/var/artifacts/e57_verified_first_split_graph_smoke_identity.json"
PYTHON_SMOKE_AUDIT="$ROOT_DIR/var/artifacts/e57_python_smoke_audit_latest.json"
GRAPH_SMOKE_AUDIT="$ROOT_DIR/var/artifacts/e57_graph_smoke_audit_latest.json"
E53_IDENTITY="$ROOT_DIR/var/artifacts/e53_verified_replay_05b_sentinel_identity.json"
IDENTITY="$ROOT_DIR/var/artifacts/e57_verified_first_split_canonical_05b_sentinel_identity.json"
AUDIT_OUT="$ROOT_DIR/var/artifacts/e57_sentinel_audit_latest.json"
...
GRAPH_PREFIX=gce57_verified_first_split_canonical_05b_50ep_sentinel
COUNTDOWN_PREFIX=cde57_verified_first_split_canonical_05b_50ep_sentinel_allcs
PYTHON_PREFIX=pye57_verified_first_split_canonical_05b_50ep_sentinel_allcs
...
for required in \
  "$PROTOCOL" "$AUDITOR" "$HELPER_AUDITOR" \
  "$PYTHON_SMOKE_IDENTITY" "$GRAPH_SMOKE_IDENTITY" \
  "$PYTHON_SMOKE_AUDIT" "$GRAPH_SMOKE_AUDIT" \
  "$E53_IDENTITY" "$E53_GRAPH_MANIFEST" \
  "$E53_COUNTDOWN_MANIFEST" "$E53_PYTHON_MANIFEST" \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing E57 sentinel prerequisite: $required" >&2
    exit 1
  fi
done
...
if [[ "$phase" == sentinel ]]; then
  for fresh in \
    ...
    if [[ -e "$fresh" ]]; then
      echo "Fresh E57 sentinel artifact required; already exists: $fresh" >&2
      exit 1
    fi
  done
fi
...
mapfile -t frozen < <(
  "$PYTHON_BIN" - \
    "$PYTHON_SMOKE_IDENTITY" "$GRAPH_SMOKE_IDENTITY" \
    "$PYTHON_SMOKE_AUDIT" "$GRAPH_SMOKE_AUDIT" <<'PY'
import json
import pathlib
import sys

python_identity = json.loads(pathlib.Path(sys.argv[1]).read_text())
graph_identity = json.loads(pathlib.Path(sys.argv[2]).read_text())
python_audit = json.loads(pathlib.Path(sys.argv[3]).read_text())
graph_audit = json.loads(pathlib.Path(sys.argv[4]).read_text())
if python_audit.get("status") != "pass" or python_audit.get("violations"):
    raise SystemExit("E57 sentinel requires a clean terminal Python smoke")
if graph_audit.get("status") != "pass" or graph_audit.get("violations"):
    raise SystemExit("E57 sentinel requires a clean terminal Graph smoke")
for key in ("source_hash", "execution_surface_hash"):
    if not python_identity.get(key) or (
        python_identity.get(key) != graph_identity.get(key)
    ):
        raise SystemExit(f"E57 smoke identities disagree on {key}")
print(python_identity["source_hash"])
print(python_identity["execution_surface_hash"])
PY
)
if [[ "${#frozen[@]}" -ne 2 ]]; then
  echo "Could not resolve E57 smoke-bound source and execution hashes" >&2
  exit 1
fi
...
if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
  echo "E57 smoke-bound source snapshot hash mismatch" >&2
  exit 1
fi
if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
  echo "E57 smoke-bound execution snapshot hash mismatch" >&2
  exit 1
fi
...
write_identity() {
  ...
  "$PYTHON_BIN" - \
    ...
    "$E53_IDENTITY" "$PYTHON_SMOKE_IDENTITY" "$GRAPH_SMOKE_IDENTITY" \
    "$PYTHON_SMOKE_AUDIT" "$GRAPH_SMOKE_AUDIT" \
...
payload = {
    "schema": "e57_verified_first_split_canonical_05b_sentinel_v1",
    ...
    "python_smoke_identity_sha256": digest(sys.argv[7]),
    "graph_smoke_identity_sha256": digest(sys.argv[8]),
    "python_smoke_audit_sha256": digest(sys.argv[9]),
    "graph_smoke_audit_sha256": digest(sys.argv[10]),
    "source_hash": sys.argv[11],
    "execution_surface_hash": sys.argv[12],
    "jobs": {
        "graph_coloring": int(sys.argv[13]),
        "countdown": int(sys.argv[14]),
        "python_factor": int(sys.argv[15]),
    },
    "job_manifest_sha256": {
        "graph_coloring": digest(sys.argv[16]),
        "countdown": digest(sys.argv[17]),
        "python_factor": digest(sys.argv[18]),
    },
    "e53_control_manifest_sha256": {
        "graph_coloring": digest(sys.argv[19]),
        "countdown": digest(sys.argv[20]),
        "python_factor": digest(sys.argv[21]),
    },
...
submit_domain() {
  ...
  case "$domain" in
    graph_coloring)
      ...
      export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E57_SENTINEL_GRAPH_NODELIST:-node103,node104,node208}"
      export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E57_SENTINEL_GRAPH_GRES:-gpu:a6000:1}"
      export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E57_SENTINEL_GRAPH_PARTITION:-lowprio}"
      export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E57_SENTINEL_GRAPH_ACCOUNT:-mltheory}"
      ;;
    countdown)
      ...
      export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E57_SENTINEL_COUNTDOWN_NODELIST:-node020,node022,node023,node024,node026}"
      export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E57_SENTINEL_COUNTDOWN_GRES:-gpu:rtx_3090:1}"
      export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E57_SENTINEL_COUNTDOWN_PARTITION:-lowprio}"
      export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E57_SENTINEL_COUNTDOWN_ACCOUNT:-allcs}"
      ;;
    python_factor)
      ...
      export OAT_ZERO_RESUME_STEPS=384
      export OAT_ZERO_RESUME_FROM=384
      export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E57_SENTINEL_PYTHON_NODELIST:-node020,node022,node023,node024,node026}"
      export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E57_SENTINEL_PYTHON_GRES:-gpu:rtx_3090:1}"
      export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E57_SENTINEL_PYTHON_PARTITION:-lowprio}"
      export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E57_SENTINEL_PYTHON_ACCOUNT:-allcs}"
      ;;
    *)
      echo "Unknown E57 sentinel domain: $domain" >&2
      exit 1
      ;;
  esac
...
if [[ "$phase" == config ]]; then
  ...
  echo "[e57] all three smoke-bound sentinel configurations passed"
  exit 0
fi
...
cleanup_held() {
  ...
  if [[ "$released" != "1" && "${#job_ids[@]}" -gt 0 ]]; then
    ...
    echo "[e57] cancelled incomplete held sentinel: ${job_ids[*]}" >&2
  fi
  exit "$status"
}
...
for manifest in "$GRAPH_MANIFEST" "$COUNTDOWN_MANIFEST" "$PYTHON_MANIFEST"; do
  ...
  if [[ "${#manifest_jobs[@]}" -ne 1 ]]; then
    echo "E57 manifest $manifest has ${#manifest_jobs[@]} jobs; expected one" >&2
    exit 1
  fi
...
for job_id in "${job_ids[@]}"; do
  ...
  for required in \
    ...
    'OAT_ZERO_SEED=9010' \
    'OAT_ZERO_VARIANT=verified_first_split_canonical' \
    ...
    if [[ "$job_record" != *"$required"* ]]; then
      echo "E57 held-job audit failed for $job_id: missing $required" >&2
      exit 1
    fi
  done
done
...
if payload.get("status") != "in_progress" or payload.get("violations"):
    raise SystemExit("E57 held sentinel failed its exact-job pre-release audit")
PY

scontrol release "${job_ids[@]}"
released=1
trap - EXIT
echo "[e57] released held sentinel cohort: ${job_ids[*]}"
