#!/usr/bin/env python3
"""Freeze and schedule the PointMaze balanced-v5 final after its gate."""
...
DEPENDENCY_BATCH = (
    ROOT
    / "ops/slurm/launch_point_maze_balanced_v5_stage_b_after_gate.slurm"
)
LAUNCHER = (
    ROOT / "ops/exp_scaling/launch_point_maze_balanced_v5_stage_b.py"
)
MANIFEST = (
    ROOT
    / "var/artifacts/"
    "point_maze_balanced_v5_stage_b_dependent_launcher_identity.json"
)
FILES = (
    ROOT
    ...
    ROOT / "ops/audit_point_maze_stage_b_05b_12pass.py",
    ROOT
...
def schedule() -> None:
    if MANIFEST.exists():
        raise FileExistsError(
            f"fresh PointMaze v5 dependent identity required: {MANIFEST}"
        )
    hashes = file_hashes()
    output = run(
        [
    ...
    try:
        ...
        for required in (
            ...
            if required not in record:
                raise RuntimeError(
                    f"held PointMaze v5 launcher lacks {required}"
                )
        atomic(
            MANIFEST,
            {
                "schema_version": (
                    "point-maze-balanced-v5-dependent-launcher-v1"
                ),
        ...
        raise
    print(f"[point-balanced-v5] scheduled final launcher {job_id}")
...
def verify_run(manifest: Path) -> None:
    ...
    if (
        ...
        raise RuntimeError("PointMaze balanced-v5 frozen launcher drift")
    subprocess.run(
