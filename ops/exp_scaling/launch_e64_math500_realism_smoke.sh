#!/usr/bin/env bash
# Configure or submit E64's verified-answer MATH realism smoke.
set -euo pipefail
...
AUDITOR="$ROOT_DIR/ops/exp_scaling/audit_e64_math500_realism_smoke.py"
PREFIX=mte64_math500_realism_smoke_e58_s43
MANIFEST="$ROOT_DIR/var/artifacts/${PREFIX}_comparative_jobs.tsv"
IDENTITY="$ROOT_DIR/var/artifacts/e64_math500_realism_smoke_identity.json"
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing E64 smoke prerequisite: $required" >&2
    exit 1
  fi
done
...
if output.get("train_rows") != 384 or output.get("eval_rows") != 500:
    raise SystemExit("E64 data row counts differ from the frozen protocol")
if (
    output.get("train_ordered_row_sha256")
...
if [[ "$phase" == full ]]; then
  for fresh in "$MANIFEST" "$IDENTITY"; do
    if [[ -e "$fresh" ]]; then
      echo "Fresh E64 smoke artifact required; already exists: $fresh" >&2
      exit 1
    fi
  done
fi
...
freeze_execution() {
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E64 source snapshot hash mismatch" >&2
    exit 1
  fi

  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e64-ops-input.XXXXXX")"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E64 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
write_identity() {
  local job_id="$1"
  "$PYTHON_BIN" - \
    "$IDENTITY" "$PROTOCOL" "$0" "$AUDITOR" "$DATA_IDENTITY" \
    "$SOURCE_HASH" "$EXECUTION_HASH" "$MANIFEST" "$job_id" <<'PY'
import hashlib
import json
import os
import pathlib
...
payload = {
    ...
    "domain": "MATH12K-96 train / held-out MATH-500 diagnostic",
...
export OAT_ZERO_EVAL_STEPS=1000000
...
if [[ "$phase" == config ]]; then
  ...
  echo "[e64] verified-answer MATH smoke configuration passed"
  exit 0
fi
...
cleanup_held() {
  ...
  if [[ "$released" != "1" && "$job_id" =~ ^[0-9]+$ ]]; then
    ...
    echo "[e64] cancelled incomplete held smoke $job_id" >&2
  fi
  exit "$status"
}
...
if [[ "${#job_ids[@]}" -ne 1 ]]; then
  echo "E64 MATH smoke has ${#job_ids[@]} jobs; expected one" >&2
  exit 1
fi
job_id="${job_ids[0]}"
...
released=1
trap - EXIT
echo "[e64] released verified-answer MATH smoke job $job_id"
