#!/usr/bin/env bash
# Restore only pending, unmaterialized E68 jobs to earlier actual estimates.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
AMENDMENT="$ROOT_DIR/paper/preregistration/e68_zero_step_restore_pvl_amendment_20260727.md"
IDENTITY="$ROOT_DIR/var/artifacts/e68_separated_support_actuator_ablation_identity.json"
OUT="$ROOT_DIR/var/artifacts/e68_zero_step_restore_pvl_amendment.json"
...
GRAPH_ORIGINAL=node103,node104,node205,node206,node207,node805
...
[[ ! -e "$OUT" ]] || {
  echo "Fresh E68 restore-pvl amendment artifact required: $OUT" >&2
  exit 1
}
...
for job_id in "${RTX_IDS[@]}"; do
  scontrol update JobId="$job_id" Partition=pvl-lowprio NodeList="$RTX_NODES"
done
...
out = Path(sys.argv[1])
payload = {
    "schema": "e68_zero_step_restore_pvl_amendment_v1",
...
released=1
trap - EXIT
echo "[e68-restore-pvl] released amended zero-step jobs: ${ALL_IDS[*]}"
