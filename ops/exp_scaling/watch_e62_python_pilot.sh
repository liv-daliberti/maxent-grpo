#!/usr/bin/env bash
# Refresh E62-R9's Python pilot audit, curve JSON, and expanding live graph.
set -u

ROOT_DIR="${OAT_ZERO_REPO_ROOT:-$PWD}"
if [[ ! -f "$ROOT_DIR/ops/exp_scaling/audit_e62_python_pilot.py" ]]; then
...
AUDIT="$ROOT_DIR/var/artifacts/e62r9_python_pilot_audit_latest.json"
...
while true; do
  date
  "$PYTHON_BIN" ops/exp_scaling/audit_e62_python_pilot.py || true
  "$PYTHON_BIN" ops/exp_scaling/parse_scaling_curve.py \
    --stamp-prefix pye62r9_open_set_sweep_05b_1ep_pilot || true
  "$PYTHON_BIN" ops/exp_scaling/plot_e62_python_pilot.py || true
