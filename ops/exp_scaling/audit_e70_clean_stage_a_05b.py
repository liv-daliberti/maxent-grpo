#!/usr/bin/env python3
"""Fail-closed filesystem audit for E70 Stage A.s exact 40-job cohort."""
...
PROTOCOL = ROOT / "paper/preregistration/e70_clean_verified_maxent_vs_compute_matched_drgrpo_stage_a_05b.md"
LAUNCHER = ROOT / "ops/exp_scaling/launch_e70_clean_stage_a_05b.sh"
OUT = ROOT / "var/artifacts/e70_clean_stage_a_05b_audit_latest.json"
CONTROL = "grpo"
...
def _mechanism_violations(
    ...
    required = {**common, **arm_specific.get(arm, {})}
...
def main() -> None:
    ...
    if identity.get("schema") != "e70_clean_stage_a_05b_v1":
    ...
    payload = {
        "schema": "e70_clean_stage_a_05b_audit_v1",
        "status": status,
        "summary": {
            "expected_runs": 40,
    ...
    print(
        f"[e70-stage-a-audit] status={status} "
        f"materialized={materialized_runs}/40 metrics={metric_runs}/40 "
        f"terminal={terminal_runs}/40 violations={len(payload['violations'])} "
        f"out={OUT}"
    )
