#!/usr/bin/env bash
# Launch the clean matched E33 3B free-form Dr.GRPO/EMA-MaxEnt cohort.
set -euo pipefail
...
case "$phase" in
  config|full) ;;
  *) echo "Usage: $0 {config|full}" >&2; exit 1 ;;
esac

PYTHON_BIN="${OAT_ZERO_PYTHON:-$ROOT_DIR/var/seed_paper_eval/paper310/bin/python}"
PROTOCOL="$ROOT_DIR/paper/preregistration/e33_modebench_freeform_3b_ema_10ep.md"
MODEL_ROOT="$ROOT_DIR/var/cache/huggingface/transformers/models--Qwen--Qwen2.5-3B-Instruct/snapshots/aa8e72537993ba99e69dfaafa59ed015b17504d1"
...
GRAPH_PREFIX=gce33_freeform_3b_ema_10ep_v3_a100
COUNTDOWN_PREFIX=cde33_freeform_3b_ema_10ep_v3_a100
IDENTITY="$ROOT_DIR/var/artifacts/e33_freeform_3b_ema_10ep_v3_a100_identity.json"
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing frozen E33 prerequisite: $required" >&2
    exit 1
  fi
done
if ! grep -q '^\*\*Status: FROZEN' "$PROTOCOL"; then
  echo "E33 protocol is not frozen" >&2
  exit 1
fi
for manifest in \
  ...
  if [[ "$phase" == full && -e "$manifest" ]]; then
    echo "Fresh E33 prefix required; manifest already exists: $manifest" >&2
    exit 1
  fi
done
...
freeze_execution() {
  ...
  SOURCE_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e33_freeform_3b_${SOURCE_HASH}/src"
  ...
  [[ "$(hash_tree "$SOURCE_ROOT")" == "$SOURCE_HASH" ]] || {
    echo "E33 source snapshot hash mismatch" >&2; exit 1;
  }

  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e33-ops-input.XXXXXX")"
  ...
  OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e33_freeform_3b_ops_${EXECUTION_HASH}/ops"
  ...
  [[ "$(hash_tree "$OPS_ROOT")" == "$EXECUTION_HASH" ]] || {
    echo "E33 execution snapshot hash mismatch" >&2; exit 1;
  }
}
...
export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E33_TRAIN_NODELIST:-node302}"
export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E33_TRAIN_GRES:-gpu:a100:1}"
export OAT_ZERO_TRAIN_CPUS_PER_TASK="${OAT_ZERO_E33_TRAIN_CPUS_PER_TASK:-16}"
export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E33_TRAIN_PARTITION:-mltheory}"
export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E33_TRAIN_ACCOUNT:-mltheory}"
export OAT_ZERO_TRAIN_MEMORY="${OAT_ZERO_E33_TRAIN_MEMORY:-96G}"
export OAT_ZERO_TRAIN_TIME_LIMIT="${OAT_ZERO_E33_TRAIN_TIME_LIMIT:-168:00:00}"

write_identity() {
  local protocol_hash launcher_hash
  protocol_hash="$(sha256sum "$PROTOCOL" | cut -d' ' -f1)"
  launcher_hash="$(sha256sum "$0" | cut -d' ' -f1)"
  "$PYTHON_BIN" - "$IDENTITY" "$protocol_hash" "$launcher_hash" "$SOURCE_HASH" "$EXECUTION_HASH" <<'PY'
import json, os, pathlib, sys, tempfile
path = pathlib.Path(sys.argv[1])
payload = {
    "schema": "e33_freeform_3b_ema_10ep_v3_a100",
    "protocol_sha256": sys.argv[2], "launcher_sha256": sys.argv[3],
    "source_hash": sys.argv[4], "execution_surface_hash": sys.argv[5],
    "model": "Qwen2.5-3B-Instruct@aa8e72537993ba99e69dfaafa59ed015b17504d1",
    "tasks": {"graph_coloring": {"prompt_pool": 192, "target_entropy": 1.622718550885717},
              "countdown": {"prompt_pool": 384, "target_entropy": 1.347109432487438}},
    "arms": ["grpo", "maxent_dual"], "seeds": [43, 44, 45], "prompt_epochs": 10,
    ...
    "placement": {"nodes": ["node302"], "gpu": "a100:1", "cpus": 16, "memory_gib": 96},
    "supersedes": "e33_freeform_3b_ema_10ep_v1_failed_before_step_0",
}
...
submit_task() {
  ...
  if [[ "$task" == graph_coloring ]]; then
    ...
    export OAT_ZERO_SAVE_STEPS=192 OAT_ZERO_SAVE_FROM=192
    export OAT_ZERO_RESUME_STEPS=192 OAT_ZERO_RESUME_FROM=192
  else
    ...
    export OAT_ZERO_SAVE_STEPS=384 OAT_ZERO_SAVE_FROM=384
    export OAT_ZERO_RESUME_STEPS=384 OAT_ZERO_RESUME_FROM=384
  fi
  "$OPS_ROOT/submit_countdown_comparative.sh"
}

if [[ "$phase" == config ]]; then
  export OAT_ZERO_COMPARATIVE_CONFIG_ONLY=1 OAT_ZERO_SBATCH_HOLD=0
  submit_task graph_coloring
  submit_task countdown
  echo "[e33] both matched 3B configurations passed; no jobs submitted"
  exit 0
fi
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX"; do
  ...
  if [[ "${#task_jobs[@]}" -ne "$EXPECTED_JOBS_PER_TASK" ]]; then
    mapfile -t submitted_jobs < <(awk -F '\t' 'FNR > 1 && $3 ~ /^[0-9]+$/ {print $3}' "$ROOT_DIR/var/artifacts/${GRAPH_PREFIX}_comparative_jobs.tsv" "$ROOT_DIR/var/artifacts/${COUNTDOWN_PREFIX}_comparative_jobs.tsv")
    scancel "${submitted_jobs[@]}" 2>/dev/null || true
    echo "E33 ${prefix} has ${#task_jobs[@]} jobs; expected $EXPECTED_JOBS_PER_TASK" >&2
    exit 1
  fi
  job_ids+=("${task_jobs[@]}")
done
...
for job_id in "${job_ids[@]}"; do
  if ! scontrol update JobId="$job_id" Partition="$OAT_ZERO_TRAIN_PARTITION"; then
    ...
    echo "E33 could not normalize held job $job_id to ${OAT_ZERO_TRAIN_PARTITION}" >&2
    exit 1
  fi
done
...
for spec in "$GRAPH_PREFIX|$GRAPH_TARGET|192|48" "$COUNTDOWN_PREFIX|$COUNTDOWN_TARGET|384|96"; do
  ...
  for job_id in "${task_jobs[@]}"; do
    ...
    for required in 'JobState=PENDING' 'Reason=JobHeldUser' \
      'OAT_ZERO_MODEL=qwen2.5-3b-instruct' 'OAT_ZERO_MAX_PROMPT_EPOCHS=10' \
      'OAT_ZERO_NUM_PROMPT_EPOCH=10' \
      'OAT_ZERO_EVAL_MODE_COVERAGE_K=8' 'OAT_ZERO_EVAL_MODE_COVERAGE_DRAWS=4' \
      'OAT_ZERO_EVAL_MODE_COVERAGE_SEED=1001' "OAT_ZERO_EVAL_PROMPT_INTERVAL=${eval_interval}" \
      "OAT_ZERO_RESUME_STEPS=${resume_steps}" "OAT_ZERO_RESUME_FROM=${resume_steps}" \
      'OAT_ZERO_MAX_RESUME_NUM=2' 'OAT_ZERO_PRUNE_RESUME_ON_SUCCESS=0' \
      'OAT_ZERO_EXPORT_STEPS=0' 'OAT_ZERO_MAX_EXPORT_NUM=1' \
      'OAT_ZERO_ADAM_OFFLOAD=1' 'OAT_ZERO_ACTIVATION_OFFLOADING=1' \
      ...
      if [[ "$job_record" != *"$required"* ]]; then
        scancel "${job_ids[@]}" 2>/dev/null || true
        echo "E33 held-job audit failed for $job_id: missing $required" >&2
        exit 1
      fi
    done
    if [[ "$arm" == maxent_dual ]]; then
      for required in 'OAT_ZERO_VARIANT=maxent_dual' \
        "OAT_ZERO_MAXENT_DUAL_TARGET_ENTROPY=${target}" \
        'OAT_ZERO_MAXENT_DUAL_MIN_ALPHA=0.000075' 'OAT_ZERO_MAXENT_DUAL_MAX_ALPHA=0.00060' \
        'OAT_ZERO_MAXENT_DUAL_ALPHA_LR=0.010' 'OAT_ZERO_MAXENT_DUAL_EMA_DECAY=0.7'; do
        [[ "$job_record" == *"$required"* ]] || { scancel "${job_ids[@]}" 2>/dev/null || true; echo "E33 treatment audit failed for $job_id: missing $required" >&2; exit 1; }
      done
    elif [[ "$arm" == grpo ]]; then
      for required in 'OAT_ZERO_VARIANT=grpo' 'OAT_ZERO_MAXENT_DUAL_TARGET_ENTROPY=0.0' 'OAT_ZERO_MAXENT_ALPHA=0.0'; do
        [[ "$job_record" == *"$required"* ]] || { scancel "${job_ids[@]}" 2>/dev/null || true; echo "E33 control audit failed for $job_id: missing $required" >&2; exit 1; }
      done
    else
      scancel "${job_ids[@]}" 2>/dev/null || true
      echo "E33 manifest has unexpected arm for $job_id: $arm" >&2
      exit 1
    fi
  done
done
...
for job_id in "${job_ids[@]}"; do
  ...
  if [[ "$job_record" != *"Partition=${OAT_ZERO_TRAIN_PARTITION}"* ]] || \
     ...
     [[ "$job_record" != *"TresPerNode=gres/gpu:a100:1"* ]] || \
     [[ "$job_record" != *"NumCPUs=${OAT_ZERO_TRAIN_CPUS_PER_TASK}"* ]] || \
     [[ "$job_record" != *"MinMemoryNode=${OAT_ZERO_TRAIN_MEMORY}"* ]]; then
    scancel "${job_ids[@]}" 2>/dev/null || true
    echo "E33 held job $job_id does not attest the frozen placement" >&2
    exit 1
  fi
done

scontrol release "${job_ids[@]}"
echo "[e33] released ${#job_ids[@]} clean matched 3B jobs: ${job_ids[*]}"
