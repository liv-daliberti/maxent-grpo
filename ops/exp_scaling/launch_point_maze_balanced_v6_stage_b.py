#!/usr/bin/env python3
"""Configure or conditionally launch the PointMaze balanced-v6 ten-cell cohort."""
...
WARMSTART_SFT = (
    ROOT / "var/artifacts/point_maze_balanced_short_warmstart_sft_v6.json"
)
WARMSTART_VIABILITY = (
    ROOT / "var/artifacts/point_maze_balanced_short_warmstart_v6_viability.json"
)
...
stage.AUDITOR = ROOT / "ops/audit_point_maze_balanced_v6_stage_b_05b_12pass.py"
stage.TRAIN_BATCH = (
    ROOT
...
stage.MODEL = ROOT / "var/models/point_maze_interactive_warmstart_v6_balanced_short"
stage.DATA = ROOT / "var/data/point_maze_algorithm_repair_v3"
stage.IDENTITY = (
    ROOT
    / "var/artifacts/point_maze_balanced_v6_stage_b_05b_12pass_identity.json"
)
stage.MANIFEST = (
    ROOT / "var/artifacts/point_maze_balanced_v6_stage_b_05b_12pass_jobs.tsv"
)
stage.SUBMISSION = (
    ROOT
    / "var/artifacts/point_maze_balanced_v6_stage_b_05b_12pass_submission.json"
)
stage.AUDIT_OUTPUT = (
    ROOT / "var/artifacts/point_maze_balanced_v6_stage_b_05b_12pass_audit.json"
)
stage.AUDIT_RUNNER = (
    ROOT
    / "var/artifacts/"
    "point_maze_balanced_v6_stage_b_05b_12pass_audit_runner_identity.json"
)
stage.SEEDS = (76631, 76632, 76633, 76634, 76635)
stage.VARIANT = "balanced_v6"
stage.ARTIFACT_STEM = "point_maze_balanced_v6_stage_b_05b_12pass"
...
def base_prerequisites() -> None:
    ...
    if (
        ...
        or warmstart.get("sft_seed") != 76621
        or warmstart.get("development_seed") != 76622
        ...
        raise RuntimeError("PointMaze balanced-v6 warm-start identity drift")
...
def qualification_passes() -> None:
    for path in (
        ...
        if not path.exists():
            raise RuntimeError(f"PointMaze balanced-v6 prerequisite absent: {path}")
    payload = json.loads(QUALIFICATION.read_text(encoding="utf-8"))
    if (
        payload.get("schema_version")
        != "point-maze-balanced-short-warmstart-v6-qualification-v1"
        or payload.get("status") != "pass"
        or payload.get("decision")
        != "eligible_for_point_maze_v6_five_seed_pair"
        or payload.get("errors") not in ([], None)
    ):
        raise RuntimeError("PointMaze balanced-v6 qualification did not authorize final")
...
def validate() -> None:
    ...
    stage.run(
        [
            ...
            "pytest",
            "-q",
            str(ROOT / "tests/test_point_maze_balanced_v6_stage_b.py"),
...
def snapshot_tree(source: Path, _prefix: str):
    return _snapshot_tree(source, "point_balanced_v6_stage_b_source")
...
def snapshot_execution():
    ...
    temporary = Path(
        tempfile.mkdtemp(
            prefix=".point-balanced-v6-stage-b.",
    ...
    target = (
        ...
        f"point_balanced_v6_stage_b_ops_{digest}"
    )
    ...
    if stage.tree_hash(target) != digest:
        raise RuntimeError("PointMaze balanced-v6 execution snapshot mismatch")
...
def atomic(path: Path, payload) -> None:
    if path == stage.IDENTITY:
        payload.update(
            schema="point-maze-balanced-v6-stage-b-05b-12pass-identity-v1",
        ...
        payload.update(
            schema="point-maze-balanced-v6-stage-b-05b-12pass-submission-v1"
        )
