#!/usr/bin/env bash
# Launch E52's full-horizon, three-domain engineering sentinel.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e52_direct_inverse_entropy_canonical_05b.md"
...
GRAPH_PREFIX=gce52_direct_inverse_entropy_canonical_05b_50ep_sentinel_v2
COUNTDOWN_PREFIX=cde52_direct_inverse_entropy_canonical_05b_50ep_sentinel_v2_allcs
PYTHON_PREFIX=pye52_direct_inverse_entropy_canonical_05b_50ep_sentinel_v2_allcs
IDENTITY="$ROOT_DIR/var/artifacts/e52_direct_inverse_entropy_canonical_05b_sentinel_v2_identity.json"
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing frozen E52 prerequisite: $required" >&2
    exit 1
  fi
done
if ! grep -q '^\*\*Status: FROZEN BEFORE SENTINEL SUBMISSION' "$PROTOCOL"; then
  echo "E52 protocol is not frozen" >&2
  exit 1
fi
if ! grep -q '^\*\*Status: FROZEN BEFORE CORRECTED RELAUNCH' "$REPAIR_PROTOCOL"; then
  echo "E52 runtime-validation repair is not frozen" >&2
  exit 1
fi
for contract in \
  'src/oat_drgrpo/maxent_controllers.py:unprojected_warmup_inverse_direct_entropy_v1' \
  'src/oat_drgrpo/learner/grpo.py:maxent_conditional_token_entropy' \
  'src/oat_drgrpo/learner/base.py:_chunked_conditional_content_entropy_from_logits' \
  'src/oat_drgrpo/online_canonical_bank.py:entropy_alpha_override' \
  'src/oat_drgrpo/args.py:maxent_inverse_adaptation' \
  'ops/run_experiment.sh:maxent_inverse_canonical)' \
  'ops/submit_countdown_comparative.sh:INCLUDE_MAXENT_INVERSE_CANONICAL_ARM'; do
  path="${contract%%:*}"
  needle="${contract#*:}"
  if ! grep -q "$needle" "$ROOT_DIR/$path"; then
    echo "Current source lacks E52 contract: $path :: $needle" >&2
    exit 1
  fi
done
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX" "$PYTHON_PREFIX"; do
  ...
  if [[ "$phase" == sentinel && -e "$manifest" ]]; then
    echo "Fresh E52 prefix required; manifest exists: $manifest" >&2
    exit 1
  fi
done
if [[ "$phase" == sentinel && -e "$IDENTITY" ]]; then
  echo "Fresh E52 identity required; file exists: $IDENTITY" >&2
  exit 1
fi
...
freeze_execution() {
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E52 source snapshot hash mismatch" >&2
    exit 1
  fi

  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e52-ops-input.XXXXXX")"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E52 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
export OAT_ZERO_TRAIN_SEEDS=9009
...
write_identity() {
  local protocol_hash repair_protocol_hash launcher_hash
  protocol_hash="$(sha256sum "$PROTOCOL" | cut -d' ' -f1)"
  repair_protocol_hash="$(sha256sum "$REPAIR_PROTOCOL" | cut -d' ' -f1)"
  launcher_hash="$(sha256sum "$0" | cut -d' ' -f1)"
  "$PYTHON_BIN" - "$IDENTITY" "$protocol_hash" "$repair_protocol_hash" \
    "$launcher_hash" "$SOURCE_HASH" "$EXECUTION_HASH" <<'PY'
import json
import os
import pathlib
import sys
...
payload = {
    ...
    "repair_protocol_sha256": sys.argv[3],
    ...
    "direct_entropy_controller": {
        ...
        "units": "conditional_content_token_nats_mean_v1",
...
if [[ "$phase" == config ]]; then
  ...
  echo "[e52] all three sentinel configurations passed; no jobs submitted"
  exit 0
fi
...
cleanup_partial_cohort() {
  ...
  if [[ "$COHORT_RELEASED" != "1" && "${#job_ids[@]}" -gt 0 ]]; then
    ...
    echo "[e52] cancelled incomplete held sentinel: ${job_ids[*]}" >&2
  fi
  exit "$status"
}
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX" "$PYTHON_PREFIX"; do
  ...
  if [[ "${#task_jobs[@]}" -ne "$EXPECTED_JOBS_PER_DOMAIN" ]]; then
    echo "E52 ${prefix} has ${#task_jobs[@]} jobs; expected 3" >&2
    exit 1
  fi
  job_ids+=("${task_jobs[@]}")
...
if len(rows) != 3 or observed != expected:
    raise SystemExit(f"E52 held manifest mismatch: {observed!r}")
PY
done
if [[ "${#job_ids[@]}" -ne "$EXPECTED_JOBS" ]]; then
  echo "E52 sentinel has ${#job_ids[@]} jobs; expected $EXPECTED_JOBS" >&2
  exit 1
fi
...
for job_id in "${job_ids[@]}"; do
  ...
  for required in \
    ...
    if [[ "$job_record" != *"$required"* ]]; then
      echo "E52 held-job audit failed for $job_id: missing $required" >&2
      exit 1
    fi
  done
  if [[ "$job_record" == *"OAT_ZERO_VARIANT=grpo"* ]]; then
    for required in \
      ...
      if [[ "$job_record" != *"$required"* ]]; then
        echo "E52 Dr.GRPO audit failed for $job_id: missing $required" >&2
        exit 1
      fi
    done
  else
    for required in \
      ...
      if [[ "$job_record" != *"$required"* ]]; then
        echo "E52 inverse-arm audit failed for $job_id: missing $required" >&2
        exit 1
      fi
    done
    if [[ "$job_record" == *"OAT_ZERO_VARIANT=maxent_inverse_canonical"* ]]; then
      for required in \
        ...
        if [[ "$job_record" != *"$required"* ]]; then
          echo "E52 hybrid audit failed for $job_id: missing $required" >&2
          exit 1
        fi
      done
    else
      for required in \
        ...
        'OAT_ZERO_ONLINE_CANONICAL_NOVELTY_BETA=0.0'; do
        if [[ "$job_record" != *"$required"* ]]; then
          echo "E52 direct-only audit failed for $job_id: missing $required" >&2
          exit 1
        fi
      done
    fi
...
COHORT_RELEASED=1
trap - EXIT
echo "[e52] released ${#job_ids[@]} sentinel jobs: ${job_ids[*]}"
echo "[e52] identity=$IDENTITY"
