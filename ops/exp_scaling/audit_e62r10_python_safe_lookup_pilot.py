#!/usr/bin/env python3
"""Fail-closed live audit for E62-R10's same-seed Python mechanism pilot."""
...
IDENTITY = ROOT / "var/artifacts/e62r10_safe_lookup_python_pilot_identity.json"
OUT = ROOT / "var/artifacts/e62r10_python_pilot_audit_latest.json"
PROTOCOL = (
    ROOT
    / "paper/preregistration/e62r10_python_safe_lookup_counterfactual_05b.md"
)
LAUNCHER = ROOT / "ops/exp_scaling/launch_e62r10_python_safe_lookup_pilot.sh"
PLOTTER = ROOT / "ops/exp_scaling/plot_e62r10_python_safe_lookup_pilot.py"
WATCHER = ROOT / "ops/exp_scaling/watch_e62r10_python_safe_lookup_pilot.sh"
...
def _run_root(arm: str, job_id: int) -> Path | None:
    candidates = sorted(
        (ROOT / "var/data").glob(
...
def _audit_arm(
    *,
    ...
    if run_root is not None:
        ...
        if metrics_path.is_file():
            try:
                ...
                violations.append(f"{arm}: cannot inspect metrics: {error}")
    crash_hits = _crash_signatures(job_id)
    for hit in crash_hits:
        violations.append(f"{arm}: crash signature {hit}")
    ...
    if terminal and arm == TREATMENT:
        ...
        if post_initial_positive_excess < 2:
            violations.append(
                "treatment: fewer than two post-initial multiplicity points"
            )
...
def audit(identity_path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "schema": "e62_python_pilot_audit_v1",
            "status": "in_progress",
    ...
    if identity.get("schema") != "e62r10_safe_lookup_python_pilot_v1":
    ...
    source_root = (
        ROOT
        / f"var/artifacts/source_snapshots/e62r10_safe_lookup_{source_hash}/src"
    )
    ops_root = (
        ROOT
        / f"var/artifacts/source_snapshots/e62r10_safe_lookup_ops_{ops_hash}/ops"
    )
    try:
    ...
    if not isinstance(jobs, dict) or set(jobs) != {CONTROL, TREATMENT}:
    ...
    for arm in (CONTROL, TREATMENT):
        ...
        if len(exact) != 1:
            violations.append(f"{arm}: manifest exact-job binding mismatch")
    ...
    arm_results = {
        ...
        for arm in (CONTROL, TREATMENT)
        if int(jobs.get(arm, -1)) >= 0
    }
    all_terminal = (
        len(arm_results) == 2
        and all(bool(result["terminal"]) for result in arm_results.values())
    )
    if CONTROL in arm_results and TREATMENT in arm_results:
        control_seeds = arm_results[CONTROL]["neutral_request_seeds"]
        treatment_seeds = arm_results[TREATMENT]["neutral_request_seeds"]
        for step in sorted(set(control_seeds) & set(treatment_seeds), key=int):
            if control_seeds[step] != treatment_seeds[step]:
                violations.append(
                    f"matched neutral request seed differs at step {step}"
                )
    if all_terminal:
        ...
        if not control_evals or not treatment_evals:
            violations.append("terminal comparison lacks landed evaluations")
        else:
            control_final = control_evals[-1]
            treatment_final = treatment_evals[-1]
            if int(control_final["step"]) != int(treatment_final["step"]):
                violations.append(
                    "terminal comparison has unmatched evaluation steps"
                )
    ...
    latest_step = max(
        (int(result["latest_step"]) for result in arm_results.values()),
        default=-1,
    )
    return {
        "schema": "e62_python_pilot_audit_v1",
...
def main() -> None:
    ...
    print(
        f"[e62-python-audit] status={payload['status']} "
