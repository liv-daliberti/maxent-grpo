#!/usr/bin/env bash
# Configure or atomically submit E61's 24-job, four-domain matched cohort.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e61_e58_vs_grpo_05b_12pass.md"
...
IDENTITY="$ROOT_DIR/var/artifacts/e61_e58_vs_grpo_05b_12pass_identity.json"

GRAPH_PREFIX=gce61_e58_vs_grpo_05b_12ep
COUNTDOWN_PREFIX=cde61_e58_vs_grpo_05b_12ep
PYTHON_PREFIX=pye61_e58_vs_grpo_05b_12ep
MATHIR_PREFIX=mie61_e58_vs_grpo_05b_12ep
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing E61 prerequisite: $required" >&2
    exit 1
  fi
done
...
for raw in sys.argv[1:]:
    ...
    if payload.get("status") != "pass" or payload.get("violations"):
        raise SystemExit(f"E61 requires a clean smoke certificate: {path}")
PY
...
if [[ "$phase" == full ]]; then
  for fresh in \
    ...
    if [[ -e "$fresh" ]]; then
      echo "Fresh E61 artifact required; already exists: $fresh" >&2
      exit 1
    fi
  done
fi
...
freeze_execution() {
  ...
  SOURCE_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e61_e58_12ep_${SOURCE_HASH}/src"
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E61 source snapshot hash mismatch" >&2
    exit 1
  fi

  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e61-ops-input.XXXXXX")"
  ...
  OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e61_e58_12ep_ops_${EXECUTION_HASH}/ops"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E61 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
payload = {
    "schema": "e61_e58_vs_grpo_05b_12pass_v1",
...
submit_domain() {
  ...
  case "$domain" in
    ...
    mathir)
      ...
      ;;
    *)
      echo "Unknown E61 domain: $domain" >&2
      exit 1
      ;;
  esac
...
for manifest in \
  ...
  if [[ "${#manifest_jobs[@]}" -ne 6 ]]; then
    echo "E61 manifest $manifest has ${#manifest_jobs[@]} jobs; expected 6" >&2
    exit 1
  fi
  job_ids+=("${manifest_jobs[@]}")
done
if [[ "${#job_ids[@]}" -ne 24 ]]; then
  echo "E61 cohort has ${#job_ids[@]} jobs; expected 24" >&2
  exit 1
fi
...
for job_id in "${job_ids[@]}"; do
  ...
  for required in \
    ...
    if [[ "$job_record" != *"$required"* ]]; then
      echo "E61 held-job audit failed for $job_id: missing $required" >&2
      exit 1
    fi
  done
  if [[ "$job_record" == *'OAT_ZERO_VARIANT=verified_first_global_replay_canonical'* ]]; then
    for required in \
      ...
      if [[ "$job_record" != *"$required"* ]]; then
        echo "E61 treatment audit failed for $job_id: missing $required" >&2
        exit 1
      fi
    done
  elif [[ "$job_record" == *'OAT_ZERO_VARIANT=grpo'* ]]; then
    if [[ "$job_record" != *'OAT_ZERO_ONLINE_CANONICAL_REPLAY=0'* ]]; then
      echo "E61 baseline audit failed for $job_id: replay is not disabled" >&2
      exit 1
    fi
  else
    echo "E61 job $job_id is neither frozen arm" >&2
    exit 1
  fi
done
