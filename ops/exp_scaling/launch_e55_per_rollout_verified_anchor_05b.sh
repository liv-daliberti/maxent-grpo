#!/usr/bin/env bash
# Launch E55's target-free common-mass smoke or three-domain sentinel.
set -euo pipefail
...
phase="${1:-}"
case "$phase" in
  config|smoke_graph|smoke_python|sentinel) ;;
  *)
    echo "Usage: $0 {config|smoke_graph|smoke_python|sentinel}" >&2
    exit 1
    ;;
esac

PYTHON_BIN="${OAT_ZERO_PYTHON:-$ROOT_DIR/var/seed_paper_eval/paper310/bin/python}"
PROTOCOL="$ROOT_DIR/paper/preregistration/e55_per_rollout_verified_anchor_05b.md"
AUDITOR="$ROOT_DIR/ops/exp_scaling/audit_e55_sentinel.py"
...
GRAPH_PREFIX=gce55_per_rollout_verified_anchor_05b_50ep_sentinel
COUNTDOWN_PREFIX=cde55_per_rollout_verified_anchor_05b_50ep_sentinel_allcs
PYTHON_PREFIX=pye55_per_rollout_verified_anchor_05b_50ep_sentinel_allcs
GRAPH_SMOKE_PREFIX=gce55_per_rollout_verified_anchor_05b_smoke
PYTHON_SMOKE_PREFIX=pye55_per_rollout_verified_anchor_05b_smoke
IDENTITY="$ROOT_DIR/var/artifacts/e55_per_rollout_verified_anchor_identity.json"
GRAPH_SMOKE_IDENTITY="$ROOT_DIR/var/artifacts/e55_per_rollout_verified_anchor_graph_smoke_identity.json"
PYTHON_SMOKE_IDENTITY="$ROOT_DIR/var/artifacts/e55_per_rollout_verified_anchor_python_smoke_identity.json"
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing E55 prerequisite: $required" >&2
    exit 1
  fi
done
if [[ "$phase" == sentinel && -e "$IDENTITY" ]]; then
  echo "Fresh E55 identity required; file exists: $IDENTITY" >&2
  exit 1
fi
if [[ "$phase" == smoke_graph && -e "$GRAPH_SMOKE_IDENTITY" ]]; then
  echo "Fresh E55 graph smoke identity required; file exists: $GRAPH_SMOKE_IDENTITY" >&2
  exit 1
fi
if [[ "$phase" == smoke_python && -e "$PYTHON_SMOKE_IDENTITY" ]]; then
  echo "Fresh E55 Python smoke identity required; file exists: $PYTHON_SMOKE_IDENTITY" >&2
  exit 1
fi
...
freeze_execution() {
  ...
  SOURCE_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e55_per_rollout_verified_anchor_${SOURCE_HASH}/src"
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E55 source snapshot hash mismatch" >&2
    exit 1
  fi

  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e55-ops-input.XXXXXX")"
  ...
  OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e55_per_rollout_verified_anchor_ops_${EXECUTION_HASH}/ops"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E55 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
payload = {
    ...
    "replay": {
        ...
        "raw_score_gradient_sum": -1.0,
        "objective_scale": 0.0625,
        "applied_score_gradient_sum": -0.0625,
        "base_alpha": 0.10,
        "capacity": 16,
...
submit_domain() {
  ...
  case "$domain" in
    graph_coloring)
      ...
      export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E55_GRAPH_NODELIST:-node302}"
      export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E55_GRAPH_GRES:-gpu:a100:1}"
      export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E55_GRAPH_PARTITION:-all}"
      export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E55_GRAPH_ACCOUNT:-mltheory}"
      ;;
    countdown)
      ...
      export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E55_COUNTDOWN_NODELIST:-node020,node022,node023,node024,node026}"
      export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E55_COUNTDOWN_GRES:-gpu:rtx_3090:1}"
      export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E55_COUNTDOWN_PARTITION:-lowprio}"
      export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E55_COUNTDOWN_ACCOUNT:-allcs}"
      ;;
    python_factor)
      ...
      export OAT_ZERO_RESUME_STEPS=384
      export OAT_ZERO_RESUME_FROM=384
      export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E55_PYTHON_NODELIST:-node020,node022,node023,node024,node026}"
      export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E55_PYTHON_GRES:-gpu:rtx_3090:1}"
      export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E55_PYTHON_PARTITION:-lowprio}"
      export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E55_PYTHON_ACCOUNT:-allcs}"
      ;;
    *)
      echo "Unknown E55 domain: $domain" >&2
      exit 1
      ;;
  esac
...
if [[ "$phase" == config ]]; then
  ...
  echo "[e55] all three sentinel configurations passed; no jobs submitted"
  exit 0
fi

if [[ "$phase" == smoke_graph || "$phase" == smoke_python ]]; then
  ...
  if [[ "$phase" == smoke_graph ]]; then
    write_identity "$GRAPH_SMOKE_IDENTITY" \
      e55_per_rollout_verified_anchor_05b_graph_smoke_v1
    export OAT_ZERO_PROTOCOL_IDENTITY="$GRAPH_SMOKE_IDENTITY"
    export RUN_STAMP_PREFIX="$GRAPH_SMOKE_PREFIX"
    export OAT_ZERO_COMPARATIVE_TASK=graph_coloring
    export OAT_ZERO_COMPARATIVE_DATA_ROOT="$GRAPH_DATA_ROOT"
    export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E55_GRAPH_SMOKE_NODELIST:-node023}"
    export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E55_GRAPH_SMOKE_GRES:-gpu:rtx_3090:1}"
    export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E55_GRAPH_SMOKE_PARTITION:-lowprio}"
    export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E55_GRAPH_SMOKE_ACCOUNT:-allcs}"
  else
    write_identity "$PYTHON_SMOKE_IDENTITY" \
      e55_per_rollout_verified_anchor_05b_python_smoke_v1
    export OAT_ZERO_PROTOCOL_IDENTITY="$PYTHON_SMOKE_IDENTITY"
    export RUN_STAMP_PREFIX="$PYTHON_SMOKE_PREFIX"
    export OAT_ZERO_COMPARATIVE_TASK=python_factor
    export OAT_ZERO_COMPARATIVE_DATA_ROOT="$PYTHON_DATA_ROOT"
    export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E55_PYTHON_SMOKE_NODELIST:-node022}"
    export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E55_PYTHON_SMOKE_GRES:-gpu:rtx_3090:1}"
    export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E55_PYTHON_SMOKE_PARTITION:-lowprio}"
    export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E55_PYTHON_SMOKE_ACCOUNT:-allcs}"
  fi
  export OAT_ZERO_TRAIN_CPUS_PER_TASK=8
  export OAT_ZERO_TRAIN_MEMORY=64G
  ...
  export OAT_ZERO_SBATCH_HOLD=0
  "$OPS_ROOT/submit_countdown_comparative.sh"
  echo "[e55] submitted ${phase#smoke_} engineering smoke"
  exit 0
fi

write_identity "$IDENTITY" e55_per_rollout_verified_anchor_05b_sentinel_v1
...
cleanup_partial_cohort() {
  ...
  if [[ "$COHORT_RELEASED" != "1" && "${#job_ids[@]}" -gt 0 ]]; then
    ...
    echo "[e55] cancelled incomplete held sentinel: ${job_ids[*]}" >&2
  fi
  exit "$status"
}
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX" "$PYTHON_PREFIX"; do
  ...
  if [[ "${#task_jobs[@]}" -ne 1 ]]; then
    echo "E55 ${prefix} has ${#task_jobs[@]} jobs; expected 1" >&2
    exit 1
  fi
  job_ids+=("${task_jobs[@]}")
done
if [[ "${#job_ids[@]}" -ne 3 ]]; then
  echo "E55 held cohort has ${#job_ids[@]} jobs; expected 3" >&2
  exit 1
fi
scontrol release "${job_ids[@]}"
COHORT_RELEASED=1
trap - EXIT
echo "[e55] released held sentinel cohort: ${job_ids[*]}"
