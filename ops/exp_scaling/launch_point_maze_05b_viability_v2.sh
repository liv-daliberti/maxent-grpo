#!/usr/bin/env bash
# Freeze and submit the development-only PointMaze base-model viability v2 prompt repair.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/point_maze_05b_viability_v2_prompt_repair_20260729.md"
EVALUATOR="$ROOT_DIR/ops/evaluate_modebench_base_viability.py"
SLURM_SCRIPT="$ROOT_DIR/ops/slurm/evaluate_point_maze_05b_viability_v2.slurm"
IDENTITY="$ROOT_DIR/var/artifacts/point_maze_05b_viability_v2_identity.json"
RECEIPT="$ROOT_DIR/var/artifacts/point_maze_05b_viability_v2.json"
...
for required in \
  ...
  "$SLURM_SCRIPT" "$ROOT_DIR/var/maze_runtime/venv/bin/python" "$V1_RECEIPT"; do
  if [[ ! -e "$required" ]]; then
...
"$PYTHON_BIN" - "$AUDIT" "$DATA_ROOT/identity.json" "$V1_RECEIPT" <<'PY'
import json
import sys
for path in sys.argv[1:]:
    ...
    if path.endswith("point_maze_05b_viability_v1.json") and (
        payload.get("status") != "fail" or payload.get("decision") != "stopped_before_training"
    ):
        raise SystemExit("PointMaze v1 is not the frozen failed gate")
PY
...
SOURCE_PARENT="$ROOT_DIR/var/artifacts/source_snapshots/point_maze_viability_v2_${SOURCE_HASH}"
...
EXECUTION_INPUT="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.pointmaze-viability-v2-ops.XXXXXX")"
cp "$EVALUATOR" "$EXECUTION_INPUT/evaluate_modebench_base_viability.py"
cp "$SLURM_SCRIPT" "$EXECUTION_INPUT/evaluate_point_maze_05b_viability_v2.slurm"
EXECUTION_HASH="$(hash_tree "$EXECUTION_INPUT")"
EXECUTION_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/point_maze_viability_v2_ops_${EXECUTION_HASH}"
...
job_id="$(
  sbatch --parsable --hold \
    ...
    "$EXECUTION_ROOT/evaluate_point_maze_05b_viability_v2.slurm"
)"
job_id="${job_id%%;*}"
if [[ ! "$job_id" =~ ^[0-9]+$ ]]; then
...
"$PYTHON_BIN" - \
  ...
  "$EVALUATOR" "$SLURM_SCRIPT" "$AUDIT" "$DATA_ROOT/identity.json" "$V1_RECEIPT" <<'PY'
import hashlib
import json
import os
import pathlib
...
payload = {
    "schema_version": "point-maze-05b-viability-identity-v2",
