#!/usr/bin/env bash
# Freeze and submit the prospective PointMaze closed-loop viability gate.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
PYTHON_BIN="$ROOT_DIR/var/seed_paper_eval/paper310/bin/python"
PROTOCOL="$ROOT_DIR/paper/preregistration/point_maze_interactive_05b_viability_v1_20260729.md"
EVALUATOR="$ROOT_DIR/ops/evaluate_point_maze_interactive_viability.py"
SLURM_SCRIPT="$ROOT_DIR/ops/slurm/evaluate_point_maze_interactive_viability.slurm"
IDENTITY="$ROOT_DIR/var/artifacts/point_maze_interactive_05b_viability_v1_identity.json"
RECEIPT="$ROOT_DIR/var/artifacts/point_maze_interactive_05b_viability_v1.json"
...
for required in \
  ...
  "$MODEL/config.json" "$WORKER_PYTHON" \
  "$ROOT_DIR/src/oat_drgrpo/point_maze_interactive_worker.py"; do
  [[ -f "$required" ]] || {
    echo "Missing PointMaze interactive prerequisite: $required" >&2
    exit 1
  }
done
...
PY
  echo "[point-interactive] configuration passed; no model sampled"
  exit 0
fi

for fresh in "$IDENTITY" "$RECEIPT"; do
  if [[ -e "$fresh" ]]; then
    echo "Fresh PointMaze interactive target required: $fresh" >&2
    exit 1
  fi
done

SOURCE_HASH="$(hash_tree "$ROOT_DIR/src")"
SOURCE_PARENT="$ROOT_DIR/var/artifacts/source_snapshots/point_interactive_${SOURCE_HASH}"
...
EXECUTION_INPUT="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.point-interactive-ops.XXXXXX")"
cp "$EVALUATOR" "$EXECUTION_INPUT/evaluate_point_maze_interactive_viability.py"
cp "$SLURM_SCRIPT" "$EXECUTION_INPUT/evaluate_point_maze_interactive_viability.slurm"
EXECUTION_HASH="$(hash_tree "$EXECUTION_INPUT")"
EXECUTION_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/point_interactive_ops_${EXECUTION_HASH}"
if [[ ! -f "$EXECUTION_ROOT/evaluate_point_maze_interactive_viability.py" ]]; then
...
job_id="$(
  sbatch --parsable --hold \
    ...
    "$EXECUTION_ROOT/evaluate_point_maze_interactive_viability.slurm"
)"
job_id="${job_id%%;*}"
[[ "$job_id" =~ ^[0-9]+$ ]] || {
  echo "Invalid PointMaze interactive job ID: $job_id" >&2
  exit 1
}
...
payload = {
    "schema_version": "point-maze-interactive-viability-identity-v1",
    ...
    "sample_count": 64,
    "prefix_count": 16,
    "policy_interface": "closed_loop_finite_action_one_token_v1",
...
trap - EXIT
echo "[point-interactive] released viability job $job_id"
echo "[point-interactive] identity=$IDENTITY"
