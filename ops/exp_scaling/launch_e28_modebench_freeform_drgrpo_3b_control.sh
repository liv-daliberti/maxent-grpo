#!/usr/bin/env bash
# Launch E28's post-hoc matched free-form Dr.GRPO controls for E25-v2.
set -euo pipefail
...
PHASE="${1:-}"
case "$PHASE" in
  config|full|release-staged|countdown-retry|countdown-release-staged) ;;
  *) echo "Usage: $0 {config|full|release-staged|countdown-retry|countdown-release-staged}" >&2; exit 2 ;;
esac

PYTHON_BIN="$ROOT_DIR/var/seed_paper_eval/paper310/bin/python"
PROTOCOL="$ROOT_DIR/paper/preregistration/e28_modebench_freeform_drgrpo_3b_control.md"
...
GRAPH_PREFIX=gce28_freeform_drgrpo_3b_v1
COUNTDOWN_PREFIX=cde28_freeform_drgrpo_3b_v1
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing frozen E28 prerequisite: $required" >&2
    exit 1
  fi
done

if ! grep -q '^\*\*Status: FROZEN POST-HOC MATCHED-CONTROL EXTENSION' "$PROTOCOL"; then
  echo "E28 protocol is not frozen" >&2
  exit 1
fi
...
export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E28_TRAIN_NODELIST:-node302}"
export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E28_TRAIN_GRES:-gpu:a100:1}"
export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E28_TRAIN_PARTITION:-mltheory}"
export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E28_TRAIN_ACCOUNT:-mltheory}"
export OAT_ZERO_TRAIN_MEMORY="${OAT_ZERO_E28_TRAIN_MEMORY:-96G}"
export OAT_ZERO_TRAIN_TIME_LIMIT="${OAT_ZERO_E28_TRAIN_TIME_LIMIT:-168:00:00}"
...
if [[ "$PHASE" == config ]]; then
  ...
  echo "[e28] both matched 3B free-form Dr.GRPO configurations passed; no jobs submitted"
  exit 0
fi

unset OAT_ZERO_COMPARATIVE_CONFIG_ONLY
export OAT_ZERO_SBATCH_HOLD=1
if [[ "$PHASE" == countdown-retry || "$PHASE" == countdown-release-staged ]]; then
  countdown_manifest="$ROOT_DIR/var/artifacts/${COUNTDOWN_PREFIX}_comparative_jobs.tsv"
  if [[ ! -f "$countdown_manifest" ]]; then
    echo "E28 Countdown recovery requires the original manifest: $countdown_manifest" >&2
    exit 1
  fi
  if [[ "$PHASE" == countdown-retry ]]; then
    ...
    mapfile -t job_ids < <(
      tail -n "+$((manifest_lines_before + 1))" "$countdown_manifest" |
        awk -F '\t' '$1 == "grpo" && $2 ~ /^(43|44|45)$/ && $3 ~ /^[0-9]+$/ {print $3}'
    )
  else
    echo "[e28] auditing the existing held Countdown recovery; no duplicate jobs submitted"
    mapfile -t job_ids < <(
      tail -n 3 "$countdown_manifest" |
        awk -F '\t' '$1 == "grpo" && $2 ~ /^(43|44|45)$/ && $3 ~ /^[0-9]+$/ {print $3}'
    )
  fi
  if (( ${#job_ids[@]} != 3 )); then
    echo "E28 Countdown recovery incomplete (${#job_ids[@]}/3); new jobs remain held" >&2
    exit 1
  fi
  for job_id in "${job_ids[@]}"; do
    ...
    for required in \
      ...
      if [[ "$job_record" != *"$required"* ]]; then
        echo "E28 Countdown recovery audit failed for $job_id: missing $required" >&2
        exit 1
      fi
    done
  done
  scontrol release "${job_ids[@]}"
  echo "[e28] released three same-stamp Countdown recovery jobs: ${job_ids[*]}"
  exit 0
fi
...
if [[ "$PHASE" == full ]]; then
  for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX"; do
    ...
    if [[ -e "$manifest" ]]; then
      echo "E28 manifest already exists; refusing duplicate cohort: $manifest" >&2
      exit 1
    fi
  done
  submit_task graph_coloring
  ...
  echo "[e28] auditing the existing held cohort; no duplicate jobs submitted"
fi

job_ids=()
for spec in \
  "$GRAPH_PREFIX|192|48" \
  "$COUNTDOWN_PREFIX|384|96"; do
  IFS='|' read -r prefix max_train save_steps <<< "$spec"
  manifest="$ROOT_DIR/var/artifacts/${prefix}_comparative_jobs.tsv"
  mapfile -t task_jobs < <(
    awk -F '\t' 'NR > 1 && $1 == "grpo" && $2 ~ /^(43|44|45)$/ && $3 ~ /^[0-9]+$/ {print $3}' "$manifest"
  )
  if (( ${#task_jobs[@]} != 3 )); then
    echo "E28 ${prefix} cohort incomplete (${#task_jobs[@]}/3); jobs remain held" >&2
    exit 1
  fi
  for job_id in "${task_jobs[@]}"; do
    ...
    for required in \
      ...
      if [[ "$job_record" != *"$required"* ]]; then
        echo "E28 held-job audit failed for $job_id: missing $required" >&2
        exit 1
      fi
    done
  done
...
if (( ${#job_ids[@]} != 6 )); then
  echo "E28 cohort incomplete (${#job_ids[@]}/6); jobs remain held" >&2
  exit 1
fi
if (( $(printf '%s\n' "${job_ids[@]}" | sort -u | wc -l) != 6 )); then
  echo "E28 manifests contain duplicate job IDs; jobs remain held" >&2
  exit 1
fi
scontrol release "${job_ids[@]}"
echo "[e28] released complete six-job matched-control cohort: ${job_ids[*]}"
