#!/usr/bin/env python3
"""Configure or launch the frozen Ant v15 slate under controller v17-r1."""
...
from oat_drgrpo.ant_maze_worker_v17_r1 import (  # noqa: E402
    _executor_identity,
    controller_identity,
)
...
base.MAKER = (
    ROOT / "ops/make_ant_maze_mode_data_v15_controller_v17_r1.py"
)
base.AUDITOR = (
    ROOT / "ops/audit_ant_maze_mode_data_v15_controller_v17_r1.py"
)
base.BATCH = (
    ROOT / "ops/slurm/admit_ant_maze_modebench_v15_controller_v17_r1.slurm"
)
base.DATA = ROOT / "var/data/ant_maze_modebench_v15_controller_v17_r1"
base.AUDIT = (
    ROOT
    / "var/artifacts/"
...
base.SUBMISSION = (
    ROOT
    / "var/artifacts/"
    "ant_maze_v15_controller_v17_r1_admission_submission.json"
)
...
def snapshot_tree(source: Path, _prefix: str):
    return _snapshot_tree(source, "ant_v15_controller_v17_r1_source")


def snapshot_execution():
    path, digest = _snapshot_execution()
    target = path.with_name(path.name.replace("ant_v14_hard_ops_", "ant_v15_v17_r1_ops_"))
...
def validate() -> None:
    ...
    base.run(
        [
            ...
            "py_compile",
            str(LAUNCHER),
            str(ROOT / "src/oat_drgrpo/ant_maze_worker_v17_r1.py"),
            str(ROOT / "src/oat_drgrpo/maze_modebench_worker_v17_r1.py"),
            str(ROOT / "src/oat_drgrpo/maze_modebench_process_v17_r1.py"),
        ],
        env=environment,
    )
...
def atomic(path: Path, payload: Any) -> None:
    if path == base.IDENTITY:
        ...
        payload = {
            **payload,
            "schema_version": "ant-maze-v17-route-generation-identity-v1",
            ...
            **executor,
            ...
            "v16_controller_outcome_loaded": True,
            "v17_controller_outcome_loaded": True,
            "v17_route_sampled_before_freeze": False,
        ...
        payload = {
            **payload,
            "schema": "ant-maze-v15-controller-v17-r1-submission-v1",
        }
