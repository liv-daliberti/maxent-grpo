#!/usr/bin/env bash
# Freeze and submit the conditional three-node AntMaze v10 exact-slate replay.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/ant_maze_v10_cross_node_audit_20260729.md"
EXPORTER="$ROOT_DIR/ops/export_ant_maze_v10_cross_node_specs.py"
AUDITOR="$ROOT_DIR/ops/audit_ant_maze_cross_node_v10.py"
AGGREGATOR="$ROOT_DIR/ops/aggregate_ant_maze_cross_node_v10.py"
SLURM_SCRIPT="$ROOT_DIR/ops/slurm/audit_ant_maze_cross_node_v10.slurm"
...
ROUTE_AUDIT="$ROOT_DIR/var/artifacts/ant_maze_modebench_v10_admission_audit.json"
CONTROLLER_RECEIPT="$ROOT_DIR/var/maze_runtime/controllers/ant_waypoint_v10.evaluation.json"
CONTROLLER_MODEL="$ROOT_DIR/var/maze_runtime/controllers/ant_waypoint_v10.zip"
SPECS="$ROOT_DIR/var/artifacts/ant_maze_v10_cross_node_specs_v1.json"
IDENTITY="$ROOT_DIR/var/artifacts/ant_maze_v10_cross_node_v1_identity.json"
AGGREGATE="$ROOT_DIR/var/artifacts/ant_maze_v10_cross_node_v1.json"
PREFIX="$ROOT_DIR/var/artifacts/ant_maze_v10_cross_node_v1_replica"
phase="${1:-}"
...
for required in "$PYTHON_BIN" "$WORKER_PYTHON" "$PROTOCOL" "$EXPORTER" \
  ...
  [[ -f "$required" ]] || {
    echo "Missing Ant v10 cross-node prerequisite: $required" >&2
    exit 1
  }
done
...
if [[ "$phase" == config ]]; then
  ...
  PYTHONPATH="$ROOT_DIR/ops:$ROOT_DIR/src" "$PYTHON_BIN" -m pytest -q \
    ...
    "$ROOT_DIR/tests/test_ant_maze_cross_node_v10.py"
  echo "[ant-v10-cross-node] configuration passed; no route executed"
  exit 0
fi
...
for required in "$DATA_ROOT/identity.json" "$ROUTE_IDENTITY" "$ROUTE_AUDIT" \
  ...
  [[ -f "$required" ]] || {
    echo "Missing admitted Ant v10 route artifact: $required" >&2
    exit 1
  }
done
...
if identity.get("schema_version") != "ant-maze-v10-route-generation-identity-v1":
    raise SystemExit("wrong Ant v10 route identity")
if identity.get("map_substitution") is not False or identity.get("language_model_sampling") is not False:
    raise SystemExit("Ant v10 route identity violates the information boundary")
if identity.get("controller_receipt_sha256") != digest(sys.argv[3]):
    raise SystemExit("Ant v10 route identity receipt hash mismatch")
if identity.get("controller_model_sha256") != digest(sys.argv[4]):
    raise SystemExit("Ant v10 route identity model hash mismatch")
if audit.get("status") != "pass" or audit.get("decision") != "admitted_to_v10_cross_node_route_determinism_gate":
    raise SystemExit("Ant v10 route audit is not admitted")
if audit.get("real_simulator_replays") != 24 or audit.get("perturbation_replays") != 2400:
    raise SystemExit("Ant v10 route audit replay count mismatch")
PYANT

for fresh in "$SPECS" "$IDENTITY" "$AGGREGATE" \
  "${PREFIX}_0.json" "${PREFIX}_1.json" "${PREFIX}_2.json"; do
  [[ ! -e "$fresh" ]] || {
    echo "Fresh Ant v10 cross-node target required: $fresh" >&2
    exit 1
  }
done
...
[[ -f "$SOURCE_ROOT/oat_drgrpo/ant_maze_worker_v10.py" ]] || {
  echo "Exact Ant v10 route source snapshot is unavailable" >&2
  exit 1
}
[[ "$(hash_tree "$SOURCE_ROOT")" == "$SOURCE_HASH" ]] || {
  echo "Exact Ant v10 route source snapshot hash mismatch" >&2
  exit 1
}

EXECUTION_INPUT="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.ant-v10-cross-node.XXXXXX")"
cp "$EXPORTER" "$EXECUTION_INPUT/export_ant_maze_v10_cross_node_specs.py"
cp "$AUDITOR" "$EXECUTION_INPUT/audit_ant_maze_cross_node_v10.py"
cp "$AGGREGATOR" "$EXECUTION_INPUT/aggregate_ant_maze_cross_node_v10.py"
cp "$SLURM_SCRIPT" "$EXECUTION_INPUT/audit_ant_maze_cross_node_v10.slurm"
cp "$PROTOCOL" "$EXECUTION_INPUT/ant_maze_v10_cross_node_audit_20260729.md"
EXECUTION_HASH="$(hash_tree "$EXECUTION_INPUT")"
EXECUTION_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/ant_v10_cross_node_ops_${EXECUTION_HASH}"
if [[ ! -f "$EXECUTION_ROOT/audit_ant_maze_cross_node_v10.py" ]]; then
...
"$PYTHON_BIN" "$EXECUTION_ROOT/export_ant_maze_v10_cross_node_specs.py" \
  --data-root "$DATA_ROOT" \
...
job_id="$(sbatch --parsable --hold \
  ...
  "$EXECUTION_ROOT/audit_ant_maze_cross_node_v10.slurm")"
job_id="${job_id%%;*}"
[[ "$job_id" =~ ^[0-9]+$ ]] || { echo "Invalid Ant v10 cross-node job ID" >&2; exit 1; }
...
for required in 'JobState=PENDING' 'Reason=JobHeldUser' 'NumNodes=3' 'NumTasks=3' \
  ...
  [[ "$job_record" == *"$required"* ]] || {
    echo "Held Ant v10 cross-node job missing $required" >&2
    exit 1
  }
done

"$PYTHON_BIN" - "$IDENTITY" "$job_id" "$SOURCE_HASH" "$EXECUTION_HASH" \
  "$EXECUTION_ROOT/ant_maze_v10_cross_node_audit_20260729.md" "$SPECS_HASH" \
  "$ROUTE_IDENTITY" "$ROUTE_AUDIT" <<'PYID'
import hashlib,json,os,pathlib,sys,tempfile
def digest(path): return hashlib.sha256(pathlib.Path(path).read_bytes()).hexdigest()
path=pathlib.Path(sys.argv[1]); payload={
 "schema_version":"ant-maze-v10-cross-node-identity-v1","job_id":int(sys.argv[2]),
...
trap - EXIT
echo "[ant-v10-cross-node] released three-node audit job $job_id"
echo "[ant-v10-cross-node] identity=$IDENTITY"
