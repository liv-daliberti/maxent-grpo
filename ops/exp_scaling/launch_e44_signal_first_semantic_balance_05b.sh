#!/usr/bin/env bash
# Launch E44's treatment-only signal-first semantic-balance cohort.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e44_signal_first_semantic_balance_05b.md"
...
IDENTITY="$ROOT_DIR/var/artifacts/e44_signal_first_semantic_balance_05b_v2_identity.json"
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing frozen E44 prerequisite: $required" >&2
    exit 1
  fi
done
if ! grep -q '^\*\*Status: FROZEN BEFORE LAUNCH' "$PROTOCOL"; then
  echo "E44 protocol is not frozen" >&2
  exit 1
fi
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "E44 MATH materialization is incomplete: $required" >&2
    exit 1
  fi
done
...
if ! grep -q 'semantic_shannon_separate_advantage' \
   ...
   ! grep -q 'xdr_task_advantage_weights' \
     "$ROOT_DIR/src/oat_drgrpo/args.py" || \
   ! grep -q 'task_advantages_for_xdr' \
   ...
   ! grep -q 'submit_arm signal_first_semantic_balance signal_first_semantic_balance' \
     "$ROOT_DIR/ops/submit_countdown_comparative.sh" || \
   ! grep -q '^  signal_first_semantic_balance)' \
     "$ROOT_DIR/ops/run_experiment.sh" || \
   ! grep -q 'export OAT_ZERO_XDR_TASK_ADVANTAGE_WEIGHTS=1' \
     "$ROOT_DIR/ops/run_experiment.sh" || \
  ...
  echo "Current source/execution stack lacks the frozen E44 combined xDr plus success-conditioned semantic mechanism" >&2
  exit 1
fi
...
if (
    ...
    raise SystemExit("E37 identity does not match the frozen E44 comparator")
...
if (
    ...
    raise SystemExit("E38 identity does not match the frozen E44 comparator")
...
if (
    ...
    raise SystemExit("E39 identity does not match the frozen E44 MATH comparator")
...
if data_hash != e39.get("data", {}).get("materialization_manifest_sha256"):
    raise SystemExit("E44 MATH materialization identity differs from frozen E39")
...
if (
    ...
    raise SystemExit("E41 identity does not match the frozen E44 parent treatment")
...
if (
    ...
    raise SystemExit("E42 identity does not match the frozen E44 parent treatment")
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX"; do
  ...
  if [[ "$phase" == full && -e "$manifest" ]]; then
    echo "Fresh E44 prefix required; manifest already exists: $manifest" >&2
    exit 1
  fi
done
if [[ "$phase" == full && -e "$IDENTITY" ]]; then
  echo "Fresh E44 identity required; artifact already exists: $IDENTITY" >&2
  exit 1
fi
...
freeze_execution() {
  ...
  SOURCE_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e44_signal_first_semantic_balance_05b_v2_${SOURCE_HASH}/src"
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E44 source snapshot hash mismatch" >&2
    exit 1
  fi

  local ops_staging ops_input
  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e44-ops-input.XXXXXX")"
  ...
  OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e44_signal_first_semantic_balance_05b_v2_ops_${EXECUTION_HASH}/ops"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E44 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
MODEBENCH_NODELIST="${OAT_ZERO_E44_MODEBENCH_TRAIN_NODELIST:-node105,node202,node203,node204}"
MODEBENCH_GRES="${OAT_ZERO_E44_MODEBENCH_TRAIN_GRES:-gpu:a5000:1}"
MODEBENCH_CPUS="${OAT_ZERO_E44_MODEBENCH_TRAIN_CPUS_PER_TASK:-4}"
MODEBENCH_MEMORY="${OAT_ZERO_E44_MODEBENCH_TRAIN_MEMORY:-32G}"
TRAIN_PARTITION="${OAT_ZERO_E44_TRAIN_PARTITION:-all}"
TRAIN_ACCOUNT="${OAT_ZERO_E44_TRAIN_ACCOUNT:-mltheory}"
TRAIN_TIME_LIMIT="${OAT_ZERO_E44_TRAIN_TIME_LIMIT:-24:00:00}"
...
payload = {
    ...
    "tasks": {
        "graph_coloring": {
            "prompt_pool": 192,
            "prefix": "gce44_signal_first_semantic_balance_05b_v2",
        },
        "countdown": {
            "prompt_pool": 384,
            "prefix": "cde44_signal_first_semantic_balance_05b_v2",
        },
    },
...
if [[ "$phase" == config ]]; then
  ...
  echo "[e44] both treatment-only configurations passed; no jobs submitted"
  exit 0
fi
...
cleanup_partial_cohort() {
  ...
  if [[ "$COHORT_RELEASED" != "1" ]]; then
    ...
    if [[ "${#cleanup_ids[@]}" -gt 0 ]]; then
      ...
      echo "[e44] cancelled incomplete held cohort: ${cleanup_ids[*]}" >&2
    fi
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX"; do
  ...
  if [[ "${#task_jobs[@]}" -ne "$EXPECTED_JOBS_PER_TASK" ]]; then
    echo "E44 ${prefix} has ${#task_jobs[@]} jobs; expected $EXPECTED_JOBS_PER_TASK" >&2
    exit 1
  fi
  job_ids+=("${task_jobs[@]}")
done
if [[ "${#job_ids[@]}" -ne "$EXPECTED_JOBS" ]]; then
  echo "E44 cohort has ${#job_ids[@]} jobs; expected $EXPECTED_JOBS" >&2
  exit 1
fi

"$PYTHON_BIN" - \
  "$ROOT_DIR/var/artifacts/${GRAPH_PREFIX}_comparative_jobs.tsv" \
  "$ROOT_DIR/var/artifacts/${COUNTDOWN_PREFIX}_comparative_jobs.tsv" <<'PY'
import csv
import pathlib
import sys

expected = {
    ("signal_first_semantic_balance", str(seed))
    for seed in (43, 44, 45)
}
for manifest_name in sys.argv[1:]:
    ...
    if len(rows) != 3 or observed != expected:
        raise SystemExit(
            f"E44 held manifest mismatch: {manifest_name} "
...
for job_id in "${job_ids[@]}"; do
  if ! scontrol update JobId="$job_id" Partition="$TRAIN_PARTITION"; then
    echo "E44 could not normalize held job $job_id to $TRAIN_PARTITION" >&2
    exit 1
  fi
done
...
for spec in \
  ...
  "$COUNTDOWN_PREFIX|$COUNTDOWN_DATA_ROOT|384|96|64|384|qwen_boxed|multi_answer|fast|256|192|512|4|370100|0|$MODEBENCH_NODELIST|$MODEBENCH_CPUS|$MODEBENCH_MEMORY"; do
  ...
  for job_id in "${task_jobs[@]}"; do
    ...
    for required in \
      ...
      if [[ "$job_record" != *"$required"* ]]; then
        echo "E44 held-job audit failed for $job_id: missing $required" >&2
        exit 1
      fi
    done
    if [[ "$arm" != signal_first_semantic_balance ]]; then
      echo "E44 manifest has unexpected arm for $job_id: $arm" >&2
      exit 1
    fi
    if [[ "$job_record" != *"Partition=${TRAIN_PARTITION}"* ]] || \
      ...
      echo "E44 held job $job_id does not attest the frozen placement" >&2
      exit 1
    fi
  done
done
...
COHORT_RELEASED=1
trap - EXIT
echo "[e44] released ${#job_ids[@]} signal-first semantic-balance jobs: ${job_ids[*]}"
echo "[e44] identity=$IDENTITY"
