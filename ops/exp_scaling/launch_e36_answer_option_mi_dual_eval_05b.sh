#!/usr/bin/env bash
# Launch E36's three-seed Dr.GRPO versus answer-option MI dual-eval cohort.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e36_answer_option_mi_dual_eval_05b.md"
...
GRAPH_PREFIX=gce36_answer_option_mi_dual_eval_05b_v1
COUNTDOWN_PREFIX=cde36_answer_option_mi_dual_eval_05b_v1
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing frozen E36 prerequisite: $required" >&2
    exit 1
  fi
done
if ! grep -q '^\*\*Status: FROZEN BEFORE LAUNCH' "$PROTOCOL"; then
  echo "E36 protocol is not frozen" >&2
  exit 1
fi
...
if [[ "$phase" == full ]]; then
  for manifest in \
    ...
    if [[ -e "$manifest" ]]; then
      echo "Fresh E36 prefix required; manifest already exists: $manifest" >&2
      exit 1
    fi
  done
fi
...
export OAT_ZERO_TRAIN_SEEDS=3601,3602,3603
...
export OAT_ZERO_EVAL_MODE_COVERAGE_SEED=360100
...
export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E36_TRAIN_NODELIST:-node105,node202,node203,node204,node302}"
export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E36_TRAIN_GRES:-gpu:1}"
export OAT_ZERO_TRAIN_CPUS_PER_TASK="${OAT_ZERO_E36_TRAIN_CPUS_PER_TASK:-8}"
export OAT_ZERO_TRAIN_MEMORY="${OAT_ZERO_E36_TRAIN_MEMORY:-64G}"
export OAT_ZERO_TRAIN_TIME_LIMIT="${OAT_ZERO_E36_TRAIN_TIME_LIMIT:-08:00:00}"
...
echo "[e36] ${phase} complete for the three-seed dual-evaluation cohort"
