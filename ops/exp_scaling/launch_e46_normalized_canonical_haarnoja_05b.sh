#!/usr/bin/env bash
# Launch E46's adaptive-only extension against the frozen E44-OGS comparators.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e46_normalized_online_canonical_haarnoja_05b.md"
...
GRAPH_PREFIX=gce46_normalized_canonical_haarnoja_05b_v1
COUNTDOWN_PREFIX=cde46_normalized_canonical_haarnoja_05b_v1
IDENTITY="$ROOT_DIR/var/artifacts/e46_normalized_canonical_haarnoja_05b_v1_identity.json"
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing frozen E46 prerequisite: $required" >&2
    exit 1
  fi
done
if ! grep -q '^\*\*Status: FROZEN BEFORE LAUNCH' "$PROTOCOL"; then
  echo "E46 protocol is not frozen" >&2
  exit 1
fi
for contract in \
  ...
  'src/oat_drgrpo/learner/grpo.py:entropy_alpha_override' \
  'src/oat_drgrpo/args.py:online_canonical_dual_target_ratio' \
  'ops/run_experiment.sh:online_canonical_haarnoja)' \
  'ops/submit_countdown_comparative.sh:submit_arm online_canonical_haarnoja online_canonical_haarnoja'; do
  path="${contract%%:*}"
  needle="${contract#*:}"
  if ! grep -q "$needle" "$ROOT_DIR/$path"; then
    echo "Current source lacks E46 contract: $path :: $needle" >&2
    exit 1
  fi
done
...
for manifest in \
  ...
  if [[ "$phase" == full && -e "$manifest" ]]; then
    echo "Fresh E46 prefix required; manifest exists: $manifest" >&2
    exit 1
  fi
done
if [[ "$phase" == full && -e "$IDENTITY" ]]; then
  echo "Fresh E46 identity required; file exists: $IDENTITY" >&2
  exit 1
fi
...
freeze_execution() {
  ...
  SOURCE_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e46_normalized_canonical_haarnoja_05b_${SOURCE_HASH}/src"
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E46 source snapshot hash mismatch" >&2
    exit 1
  fi

  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e46-ops-input.XXXXXX")"
  ...
  OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e46_normalized_canonical_haarnoja_05b_ops_${EXECUTION_HASH}/ops"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E46 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E46_TRAIN_NODELIST:-node302}"
export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E46_TRAIN_GRES:-gpu:a100:1}"
export OAT_ZERO_TRAIN_CPUS_PER_TASK="${OAT_ZERO_E46_TRAIN_CPUS_PER_TASK:-8}"
export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E46_TRAIN_PARTITION:-all}"
export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E46_TRAIN_ACCOUNT:-mltheory}"
export OAT_ZERO_TRAIN_MEMORY="${OAT_ZERO_E46_TRAIN_MEMORY:-64G}"
export OAT_ZERO_TRAIN_TIME_LIMIT="${OAT_ZERO_E46_TRAIN_TIME_LIMIT:-24:00:00}"
...
payload = {
    "schema": "e46_normalized_canonical_haarnoja_05b_v1",
    ...
    "arms": ["online_canonical_haarnoja"],
    "comparators": {
        "prefixes": [
    ...
    "num_samples": 16,
    "prompt_epochs": 10,
    "placement": {"node": "node302", "gpu_per_job": "a100:1"},
}
...
if [[ "$phase" == config ]]; then
  ...
  echo "[e46] both adaptive-only configurations passed; no jobs submitted"
  exit 0
fi
...
cleanup_partial_cohort() {
  ...
  if [[ "$COHORT_RELEASED" != "1" && "${#job_ids[@]}" -gt 0 ]]; then
    ...
    echo "[e46] cancelled incomplete held cohort: ${job_ids[*]}" >&2
  fi
  exit "$status"
}
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX"; do
  ...
  if [[ "${#task_jobs[@]}" -ne "$EXPECTED_JOBS_PER_TASK" ]]; then
    echo "E46 ${prefix} has ${#task_jobs[@]} jobs; expected $EXPECTED_JOBS_PER_TASK" >&2
    exit 1
  fi
  job_ids+=("${task_jobs[@]}")
...
expected = {("online_canonical_haarnoja", str(seed)) for seed in (43, 44, 45)}
observed = {(row["arm"], row["seed"]) for row in rows}
if len(rows) != 3 or observed != expected:
    raise SystemExit(f"E46 held manifest mismatch: {observed!r}")
PY
done
if [[ "${#job_ids[@]}" -ne "$EXPECTED_JOBS" ]]; then
  echo "E46 cohort has ${#job_ids[@]} jobs; expected $EXPECTED_JOBS" >&2
  exit 1
fi
  ...
  for job_id in "${task_jobs[@]}"; do
    ...
    for required in \
      ...
      if [[ "$job_record" != *"$required"* ]]; then
        echo "E46 held-job audit failed for $job_id: missing $required" >&2
        exit 1
      fi
    done
    if [[ "$job_record" != *"--nodelist=node302"* ]] || \
      ...
      echo "E46 held job $job_id does not attest one-A100 node302 placement" >&2
      exit 1
    fi
  done
done
...
COHORT_RELEASED=1
trap - EXIT
echo "[e46] released ${#job_ids[@]} adaptive jobs: ${job_ids[*]}"
echo "[e46] identity=$IDENTITY"
