#!/usr/bin/env python3
"""Render E62-R9's live same-seed Python mechanism comparison."""
...
CURVE = (
    ROOT
    / "var/artifacts/"
    "pye62r9_open_set_sweep_05b_1ep_pilot_scaling_curve.json"
)
IDENTITY = (
    ROOT / "var/artifacts/e62r9_open_set_sweep_python_pilot_identity.json"
)
AUDIT = ROOT / "var/artifacts/e62r9_python_pilot_audit_latest.json"
OUT = ROOT / "paper/figures/e62r9_python_open_set_sweep_pilot_live"
...
def _run_metrics(identity: dict[str, Any], arm: str) -> Path | None:
    ...
    candidates = sorted(
        (ROOT / "var/data").glob(
            "*pye62r9_open_set_sweep_05b_1ep_pilot_"
...
def main() -> None:
    ...
    ax.set_ylim(bottom=0)
    ax.set_title("E62 verified transform discoveries and activity", fontsize=9)
    ax2 = ax.twinx()
    ...
    handles = [
        ...
        Line2D(
            ...
            lw=2.5,
            marker="D",
            label="E62-R9 validator-preserving transform, seed 9011",
        ),
    ]
    fig.legend(
        handles=handles,
    ...
    fig.suptitle(
        "E62 executable Python factors — same-seed verified-transform "
        "mechanism pilot\n"
    ...
    fig.text(
        0.5,
        0.006,
        "A model-generated verified outcome is transformed by a "
        "validator-preserving task rule and independently revalidated; "
