#!/usr/bin/env python3
"""Live fail-closed audit for E65R1's prospective 12-job repair cohort."""
...
IDENTITY = (
    ROOT / "var/artifacts/e65r1_entropy_gated_singleton_confirmation_identity.json"
)
PROTOCOL = (
    ROOT / "paper/preregistration/e65_five_domain_terminal_confirmation_05b.md"
)
LAUNCHER = (
    ROOT / "ops/exp_scaling/launch_e65_entropy_gated_singleton_confirmation.sh"
)
OUT = (
    ROOT / "var/artifacts/e65r1_entropy_gated_singleton_confirmation_audit_latest.json"
)
EXPECTED_STEPS = {
...
RTX2080_BFLOAT16 = re.compile(
    r"Bfloat16 is only supported on GPUs with compute capability of at least "
    r"8\.0.*RTX 2080 Ti GPU has compute capability 7\.5",
...
def _scan_log(path: Path) -> tuple[list[str], int]:
    ...
    for match in UNCAUGHT.finditer(text):
        ...
        if "Traceback" in match.group(0) and SIGTERM.search(nearby):
            interruptions += 1
        elif "Traceback" in match.group(0) and RTX2080_BFLOAT16.search(nearby):
            # Narrowly registered zero-step placement incompatibility from
            # e65r1_rtx2080_rollback_amendment_20260727.md. A later unrelated
            # traceback will fall outside this attempt-local window.
            interruptions += 1
        else:
...
def main() -> None:
    ...
    if not IDENTITY.is_file():
        payload = {
            "schema": "e65r1_entropy_gated_singleton_confirmation_audit_v1",
    ...
    print(
        f"[e65r1-audit] status={payload['status']} "
