#!/usr/bin/env bash
# Configure or submit E58's exact-seed global verified-replay Python smoke.
set -euo pipefail
...
AUDITOR="$ROOT_DIR/ops/exp_scaling/audit_e58_global_replay_smoke.py"
...
IDENTITY="$ROOT_DIR/var/artifacts/e58_global_verified_replay_python_smoke_attempt3_identity.json"
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing E58 smoke prerequisite: $required" >&2
    exit 1
  fi
done
...
if [[ "$phase" == python ]]; then
  for fresh in "$MANIFEST" "$IDENTITY"; do
    if [[ -e "$fresh" ]]; then
      echo "Fresh E58 smoke artifact required; already exists: $fresh" >&2
      exit 1
    fi
  done
fi
...
freeze_execution() {
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E58 source snapshot hash mismatch" >&2
    exit 1
  fi

  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e58-ops-input.XXXXXX")"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E58 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
write_identity() {
  ...
  "$PYTHON_BIN" - \
    ...
    "$manifest_hash" "$SOURCE_HASH" "$EXECUTION_HASH" "$job_id" <<'PY'
import json
import os
import pathlib
import sys
...
payload = {
    "schema": "e58_global_verified_replay_python_smoke_v1",
    "attempt": 3,
...
export OAT_ZERO_SAVE_STEPS=256
export OAT_ZERO_SAVE_FROM=256
export OAT_ZERO_MAX_SAVE_NUM=1
export OAT_ZERO_RESUME_STEPS=256
export OAT_ZERO_RESUME_FROM=256
...
export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E58_PYTHON_NODELIST:-node020,node022,node023,node024,node026}"
export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E58_PYTHON_GRES:-gpu:rtx_3090:1}"
export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E58_PYTHON_PARTITION:-lowprio}"
export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E58_PYTHON_ACCOUNT:-allcs}"

if [[ "$phase" == config ]]; then
  export OAT_ZERO_COMPARATIVE_CONFIG_ONLY=1
  export OAT_ZERO_SBATCH_HOLD=0
  "$OPS_ROOT/submit_countdown_comparative.sh"
  echo "[e58] Python global-replay smoke configuration passed"
  exit 0
fi
...
cleanup_held() {
  ...
  if [[ "$released" != "1" && "$job_id" =~ ^[0-9]+$ ]]; then
    ...
    echo "[e58] cancelled incomplete held smoke $job_id" >&2
  fi
  exit "$status"
}
...
if [[ "${#job_ids[@]}" -ne 1 ]]; then
  echo "E58 Python smoke has ${#job_ids[@]} jobs; expected one" >&2
  exit 1
fi
job_id="${job_ids[0]}"
...
released=1
trap - EXIT
echo "[e58] released Python global-replay smoke job $job_id"
