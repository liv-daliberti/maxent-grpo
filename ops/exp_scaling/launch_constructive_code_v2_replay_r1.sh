...
REPAIR_PROTOCOL="$ROOT_DIR/paper/preregistration/constructive_code_v2_replay_r1_relocation_repair_20260729.md"
FAILED_IDENTITY="$ROOT_DIR/var/artifacts/constructive_code_v2_replay_identity.json"
FAILED_LOG="$ROOT_DIR/var/artifacts/logs/constructive-code-v2-gate-30185883.err"
SLURM_SCRIPT="$ROOT_DIR/ops/slurm/replay_constructive_code_v2.slurm"
IDENTITY="$ROOT_DIR/var/artifacts/constructive_code_v2_replay_r1_identity.json"
...
for required in "$PYTHON_BIN" "$PROTOCOL" "$REPAIR_PROTOCOL" "$FAILED_IDENTITY" "$FAILED_LOG" "$SLURM_SCRIPT" "$IMAGE" "$TESTLIB/testlib.h"; do
...
if [[ "$phase" == config ]]; then
  ...
  bash -n "$SLURM_SCRIPT"
  echo "[constructive-v2-replay-r1] configuration passed; no candidate executed"
  exit 0
fi
...
OPS_INPUT="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.constructive-v2-replay-r1-ops.XXXXXX")"
for file in "${FILES[@]}"; do
...
cp "$REPAIR_PROTOCOL" "$OPS_INPUT/protocol/constructive_code_v2_replay_r1_relocation_repair_20260729.md"
mkdir -p "$OPS_INPUT/antecedents"
cp "$FAILED_IDENTITY" "$OPS_INPUT/antecedents/r0_identity.json"
cp "$FAILED_LOG" "$OPS_INPUT/antecedents/r0_stderr.log"
...
OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/constructive_code_v2_replay_r1_ops_${OPS_HASH}"
...
"$PYTHON_BIN" - "$IDENTITY" "$job_id" "$SOURCE_HASH" "$OPS_HASH" \
  ...
  "$OPS_ROOT/testlib/testlib.h" "$OPS_ROOT/inputs/v2/manifest.json" \
  "$OPS_ROOT/protocol/constructive_code_v2_replay_r1_relocation_repair_20260729.md" \
  "$OPS_ROOT/antecedents/r0_identity.json" "$OPS_ROOT/antecedents/r0_stderr.log" <<PY
import hashlib
import json
import os
import pathlib
...
payload = {
    "schema_version": "constructive-code-v2-replay-r1-identity-v1",
    ...
    "repair_protocol_sha256": digest(sys.argv[13]),
    "failed_r0_identity_sha256": digest(sys.argv[14]),
    "failed_r0_stderr_sha256": digest(sys.argv[15]),
    "failed_r0_job_id": 30185883,
    "r0_candidate_execution_count": 0,
    "repair_scope": "relocation-safe logical v1 root; all executable criteria unchanged",
...
trap - EXIT
echo "[constructive-v2-replay-r1] released gate job $job_id"
echo "[constructive-v2-replay-r1] identity=$IDENTITY"
