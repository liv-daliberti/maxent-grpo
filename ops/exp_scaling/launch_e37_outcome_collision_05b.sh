#!/usr/bin/env bash
# Launch the clean matched E37 0.5B Dr.GRPO/outcome-collision cohort.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e37_outcome_collision_05b.md"
...
IDENTITY="$ROOT_DIR/var/artifacts/e37_outcome_collision_05b_v1_identity.json"
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing frozen E37 prerequisite: $required" >&2
    exit 1
  fi
done
if ! grep -q '^\*\*Status: FROZEN BEFORE LAUNCH' "$PROTOCOL"; then
  echo "E37 protocol is not frozen" >&2
  exit 1
fi
...
for manifest in \
  ...
  if [[ "$phase" == full && -e "$manifest" ]]; then
    echo "Fresh E37 prefix required; manifest already exists: $manifest" >&2
    exit 1
  fi
done
...
freeze_execution() {
  ...
  SOURCE_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e37_outcome_collision_05b_${SOURCE_HASH}/src"
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E37 source snapshot hash mismatch" >&2
    exit 1
  fi

  local ops_staging ops_input
  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e37-ops-input.XXXXXX")"
  ...
  OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e37_outcome_collision_05b_ops_${EXECUTION_HASH}/ops"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E37 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
export OAT_ZERO_ONLY_ARMS=grpo,outcome_collision
...
export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E37_TRAIN_NODELIST:-node105,node202,node203,node204}"
export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E37_TRAIN_GRES:-gpu:a5000:1}"
export OAT_ZERO_TRAIN_CPUS_PER_TASK="${OAT_ZERO_E37_TRAIN_CPUS_PER_TASK:-4}"
export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E37_TRAIN_PARTITION:-all}"
export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E37_TRAIN_ACCOUNT:-mltheory}"
export OAT_ZERO_TRAIN_MEMORY="${OAT_ZERO_E37_TRAIN_MEMORY:-32G}"
export OAT_ZERO_TRAIN_TIME_LIMIT="${OAT_ZERO_E37_TRAIN_TIME_LIMIT:-24:00:00}"
...
payload = {
    "schema": "e37_outcome_collision_05b_v1",
    ...
    "outcome_collision_coefficient": 0.10,
    "seeds": [43, 44, 45],
    "num_samples": 16,
    "prompt_epochs": 10,
    "evaluation": {
...
if [[ "$phase" == config ]]; then
  ...
  echo "[e37] both matched 0.5B configurations passed; no jobs submitted"
  exit 0
fi
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX"; do
  ...
  if [[ "${#task_jobs[@]}" -ne "$EXPECTED_JOBS_PER_TASK" ]]; then
    ...
    echo "E37 ${prefix} has ${#task_jobs[@]} jobs; expected $EXPECTED_JOBS_PER_TASK" >&2
    exit 1
  fi
  job_ids+=("${task_jobs[@]}")
done

# Normalize the site-filtered partition while jobs are still held, then attest
# both their shared execution contract and the one frozen treatment knob.
for job_id in "${job_ids[@]}"; do
  if ! scontrol update JobId="$job_id" Partition="$OAT_ZERO_TRAIN_PARTITION"; then
    scancel "${job_ids[@]}" 2>/dev/null || true
    echo "E37 could not normalize held job $job_id to ${OAT_ZERO_TRAIN_PARTITION}" >&2
    exit 1
  fi
done
  ...
  for job_id in "${task_jobs[@]}"; do
    ...
    for required in \
      ...
      if [[ "$job_record" != *"$required"* ]]; then
        scancel "${job_ids[@]}" 2>/dev/null || true
        echo "E37 held-job audit failed for $job_id: missing $required" >&2
        exit 1
      fi
    done
    if [[ "$arm" == outcome_collision ]]; then
      for required in \
        'OAT_ZERO_VARIANT=outcome_collision' \
        'OAT_ZERO_OUTCOME_COLLISION_COEF=0.10'; do
        if [[ "$job_record" != *"$required"* ]]; then
          scancel "${job_ids[@]}" 2>/dev/null || true
          echo "E37 treatment audit failed for $job_id: missing $required" >&2
          exit 1
        fi
      done
    elif [[ "$arm" == grpo ]]; then
      for required in \
        ...
        'OAT_ZERO_OUTCOME_COLLISION_COEF=0.0'; do
        if [[ "$job_record" != *"$required"* ]]; then
          scancel "${job_ids[@]}" 2>/dev/null || true
          echo "E37 control audit failed for $job_id: missing $required" >&2
          exit 1
        fi
      done
    else
      scancel "${job_ids[@]}" 2>/dev/null || true
      echo "E37 manifest has unexpected arm for $job_id: $arm" >&2
      exit 1
    fi
  done
done
...
for job_id in "${job_ids[@]}"; do
  ...
  if [[ "$job_record" != *"Partition=${OAT_ZERO_TRAIN_PARTITION}"* ]] || \
    ...
    scancel "${job_ids[@]}" 2>/dev/null || true
    echo "E37 held job $job_id does not attest the frozen placement" >&2
    exit 1
  fi
done

scontrol release "${job_ids[@]}"
echo "[e37] released ${#job_ids[@]} clean matched 0.5B jobs: ${job_ids[*]}"
