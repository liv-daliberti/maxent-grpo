#!/usr/bin/env bash
# Submit E66's same-plumbing, actuator-off controls from E65R1 snapshots.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e66_same_plumbing_actuator_ablation_05b.md"
...
GRAPH_PREFIX=gce66_same_plumbing_control_05b_12ep
COUNTDOWN_PREFIX=cde66_same_plumbing_control_05b_12ep
PYTHON_PREFIX=pye66_same_plumbing_control_05b_12ep
MATHIR_PREFIX=mie66_same_plumbing_control_05b_12ep
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing E66 prerequisite: $required" >&2
    exit 1
  fi
done
...
if [[ "$phase" == full ]]; then
  for fresh in \
    ...
    if [[ -e "$fresh" ]]; then
      echo "Fresh E66 artifact required; already exists: $fresh" >&2
      exit 1
    fi
  done
fi
...
for flag in \
  ...
  VERIFIED_ENTROPY_GATED_SINGLETON_ESCAPE_CANONICAL; do
...
submit_domain() {
  ...
  case "$domain" in
    ...
    countdown)
      ...
      export OAT_ZERO_TRAIN_NODELIST=node024
...
if [[ "$phase" == config ]]; then
  ...
  echo "[e66] all four same-plumbing control configurations passed"
  exit 0
fi
...
for manifest in \
  ...
  if [[ "${#manifest_jobs[@]}" -ne 3 ]]; then
    echo "E66 manifest $manifest has ${#manifest_jobs[@]} jobs; expected 3" >&2
    exit 1
  fi
...
payload = {
    "schema": "e66_same_plumbing_actuator_ablation_v1",
    ...
    "comparison_role": "same_plumbing_actuator_off_control",
    "seeds": [43, 44, 45],
    "passes": 12,
...
if [[ "${#job_ids[@]}" -ne 12 ]]; then
  echo "E66 expected 12 jobs; got ${#job_ids[@]}" >&2
  exit 1
fi
for job_id in "${job_ids[@]}"; do
  ...
  for required in \
    ...
    if [[ "$record" != *"$required"* ]]; then
      echo "E66 held-job audit failed for $job_id: missing $required" >&2
      exit 1
    fi
  done
done
...
released=1
trap - EXIT
echo "[e66] released 12 jobs: ${job_ids[*]}"
echo "[e66] identity=$IDENTITY"
