#!/usr/bin/env bash
# Submit the conditional development-only AntMaze v9 base-model viability gate.
set -euo pipefail
...
CROSS_NODE_IDENTITY="$ROOT_DIR/var/artifacts/ant_maze_v9_cross_node_v1_identity.json"
CROSS_NODE_AUDIT="$ROOT_DIR/var/artifacts/ant_maze_v9_cross_node_v1.json"
PROTOCOL="$ROOT_DIR/paper/preregistration/ant_maze_05b_viability_v9_20260729.md"
EVALUATOR="$ROOT_DIR/ops/evaluate_modebench_base_viability.py"
SLURM_SCRIPT="$ROOT_DIR/ops/slurm/evaluate_ant_maze_05b_viability_v9.slurm"
IDENTITY="$ROOT_DIR/var/artifacts/ant_maze_05b_viability_v9_identity.json"
RECEIPT="$ROOT_DIR/var/artifacts/ant_maze_05b_viability_v9.json"
...
for required in "$PYTHON_BIN" "$MODEL_ROOT/config.json" "$PROTOCOL" \
  ...
  [[ -e "$required" ]] || {
    echo "Missing AntMaze v9 viability prerequisite: $required" >&2
    exit 1
  }
done
...
if [[ "$phase" == config ]]; then
  ...
  echo "[antmaze-v9-viability] configuration passed; no model sampled"
  exit 0
fi
...
for required in "$DATA_ROOT/identity.json" "$DATA_ROOT/dev/dataset_dict.json" \
  ...
  [[ -f "$required" ]] || {
    echo "Missing admitted AntMaze v9 viability antecedent: $required" >&2
    exit 1
  }
done
...
if route.get("schema_version") != "ant-maze-v9-route-generation-identity-v1":
    raise SystemExit("wrong AntMaze v9 route identity")
if identity.get("schema_version") != "ant-maze-v9-cross-node-identity-v1":
    raise SystemExit("wrong AntMaze v9 cross-node identity")
if identity.get("language_model_sampling") is not False:
    raise SystemExit("AntMaze v9 cross-node identity sampled a language model")
if audit.get("status") != "pass" or audit.get("decision") != "eligible_for_frozen_05b_viability_gate_v9":
    raise SystemExit("AntMaze v9 cross-node audit is not admitted")
if audit.get("validated_count") != 216 or audit.get("node_count") != 3:
    raise SystemExit("AntMaze v9 cross-node audit count mismatch")
PYANT

for fresh in "$IDENTITY" "$RECEIPT"; do
  [[ ! -e "$fresh" ]] || {
    echo "Fresh AntMaze v9 viability artifact required: $fresh" >&2
    exit 1
  }
done
...
[[ -f "$SOURCE_ROOT/oat_drgrpo/ant_maze_worker_v9.py" ]] || {
  echo "Exact AntMaze v9 route source snapshot is unavailable" >&2
  exit 1
}
[[ "$(hash_tree "$SOURCE_ROOT")" == "$SOURCE_HASH" ]] || {
  echo "Exact AntMaze v9 route source snapshot hash mismatch" >&2
  exit 1
}

EXECUTION_INPUT="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.ant-v9-viability.XXXXXX")"
cp "$EVALUATOR" "$EXECUTION_INPUT/evaluate_modebench_base_viability.py"
cp "$SLURM_SCRIPT" "$EXECUTION_INPUT/evaluate_ant_maze_05b_viability_v9.slurm"
cp "$PROTOCOL" "$EXECUTION_INPUT/ant_maze_05b_viability_v9_20260729.md"
EXECUTION_HASH="$(hash_tree "$EXECUTION_INPUT")"
EXECUTION_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/ant_maze_v9_viability_ops_${EXECUTION_HASH}"
...
job_id="$(sbatch --parsable --hold \
  ...
  "$EXECUTION_ROOT/evaluate_ant_maze_05b_viability_v9.slurm")"
job_id="${job_id%%;*}"
[[ "$job_id" =~ ^[0-9]+$ ]] || { echo "Invalid AntMaze v9 viability job ID" >&2; exit 1; }
...
for required in 'JobState=PENDING' 'Reason=JobHeldUser' 'TresPerNode=gres/gpu:a5000:1' \
  ...
  [[ "$job_record" == *"$required"* ]] || {
    echo "Held AntMaze v9 viability job missing $required" >&2
    exit 1
  }
done

"$PYTHON_BIN" - "$IDENTITY" "$job_id" "$SOURCE_HASH" "$EXECUTION_HASH" \
  "$EXECUTION_ROOT/ant_maze_05b_viability_v9_20260729.md" "$EVALUATOR" \
...
path=pathlib.Path(sys.argv[1]); payload={
 "schema_version":"ant-maze-05b-viability-identity-v9","job_id":int(sys.argv[2]),
 ...
 "sample_count":64,"prefix_count":16,"sampling_seed":97309,
 "minimum_prefix_success_prompts":2,"minimum_multimode_prompts":1,
 "assistant_prefix":"\\boxed{","prompt_repair":"ant_maze_v9",
...
trap - EXIT
echo "[antmaze-v9-viability] released job $job_id"
echo "[antmaze-v9-viability] identity=$IDENTITY"
