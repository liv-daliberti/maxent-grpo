#!/usr/bin/env python3
"""Freeze and schedule the Ant v15/v17 admission launcher dependency."""
...
CONTROLLER_JOB = 30205033
DEPENDENCY_BATCH = (
    ROOT
    / "ops/slurm/"
    "launch_ant_maze_v15_controller_v17_r1_after_controller_r2.slurm"
)
LAUNCHER = (
    ROOT
    / "ops/exp_scaling/"
...
MANIFEST = (
    ROOT
    / "var/artifacts/"
    "ant_maze_v15_controller_v17_r1_dependent_launcher_identity.json"
)
FILES = (
    ROOT
...
def schedule() -> None:
    ...
    try:
        ...
        atomic(
            ...
            {
                "schema_version": (
                    "ant-maze-v15-controller-v17-r1-dependent-launcher-v1"
                ),
        ...
        raise
    print(f"[ant-v15-v17] scheduled dependent launcher {job_id}")
...
def verify_run(manifest: Path) -> None:
    ...
    if payload.get("schema_version") != (
        ...
        raise RuntimeError("Ant v15/v17 dependent-launcher schema mismatch")
    if payload.get("controller_job_id") != CONTROLLER_JOB:
        raise RuntimeError("Ant v15/v17 controller dependency drift")
    if payload.get("file_sha256") != file_hashes():
        raise RuntimeError("Ant v15/v17 frozen launcher inputs drifted")
    subprocess.run(
        [str(PYTHON), str(LAUNCHER), "run"], cwd=ROOT, check=True
    )
