#!/usr/bin/env bash
# Launch E34's bounded free-form Dr.GRPO versus semantic answer-option MI pilot.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e34_answer_option_mi_05b_pilot.md"
...
GRAPH_PREFIX=gce34_answer_option_mi_05b_pilot_v2
COUNTDOWN_PREFIX=cde34_answer_option_mi_05b_pilot_v2
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing frozen E34 prerequisite: $required" >&2
    exit 1
  fi
done
if ! grep -q '^\*\*Status: FROZEN BEFORE LAUNCH' "$PROTOCOL"; then
  echo "E34 protocol is not frozen" >&2
  exit 1
fi
...
if [[ "$phase" == full ]]; then
  for manifest in \
    ...
    if [[ -e "$manifest" ]]; then
      echo "Fresh E34 prefix required; manifest already exists: $manifest" >&2
      exit 1
    fi
  done
fi
...
export OAT_ZERO_TRAIN_SEEDS=3401
...
export OAT_ZERO_EVAL_MODE_COVERAGE_SEED=340100
...
export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E34_TRAIN_NODELIST:-node105,node202,node203,node204,node302}"
export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E34_TRAIN_GRES:-gpu:1}"
export OAT_ZERO_TRAIN_CPUS_PER_TASK="${OAT_ZERO_E34_TRAIN_CPUS_PER_TASK:-8}"
export OAT_ZERO_TRAIN_MEMORY="${OAT_ZERO_E34_TRAIN_MEMORY:-64G}"
export OAT_ZERO_TRAIN_TIME_LIMIT="${OAT_ZERO_E34_TRAIN_TIME_LIMIT:-08:00:00}"
...
echo "[e34] ${phase} complete for matched graph-coloring and Countdown pilots"
