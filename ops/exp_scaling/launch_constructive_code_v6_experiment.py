#!/usr/bin/env python3
"""Launch identity-bound ConstructiveCode v6 smoke or final cells."""
...
import train_constructive_code_v6 as train  # noqa: E402


PYTHON = ROOT / "var/seed_paper_eval/paper310/bin/python"
TRAINER = ROOT / "ops/train_constructive_code_v6.py"
AUDITOR = ROOT / "ops/audit_constructive_code_v6.py"
TRAIN_BATCH = ROOT / "ops/slurm/train_constructive_code_v6.slurm"
AUDIT_BATCH = ROOT / "ops/slurm/audit_constructive_code_v6.slurm"
STAGE_DEP_BATCH = (
    ROOT / "ops/slurm/launch_constructive_code_v6_stage_b_after_smoke.slurm"
)
V6_EVALUATOR = ROOT / "ops/evaluate_constructive_code_v6_coder_viability.py"
GATE_AUDITOR = ROOT / "ops/audit_constructive_code_v6_gate.py"
...
GATE_IDENTITY = ROOT / "var/artifacts/constructive_code_v6_gate_identity.json"
GATE_AUDIT = ROOT / "var/artifacts/constructive_code_v6_gate_audit.json"
VIABILITY = ROOT / "var/artifacts/constructive_code_v6_coder_05b_viability.json"
...
PAIRED_PROTOCOL = (
    ROOT
    / "paper/preregistration/constructive_code_v6_paired_online_smoke_20260730.md"
)
STAGE_PROTOCOL = (
    ROOT
    / "paper/preregistration/constructive_code_v6_stage_b_05b_12pass_20260730.md"
)
PAIRED_IDENTITY = ROOT / "var/artifacts/constructive_code_v6_paired_smoke_identity.json"
PAIRED_SUBMISSION = ROOT / "var/artifacts/constructive_code_v6_paired_smoke_submission.json"
PAIRED_AUDIT = ROOT / "var/artifacts/constructive_code_v6_paired_smoke_audit.json"
STAGE_IDENTITY = ROOT / "var/artifacts/constructive_code_v6_stage_b_identity.json"
STAGE_SUBMISSION = ROOT / "var/artifacts/constructive_code_v6_stage_b_submission.json"
STAGE_AUDIT = ROOT / "var/artifacts/constructive_code_v6_stage_b_audit.json"
STAGE_MANIFEST = (
    ROOT
...
def execution_snapshot(mode: str) -> tuple[str, Path, dict[str, str]]:
    ...
    files = {
        "train_constructive_code_v6.py": TRAINER,
        "audit_constructive_code_v6.py": AUDITOR,
        "train_constructive_code_v6.slurm": TRAIN_BATCH,
        "audit_constructive_code_v6.slurm": AUDIT_BATCH,
        "launch_constructive_code_v6_stage_b_after_smoke.slurm": STAGE_DEP_BATCH,
        "evaluate_constructive_code_v6_coder_viability.py": V6_EVALUATOR,
        "audit_constructive_code_v6_gate.py": GATE_AUDITOR,
        ...
        "launch_constructive_code_v6_experiment.py": Path(__file__).resolve(),
    }
    snapshots = ROOT / "var/artifacts/source_snapshots"
    snapshots.mkdir(parents=True, exist_ok=True)
    temporary = Path(
        tempfile.mkdtemp(prefix=".constructive-v6-experiment.", dir=snapshots)
    )
    for name, source in files.items():
        shutil.copy2(source, temporary / name)
    execution_hash = shell_tree_sha256(temporary)
    destination = snapshots / f"constructive_code_v6_experiment_{execution_hash}"
...
def launch() -> None:
    ...
    if args.phase == "config":
        run(
            [
                ...
                "pytest",
                "-q",
                str(ROOT / "tests/test_constructive_code_v6_training.py"),
                str(ROOT / "tests/test_constructive_code_v6_coder_viability.py"),
                str(ROOT / "tests/test_constructive_code_v6_gate.py"),
            ],
            timeout=300,
            capture=False,
        )
        print(f"[constructive-v6-launch] {args.mode} configuration passed; no jobs submitted")
        return
    ...
    if args.mode == train.PAIRED_MODE:
        for arm in train.ARMS:
            stem = f"constructive_code_v6_paired_smoke_{arm}"
            specs.append((arm, arm, 78101, stem))
    else:
        for arm in train.ARMS:
            for seed in (43, 44, 45, 46, 47):
                label = f"{arm}/s{seed}"
                stem = f"constructive_code_v6_stage_b_{arm}_s{seed}"
    ...
    try:
        for label, arm, seed, stem in specs:
            ...
            job_id = sbatch(
                "--hold",
                f"--job-name=constructive-v6-{args.mode}-{arm}-s{seed}",
        ...
        audit_job = sbatch(
            ...
            f"--job-name=audit-constructive-v6-{args.mode}",
        ...
        identity = {
            "schema": (
                "constructive-code-v6-paired-smoke-identity-v1"
                if args.mode == train.PAIRED_MODE
                else "constructive-code-v6-stage-b-identity-v1"
            ),
            ...
            "v6_manifest_sha256": sha256_file(V6_MANIFEST),
        ...
        submission = {
            "schema": (
                "constructive-code-v6-paired-smoke-submission-v1"
                if args.mode == train.PAIRED_MODE
                else "constructive-code-v6-stage-b-submission-v1"
            ),
            "generated_at": datetime.now(timezone.utc).isoformat(),
        ...
        raise
    print(
        f"[constructive-v6-launch] mode={args.mode} cells={len(records)} "
