#!/usr/bin/env bash
# Launch E49S's matched three-epoch deterministic-MathIR hard-MATH toy.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e49s_deterministic_mathir_repairs_20260724.md"
ADVANCEMENT="$ROOT_DIR/var/artifacts/e49s_deterministic_mathir_repairs_v1/advancement_decision.json"
CERTIFICATES="$ROOT_DIR/var/artifacts/e49s_deterministic_mathir_repairs_v1/deterministic_certificates.jsonl"
REPAIR_IDENTITY="$ROOT_DIR/var/artifacts/e49s_deterministic_mathir_repairs_v1/frozen_identity.json"
DATA_ROOT="$ROOT_DIR/var/data/e49s_deterministic_mathir_toy"
ENDPOINT_RECORD="${E49S_QWEN72_ENDPOINT_RECORD:-$ROOT_DIR/var/artifacts/e49c_math_strategy_qwen72_v1/qwen72_endpoint.json}"
...
PREFIX=e49s_deterministic_mathir_toy_05b_3ep_v1
...
if [[ ! -x "$PYTHON_BIN" ]] \
  ...
  echo "E49S frozen prerequisite is missing" >&2
  exit 1
fi

read -r ADVANCE TRAIN_MULTI EVAL_MULTI <<<"$(
  "$PYTHON_BIN" - "$ADVANCEMENT" <<'PY'
import json
import sys
decision = json.load(open(sys.argv[1], encoding="utf-8"))
print(
    decision.get("advance_to_matched_toy_training", False),
    decision.get("support_counts", {}).get("train_multi", -1),
    decision.get("support_counts", {}).get("eval_multi", -1),
)
PY
)"
if [[ "$ADVANCE" != True ]] || (( TRAIN_MULTI < 10 || EVAL_MULTI < 10 )); then
  echo "E49S deterministic repair gate has not passed" >&2
  exit 1
fi
...
if (
    manifest.get("schema")
    != "e49s_deterministic_mathir_materialization_v1"
    or manifest.get("menu_count") != 100
):
    raise SystemExit("invalid E49S data manifest")
...
if [[ "$TRAIN_DATA_HASH" != "$EXPECTED_TRAIN_HASH" ]] \
  ...
  echo "E49S data identity mismatch" >&2
  exit 1
fi
...
if any(record.get(key) != value for key, value in expected.items()):
    raise SystemExit("unexpected E49S judge endpoint")
print(f"http://{record['node']}:{int(record['port'])}/v1")
PY
)"

SOURCE_HASH="$(hash_tree "$ROOT_DIR/src")"
SOURCE_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e49s_deterministic_mathir_${SOURCE_HASH}/src"
...
if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
  echo "E49S source snapshot mismatch" >&2
  exit 1
fi

OPS_INPUT="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e49s-ops.XXXXXX")"
...
OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e49s_deterministic_mathir_ops_${OPS_HASH}/ops"
...
if [[ "$mode" == run && -e "$MANIFEST" ]]; then
  echo "Fresh E49S launch required; manifest already exists: $MANIFEST" >&2
  exit 1
fi
...
"$PYTHON_BIN" - "$IDENTITY" "$SOURCE_HASH" "$OPS_HASH" "$ENDPOINT" \
  ...
  "$ADVANCEMENT" "$CERTIFICATES" "$REPAIR_IDENTITY" "$ENDPOINT_RECORD" \
  "$LAUNCHER_HASH" <<'PY'
import hashlib
import json
import os
...
payload = {
    "schema": "e49s_deterministic_mathir_toy_05b_3ep_v1",
    ...
    "advancement_decision_sha256": sha256(sys.argv[9]),
    "deterministic_certificates_sha256": sha256(sys.argv[10]),
    "repair_identity_sha256": sha256(sys.argv[11]),
    ...
    "prompt_epochs": 3,
    "num_samples": 16,
    "support_counts": {"train_multi": 10, "eval_multi": 10},
    "task_reward_gate": "exact_trace_contract_plus_two_execution_audits",
    "method": {
...
if [[ "$mode" == config ]]; then
  ...
  echo "[e49s] configuration passed; no jobs submitted"
  exit 0
fi
...
if [[ "$rows" != 2 ]]; then
  echo "E49S expected two matched jobs, observed $rows" >&2
  exit 1
fi
echo "[e49s] submitted matched three-epoch toy cohort"
