...
LAUNCHER="$ROOT_DIR/ops/exp_scaling/launch_constructive_code_v6_experiment.py"
BATCH="$ROOT_DIR/ops/slurm/launch_constructive_code_v6_paired_after_viability.slurm"
OUTPUT="$ROOT_DIR/var/artifacts/constructive_code_v6_paired_dependency_identity.json"
bash -n "$BATCH"
"$ROOT_DIR/var/seed_paper_eval/paper310/bin/python" -m py_compile "$LAUNCHER"
if [[ "$phase" == config ]]; then
  echo "[constructive-v6-paired-dependency] configuration passed; no job submitted"
  exit 0
fi
...
path=pathlib.Path(sys.argv[1]); payload={
 "schema":"constructive-code-v6-paired-dependency-identity-v1",
...
os.replace(tmp,path)
PY
echo "[constructive-v6-paired-dependency] submitted job $job_id afterok:$viability_job"
