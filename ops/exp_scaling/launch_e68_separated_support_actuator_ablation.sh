#!/usr/bin/env bash
# Freeze and atomically submit E68's 12-job prospective repair cohort.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e68_separated_support_actuator_ablation_05b.md"
...
PYTHON_PREFIX=pye68_separated_support_actuator_05b_12ep
MATHIR_PREFIX=mie68_separated_support_actuator_05b_12ep
...
for required in \
  "$PROTOCOL" "$CAPACITY_PROBE" "$MODEL_ROOT/config.json" \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing E68 prerequisite: $required" >&2
    exit 1
  fi
done
...
if [[ "$phase" == full ]]; then
  for fresh in \
    ...
    if [[ -e "$fresh" ]]; then
      echo "Fresh E68 artifact required; already exists: $fresh" >&2
      exit 1
    fi
  done
fi
...
freeze_execution() {
  ...
  SOURCE_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e68_separated_support_${SOURCE_HASH}/src"
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E68 source snapshot hash mismatch" >&2
    exit 1
  fi

  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e68-ops-input.XXXXXX")"
  ...
  OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e68_separated_support_ops_${EXECUTION_HASH}/ops"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E68 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
submit_domain() {
  ...
  case "$domain" in
    graph_coloring)
      ...
      export OAT_ZERO_TRAIN_NODELIST=node103,node104
      export OAT_ZERO_TRAIN_GRES=gpu:a6000:1
      export OAT_ZERO_TRAIN_PARTITION=pvl-lowprio
    ...
    countdown)
      ...
      export OAT_ZERO_TRAIN_NODELIST=node020,node021,node022,node024,node026
...
if [[ "$phase" == config ]]; then
  ...
  echo "[e68] all four configurations passed"
  exit 0
fi
...
for manifest in \
  ...
  if [[ "${#manifest_jobs[@]}" -ne 3 ]]; then
    echo "E68 manifest $manifest has ${#manifest_jobs[@]} jobs; expected 3" >&2
    exit 1
  fi
...
python - "$IDENTITY" "$PROTOCOL" "$0" \
  ...
  "$MATHIR_MANIFEST" "$CAPACITY_PROBE" <<'PY'
import csv
import hashlib
import json
import os
...
payload = {
    "schema": "e68_separated_support_actuator_ablation_v1",
    "protocol_sha256": digest(sys.argv[2]),
    "launcher_sha256": digest(sys.argv[3]),
    "capacity_probe_sha256": digest(sys.argv[8]),
...
if [[ "${#job_ids[@]}" -ne 12 ]]; then
  echo "E68 expected 12 jobs; got ${#job_ids[@]}" >&2
  exit 1
fi
for job_id in "${job_ids[@]}"; do
  ...
  for required in \
    ...
    if [[ "$record" != *"$required"* ]]; then
      echo "E68 held-job audit failed for $job_id: missing $required" >&2
      exit 1
    fi
  done
done
...
released=1
trap - EXIT
echo "[e68] released 12 jobs: ${job_ids[*]}"
echo "[e68] identity=$IDENTITY"
