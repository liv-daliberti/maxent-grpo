#!/usr/bin/env bash
# Freeze and submit the development-only PantryPlan base-model viability gate.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/pantry_plan_05b_viability_v2_20260729.md"
EVALUATOR="$ROOT_DIR/ops/evaluate_modebench_base_viability.py"
SLURM_SCRIPT="$ROOT_DIR/ops/slurm/evaluate_pantry_plan_05b_viability.slurm"
IDENTITY="$ROOT_DIR/var/artifacts/pantry_plan_05b_viability_v2_identity.json"
RECEIPT="$ROOT_DIR/var/artifacts/pantry_plan_05b_viability_v2.json"
...
SOURCE_PARENT="$ROOT_DIR/var/artifacts/source_snapshots/pantry_plan_viability_${SOURCE_HASH}"
...
EXECUTION_INPUT="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.pantry-viability-ops.XXXXXX")"
cp "$EVALUATOR" "$EXECUTION_INPUT/evaluate_modebench_base_viability.py"
cp "$SLURM_SCRIPT" "$EXECUTION_INPUT/evaluate_pantry_plan_05b_viability.slurm"
EXECUTION_HASH="$(hash_tree "$EXECUTION_INPUT")"
EXECUTION_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/pantry_plan_viability_ops_${EXECUTION_HASH}"
...
job_id="$(
  sbatch --parsable --hold \
    ...
    "$EXECUTION_ROOT/evaluate_pantry_plan_05b_viability.slurm"
)"
job_id="${job_id%%;*}"
if [[ ! "$job_id" =~ ^[0-9]+$ ]]; then
...
payload = {
    "schema_version": "pantry-plan-05b-viability-identity-v2",
