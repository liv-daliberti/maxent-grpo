#!/usr/bin/env bash
# Launch E43's treatment-only, success-conditioned signed semantic Shannon advantage cohort.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e43_success_conditioned_signed_semantic_shannon_05b.md"
...
GRAPH_PREFIX=gce43_success_conditioned_signed_semantic_shannon_05b_v1
COUNTDOWN_PREFIX=cde43_success_conditioned_signed_semantic_shannon_05b_v1
MATH_PREFIX=mte43_math12k_384_success_conditioned_signed_semantic_shannon_05b_v1
IDENTITY="$ROOT_DIR/var/artifacts/e43_success_conditioned_signed_semantic_shannon_05b_v1_identity.json"
...
E41_IDENTITY="$ROOT_DIR/var/artifacts/e41_semantic_shannon_advantage_05b_v1_identity.json"
E41_GRAPH_MANIFEST="$ROOT_DIR/var/artifacts/gce41_semantic_shannon_advantage_05b_v1_comparative_jobs.tsv"
E41_COUNTDOWN_MANIFEST="$ROOT_DIR/var/artifacts/cde41_semantic_shannon_advantage_05b_v1_comparative_jobs.tsv"
E41_MATH_MANIFEST="$ROOT_DIR/var/artifacts/mte41_math12k_384_semantic_shannon_advantage_05b_v1_comparative_jobs.tsv"
E42_IDENTITY="$ROOT_DIR/var/artifacts/e42_quality_gated_semantic_novelty_05b_v1_identity.json"
E42_GRAPH_MANIFEST="$ROOT_DIR/var/artifacts/gce42_quality_gated_semantic_novelty_05b_v1_comparative_jobs.tsv"
E42_COUNTDOWN_MANIFEST="$ROOT_DIR/var/artifacts/cde42_quality_gated_semantic_novelty_05b_v1_comparative_jobs.tsv"
E42_MATH_MANIFEST="$ROOT_DIR/var/artifacts/mte42_math12k_384_quality_gated_semantic_novelty_05b_v1_comparative_jobs.tsv"
...
for required in \
  ...
  "$E41_IDENTITY" \
  "$E41_GRAPH_MANIFEST" \
  "$E41_COUNTDOWN_MANIFEST" \
  "$E41_MATH_MANIFEST" \
  "$E42_IDENTITY" \
  "$E42_GRAPH_MANIFEST" \
  "$E42_COUNTDOWN_MANIFEST" \
  "$E42_MATH_MANIFEST" \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing frozen E43 prerequisite: $required" >&2
    exit 1
  fi
done
if ! grep -q '^\*\*Status: FROZEN BEFORE LAUNCH' "$PROTOCOL"; then
  echo "E43 protocol is not frozen" >&2
  exit 1
fi
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "E43 MATH materialization is incomplete: $required" >&2
    exit 1
  fi
done
...
if ! grep -q 'semantic_shannon_separate_advantage' \
   ...
   ! grep -q 'OAT_ZERO_SEMANTIC_SHANNON_SUCCESS_CONDITIONED_SIGNED_ADVANTAGE' \
     "$ROOT_DIR/ops/train.sh" || \
   ! grep -q 'submit_arm success_conditioned_signed_semantic_shannon success_conditioned_signed_semantic_shannon' \
  ...
  echo "Current source/execution stack lacks the frozen E43 success-conditioned signed semantic-Shannon advantage mechanism" >&2
  exit 1
fi
...
"$PYTHON_BIN" - \
  ...
  "$E39_IDENTITY" "$E39_MATH_MANIFEST" "$MATH_DATA_IDENTITY" \
  "$E41_IDENTITY" "$E41_GRAPH_MANIFEST" "$E41_COUNTDOWN_MANIFEST" \
  "$E41_MATH_MANIFEST" "$E42_IDENTITY" "$E42_GRAPH_MANIFEST" \
  "$E42_COUNTDOWN_MANIFEST" "$E42_MATH_MANIFEST" <<'PY'
import csv
import hashlib
import json
import pathlib
...
if (
    ...
    raise SystemExit("E37 identity does not match the frozen E43 comparator")
...
if (
    ...
    raise SystemExit("E38 identity does not match the frozen E43 comparator")
...
if (
    ...
    raise SystemExit("E39 identity does not match the frozen E43 MATH comparator")
...
if data_hash != e39.get("data", {}).get("materialization_manifest_sha256"):
    raise SystemExit("E43 MATH materialization identity differs from frozen E39")

e41 = read_json(sys.argv[10])
expected_e41_modebench_evaluation = {
    "draws": 4,
    "k": 8,
    "pass_at_1": "greedy",
    "prompt": "neutral_qwen_boxed",
...
if (
    ...
    or e41.get("new_arms") != ["semantic_shannon_advantage"]
    or e41.get("seeds") != [43, 44, 45]
    or e41.get("num_samples") != 16
    or e41.get("prompt_epochs") != 10
    or e41.get("evaluation", {}).get("modebench")
    != expected_e41_modebench_evaluation
    or e41.get("evaluation", {}).get("math")
    != expected_e41_math_evaluation
    or e41.get("semantic_shannon_advantage", {}).get("coefficient") != 0.10
    or e41.get("semantic_shannon_advantage", {}).get("surprisal_clip") != 5.0
    or e41.get("semantic_shannon_advantage", {}).get("pseudocount") != 1.0
    or e41.get("semantic_shannon_advantage", {}).get("separate_advantage")
    is not True
    or e41.get("tasks", {}).get("graph_coloring", {}).get("prompt_pool") != 192
    or e41.get("tasks", {}).get("countdown", {}).get("prompt_pool") != 384
    or e41.get("tasks", {}).get("math12k_384", {}).get("prompt_pool") != 384
    or e41.get("tasks", {}).get("math12k_384", {}).get("eval_rows") != 500
    or e41.get("tasks", {})
    .get("math12k_384", {})
    .get("materialization_manifest_sha256")
    != data_hash
):
    raise SystemExit("E41 identity does not match the frozen E43 parent treatment")

expected_e41_rows = {
    ("semantic_shannon_advantage", str(seed))
    for seed in (43, 44, 45)
}
for manifest_name in sys.argv[11:14]:
    rows = read_rows(manifest_name)
    observed = {(row["arm"], row["seed"]) for row in rows}
    if len(rows) != 3 or observed != expected_e41_rows:
        raise SystemExit(
            f"E41 parent manifest is not the frozen three-run treatment: "
            f"{manifest_name}"
        )

e42 = read_json(sys.argv[14])
expected_e42_modebench_evaluation = {
    **expected_e41_modebench_evaluation,
    "step_zero_pairing_to_e41": "exact",
}
expected_e42_math_evaluation = {
    **expected_e41_math_evaluation,
    "step_zero_pairing_to_e41": "exact",
}
if (
    e42.get("schema") != "e42_quality_gated_semantic_novelty_05b_v1"
    or e42.get("model") != e41.get("model")
    or e42.get("new_arms") != ["quality_gated_semantic_novelty"]
    or e42.get("seeds") != [43, 44, 45]
    or e42.get("num_samples") != 16
    or e42.get("prompt_epochs") != 10
    or e42.get("evaluation", {}).get("modebench")
    != expected_e42_modebench_evaluation
    or e42.get("evaluation", {}).get("math")
    != expected_e42_math_evaluation
    or e42.get("tasks", {}).get("graph_coloring", {}).get("prompt_pool") != 192
    or e42.get("tasks", {}).get("countdown", {}).get("prompt_pool") != 384
    or e42.get("tasks", {}).get("math12k_384", {}).get("prompt_pool") != 384
    or e42.get("tasks", {}).get("math12k_384", {}).get("eval_rows") != 500
    or e42.get("tasks", {})
    .get("math12k_384", {})
    .get("materialization_manifest_sha256")
    != data_hash
    or e42.get("quality_gated_semantic_novelty", {}).get("coefficient")
    != 0.10
    or e42.get("quality_gated_semantic_novelty", {}).get("surprisal_clip")
    != 5.0
    or e42.get("quality_gated_semantic_novelty", {}).get("pseudocount")
    != 1.0
    or e42.get("quality_gated_semantic_novelty", {}).get(
        "separate_advantage"
    )
    is not True
    or e42.get("quality_gated_semantic_novelty", {}).get(
    ...
    raise SystemExit("E42 identity does not match the frozen E43 parent treatment")

expected_e42_rows = {
    ("quality_gated_semantic_novelty", str(seed))
    for seed in (43, 44, 45)
}
for manifest_name in sys.argv[15:18]:
    rows = read_rows(manifest_name)
    observed = {(row["arm"], row["seed"]) for row in rows}
    if len(rows) != 3 or observed != expected_e42_rows:
        raise SystemExit(
            f"E42 parent manifest is not the frozen three-run treatment: "
            f"{manifest_name}"
        )
PY
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX" "$MATH_PREFIX"; do
  ...
  if [[ "$phase" == full && -e "$manifest" ]]; then
    echo "Fresh E43 prefix required; manifest already exists: $manifest" >&2
    exit 1
  fi
done
if [[ "$phase" == full && -e "$IDENTITY" ]]; then
  echo "Fresh E43 identity required; artifact already exists: $IDENTITY" >&2
  exit 1
fi
...
freeze_execution() {
  ...
  SOURCE_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e43_success_conditioned_signed_semantic_shannon_05b_${SOURCE_HASH}/src"
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E43 source snapshot hash mismatch" >&2
    exit 1
  fi

  local ops_staging ops_input
  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e43-ops-input.XXXXXX")"
  ...
  OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e43_success_conditioned_signed_semantic_shannon_05b_ops_${EXECUTION_HASH}/ops"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E43 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
export OAT_ZERO_ONLY_ARMS=success_conditioned_signed_semantic_shannon
...
export OAT_ZERO_INCLUDE_SUCCESS_CONDITIONED_SIGNED_SEMANTIC_SHANNON_ARM=1
...
MODEBENCH_NODELIST="${OAT_ZERO_E43_MODEBENCH_TRAIN_NODELIST:-node105,node202,node203,node204}"
MODEBENCH_GRES="${OAT_ZERO_E43_MODEBENCH_TRAIN_GRES:-gpu:a5000:1}"
MODEBENCH_CPUS="${OAT_ZERO_E43_MODEBENCH_TRAIN_CPUS_PER_TASK:-4}"
MODEBENCH_MEMORY="${OAT_ZERO_E43_MODEBENCH_TRAIN_MEMORY:-32G}"
MATH_NODELIST="${OAT_ZERO_E43_MATH_TRAIN_NODELIST:-node105,node202,node203,node204}"
MATH_GRES="${OAT_ZERO_E43_MATH_TRAIN_GRES:-gpu:a5000:1}"
MATH_CPUS="${OAT_ZERO_E43_MATH_TRAIN_CPUS_PER_TASK:-8}"
MATH_MEMORY="${OAT_ZERO_E43_MATH_TRAIN_MEMORY:-64G}"
TRAIN_PARTITION="${OAT_ZERO_E43_TRAIN_PARTITION:-all}"
TRAIN_ACCOUNT="${OAT_ZERO_E43_TRAIN_ACCOUNT:-mltheory}"
TRAIN_TIME_LIMIT="${OAT_ZERO_E43_TRAIN_TIME_LIMIT:-24:00:00}"
...
write_identity() {
  ...
  local e41_identity_hash e41_graph_manifest_hash
  local e41_countdown_manifest_hash e41_math_manifest_hash
  local e42_identity_hash e42_graph_manifest_hash
  local e42_countdown_manifest_hash e42_math_manifest_hash math_data_hash
  ...
  e41_identity_hash="$(sha256sum "$E41_IDENTITY" | cut -d' ' -f1)"
  e41_graph_manifest_hash="$(sha256sum "$E41_GRAPH_MANIFEST" | cut -d' ' -f1)"
  e41_countdown_manifest_hash="$(sha256sum "$E41_COUNTDOWN_MANIFEST" | cut -d' ' -f1)"
  e41_math_manifest_hash="$(sha256sum "$E41_MATH_MANIFEST" | cut -d' ' -f1)"
  e42_identity_hash="$(sha256sum "$E42_IDENTITY" | cut -d' ' -f1)"
  e42_graph_manifest_hash="$(sha256sum "$E42_GRAPH_MANIFEST" | cut -d' ' -f1)"
  e42_countdown_manifest_hash="$(sha256sum "$E42_COUNTDOWN_MANIFEST" | cut -d' ' -f1)"
  e42_math_manifest_hash="$(sha256sum "$E42_MATH_MANIFEST" | cut -d' ' -f1)"
  ...
  "$PYTHON_BIN" - "$IDENTITY" "$protocol_hash" "$launcher_hash" \
    ...
    "$e39_identity_hash" "$e39_manifest_hash" "$math_data_hash" \
    "$e41_identity_hash" "$e41_graph_manifest_hash" \
    "$e41_countdown_manifest_hash" "$e41_math_manifest_hash" \
    "$e42_identity_hash" "$e42_graph_manifest_hash" \
    "$e42_countdown_manifest_hash" "$e42_math_manifest_hash" <<'PY'
import json
import os
import pathlib
import sys
...
payload = {
    "schema": "e43_success_conditioned_signed_semantic_shannon_05b_v1",
    ...
    "comparators": {
        ...
        "parent_e41": {
            "experiment": "E41",
            "identity": "e41_semantic_shannon_advantage_05b_v1_identity.json",
            "identity_sha256": sys.argv[15],
            "graph_prefix": "gce41_semantic_shannon_advantage_05b_v1",
            "graph_manifest_sha256": sys.argv[16],
            "countdown_prefix": "cde41_semantic_shannon_advantage_05b_v1",
            "countdown_manifest_sha256": sys.argv[17],
            "math_prefix": "mte41_math12k_384_semantic_shannon_advantage_05b_v1",
            "math_manifest_sha256": sys.argv[18],
            "arms": ["semantic_shannon_advantage"],
        },
        "parent_e42": {
            "experiment": "E42",
            "identity": "e42_quality_gated_semantic_novelty_05b_v1_identity.json",
            "identity_sha256": sys.argv[19],
            "graph_prefix": "gce42_quality_gated_semantic_novelty_05b_v1",
            "graph_manifest_sha256": sys.argv[20],
            "countdown_prefix": "cde42_quality_gated_semantic_novelty_05b_v1",
            "countdown_manifest_sha256": sys.argv[21],
            "math_prefix": "mte42_math12k_384_quality_gated_semantic_novelty_05b_v1",
            "math_manifest_sha256": sys.argv[22],
            "arms": ["quality_gated_semantic_novelty"],
        },
    },
    "model": (
        "Qwen2.5-0.5B-Instruct@"
    ...
    "tasks": {
        "graph_coloring": {
            "prompt_pool": 192,
            "prefix": "gce43_success_conditioned_signed_semantic_shannon_05b_v1",
        },
        "countdown": {
            "prompt_pool": 384,
        ...
        "math12k_384": {
            "prompt_pool": 384,
            "eval_rows": 500,
            "prefix": "mte43_math12k_384_success_conditioned_signed_semantic_shannon_05b_v1",
            "materialization_manifest_sha256": sys.argv[14],
        },
    },
    "new_arms": ["success_conditioned_signed_semantic_shannon"],
    "success_conditioned_signed_semantic_shannon": {
        ...
        "raw_advantage": (
            "(0.10/5.0)*(realized_clipped_surprise-"
            "predictive_clipped_surprise_expectation)"
        ),
    ...
    "evaluation": {
        "modebench": {
            ...
            "step_zero_pairing_to_e37_e38": "exact",
            "step_zero_pairing_to_e41": "exact",
            "step_zero_pairing_to_e42": "exact",
        },
        "math": {
            "prompt": "neutral_qwen_math",
            "benchmark": "full_MATH_500",
...
if [[ "$phase" == config ]]; then
  ...
  submit_task math
  echo "[e43] all three treatment-only configurations passed; no jobs submitted"
  exit 0
fi
...
cleanup_partial_cohort() {
  ...
  if [[ "$COHORT_RELEASED" != "1" ]]; then
    ...
    if [[ "${#cleanup_ids[@]}" -gt 0 ]]; then
      ...
      echo "[e43] cancelled incomplete held cohort: ${cleanup_ids[*]}" >&2
    fi
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX" "$MATH_PREFIX"; do
  ...
  if [[ "${#task_jobs[@]}" -ne "$EXPECTED_JOBS_PER_TASK" ]]; then
    echo "E43 ${prefix} has ${#task_jobs[@]} jobs; expected $EXPECTED_JOBS_PER_TASK" >&2
    exit 1
  fi
  job_ids+=("${task_jobs[@]}")
done
if [[ "${#job_ids[@]}" -ne "$EXPECTED_JOBS" ]]; then
  echo "E43 cohort has ${#job_ids[@]} jobs; expected $EXPECTED_JOBS" >&2
  exit 1
fi
...
for manifest_name in sys.argv[1:]:
    ...
    if len(rows) != 3 or observed != expected:
        raise SystemExit(
            f"E43 held manifest mismatch: {manifest_name} "
...
for job_id in "${job_ids[@]}"; do
  if ! scontrol update JobId="$job_id" Partition="$TRAIN_PARTITION"; then
    echo "E43 could not normalize held job $job_id to $TRAIN_PARTITION" >&2
    exit 1
  fi
done
  ...
  for job_id in "${task_jobs[@]}"; do
    ...
    for required in \
      ...
      if [[ "$job_record" != *"$required"* ]]; then
        echo "E43 held-job audit failed for $job_id: missing $required" >&2
        exit 1
      fi
    done
    if [[ "$arm" != success_conditioned_signed_semantic_shannon ]]; then
      echo "E43 manifest has unexpected arm for $job_id: $arm" >&2
      exit 1
    fi
    if [[ "$job_record" != *"Partition=${TRAIN_PARTITION}"* ]] || \
      ...
      echo "E43 held job $job_id does not attest the frozen placement" >&2
      exit 1
    fi
  done
done
...
COHORT_RELEASED=1
trap - EXIT
echo "[e43] released ${#job_ids[@]} success-conditioned signed semantic Shannon advantage jobs: ${job_ids[*]}"
echo "[e43] identity=$IDENTITY math_data=$MATH_DATA_IDENTITY"
