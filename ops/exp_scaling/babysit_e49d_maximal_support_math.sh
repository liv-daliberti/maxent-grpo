#!/usr/bin/env bash
# Durable E49D orchestration: judge -> menus -> matched toy -> gate -> full.
set -euo pipefail
...
MENU_JOB_LAUNCHER="$ROOT_DIR/ops/math_strategy_calibration/launch_e49d_menu_job.sh"
LAUNCHER="$ROOT_DIR/ops/exp_scaling/launch_e49d_maximal_support_math_05b.sh"
ANALYZER="$ROOT_DIR/ops/exp_scaling/analyze_e49d_maximal_support_math.py"
AUDIT_JOB_LAUNCHER="$ROOT_DIR/ops/exp_scaling/launch_e49d_eval_audit_job.sh"
TOY_SOURCE="$ROOT_DIR/var/data/e49b_math_strategy_toy"
TOY_DATA="$ROOT_DIR/var/data/e49d_math_strategy_menu_toy"
TOY_EVIDENCE="$ROOT_DIR/var/artifacts/e49d_math_strategy_menu_toy_v1"
FULL_SOURCE="$ROOT_DIR/var/data/math12k_384_math500"
FULL_DATA="$ROOT_DIR/var/data/e49d_math_strategy_menu_full"
FULL_EVIDENCE="$ROOT_DIR/var/artifacts/e49d_math_strategy_menu_full_v1"
TOY_MANIFEST="$ROOT_DIR/var/artifacts/e49d_maximal_support_math_toy_05b_v1_comparative_jobs.tsv"
FULL_MANIFEST="$ROOT_DIR/var/artifacts/e49d_maximal_support_math_full_05b_v1_comparative_jobs.tsv"
TOY_DECISION="$ROOT_DIR/var/artifacts/e49d_maximal_support_math_toy_advancement.json"
FULL_REPORT="$ROOT_DIR/var/artifacts/e49d_maximal_support_math_full_report.json"
STATUS="$ROOT_DIR/var/artifacts/e49d_babysitter_status.json"
TOY_MONITOR_PID="$ROOT_DIR/var/artifacts/e49d_toy_monitor.pid"
FULL_MONITOR_PID="$ROOT_DIR/var/artifacts/e49d_full_monitor.pid"
TOY_MENU_JOB="$TOY_EVIDENCE/materialization_job.json"
FULL_MENU_JOB="$FULL_EVIDENCE/materialization_job.json"
TOY_AUDIT="$ROOT_DIR/var/artifacts/e49d_toy_terminal_execution_audit.json"
FULL_AUDIT="$ROOT_DIR/var/artifacts/e49d_full_terminal_execution_audit.json"
TOY_AUDIT_JOB="$ROOT_DIR/var/artifacts/e49d_toy_terminal_execution_audit_job.json"
FULL_AUDIT_JOB="$ROOT_DIR/var/artifacts/e49d_full_terminal_execution_audit_job.json"
...
payload = {
    "schema": "e49d_babysitter_status_v1",
    "phase": sys.argv[2],
    "detail": sys.argv[3],
...
start_monitor() {
  ...
  bash "$ROOT_DIR/ops/exp_scaling/monitor_e49d_maximal_support_math.sh" \
    "$stage" >/dev/null 2>&1 &
  local monitor_pid=$!
...
archive_finished_record() {
  ...
  if ! state="$(
    ...
    local accounted_state
    if ! accounted_state="$(
      timeout 20 sacct -n -X -j "$job_id" \
        --format=State --parsable2 2>/dev/null
    )"; then
      return 1
    fi
    accounted_state="$(
      printf '%s\n' "$accounted_state" \
        | sed -n 's/[[:space:]]//g; /^$/d; 1p'
    )"
    case "$accounted_state" in
...
"$PYTHON_BIN" "$ROOT_DIR/ops/exp_scaling/audit_e49d_prompt_lengths.py" \
  --data "$TOY_DATA" \
  --output "$ROOT_DIR/var/artifacts/e49d_toy_prompt_length_audit.json" \
...
"$PYTHON_BIN" "$ROOT_DIR/ops/exp_scaling/audit_e49d_prompt_lengths.py" \
  --data "$FULL_DATA" \
  --output "$ROOT_DIR/var/artifacts/e49d_full_prompt_length_audit.json" \
  --max-prompt-tokens 2048 >/dev/null
...
status complete "E49D full-stage report passed"
