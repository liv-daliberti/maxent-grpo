#!/usr/bin/env python3
"""Gate E49D's matched finite-action MATH toy and full stages."""
...
def _eval_audit(stage: str) -> tuple[dict[str, Any] | None, str | None]:
    output = (
        ROOT
        / "var/artifacts"
        / f"e49d_{stage}_terminal_execution_audit.json"
    )
...
def _terminal_audit_metrics(
    ...
    return {
        ...
        "eligible_audited_distinct8": float(
            sampled[
                "eligible_audited_distinct_correct_strategies_mean"
            ]
        ),
        "eligible_audited_support2_fraction": float(
            sampled[
                "eligible_audited_support_at_least_two_prompt_fraction"
            ]
        ),
        "multi_route_eligible_prompt_fraction": float(
...
def analyze(stage: str, output: pathlib.Path) -> dict[str, Any]:
    ...
    if not all(complete.values()):
        report = {
            "schema": "e49d_maximal_support_math_stage_report_v1",
            "stage": stage,
            "prefix": prefix,
    ...
    mechanism_checks = {
        ...
        "double_certified_multi_route_eval_fraction_at_least_twenty_percent": (
            audit_metrics["grpo"]["multi_route_eligible_prompt_fraction"]
            >= 0.20 - 1e-12
            and abs(
                audit_metrics["grpo"][
                    "multi_route_eligible_prompt_fraction"
                ]
        ...
        "treatment_terminal_discovered_multiple_training_routes": (
            support2_fraction is not None and support2_fraction > 0
        ),
    ...
    coverage_margin = 0.10 if stage == "toy" else 0.05
    quality_checks[
        f"treatment_eligible_eval_support2_plus_{int(coverage_margin * 100)}_points"
    ] = (
        audit_metrics["online_canonical_haarnoja"][
            "eligible_audited_support2_fraction"
        ]
        >= audit_metrics["grpo"]["eligible_audited_support2_fraction"]
        + coverage_margin
        - 1e-12
    )
    if stage == "toy":
        quality_checks[
            "treatment_eligible_distinct_strategy_mean_plus_point_two"
        ] = (
            audit_metrics["online_canonical_haarnoja"][
                "eligible_audited_distinct8"
            ]
            >= audit_metrics["grpo"]["eligible_audited_distinct8"]
            + 0.20
            - 1e-12
        )
