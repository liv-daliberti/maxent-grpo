#!/usr/bin/env bash
# Launch E51's fresh three-domain matched policy-entropy canonical cohort.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e51_policy_entropy_adaptive_canonical_05b_50ep.md"
...
GRAPH_PREFIX=gce51_policy_entropy_adaptive_canonical_05b_50ep_v2
COUNTDOWN_PREFIX=cde51_policy_entropy_adaptive_canonical_05b_50ep_v2_allcs
PYTHON_PREFIX=pye51_policy_entropy_adaptive_canonical_05b_50ep_v2_allcs
IDENTITY="$ROOT_DIR/var/artifacts/e51_policy_entropy_adaptive_canonical_05b_50ep_v2_identity.json"
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing frozen E51 prerequisite: $required" >&2
    exit 1
  fi
done
if ! grep -q '^\*\*Status: FROZEN BEFORE CORRECTED RELAUNCH' "$PROTOCOL"; then
  echo "E51 protocol is not frozen" >&2
  exit 1
fi
for contract in \
  'src/oat_drgrpo/online_canonical_controller.py:unprojected_warmup_inverse_policy_entropy_alpha_v2' \
  'src/oat_drgrpo/learner/run.py:online_canonical_policy_entropy_observed' \
  'src/oat_drgrpo/learner/grpo.py:verified_discovery_cumulative_outcomes' \
  'src/oat_drgrpo/args.py:online_canonical_policy_entropy_adaptation' \
  'ops/run_experiment.sh:online_canonical_policy_entropy)' \
  'ops/submit_countdown_comparative.sh:INCLUDE_ONLINE_CANONICAL_POLICY_ENTROPY_ARM'; do
  path="${contract%%:*}"
  needle="${contract#*:}"
  if ! grep -q "$needle" "$ROOT_DIR/$path"; then
    echo "Current source lacks E51 contract: $path :: $needle" >&2
    exit 1
  fi
done
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX" "$PYTHON_PREFIX"; do
  ...
  if [[ "$phase" == full && -e "$manifest" ]]; then
    echo "Fresh E51 prefix required; manifest exists: $manifest" >&2
    exit 1
  fi
done
if [[ "$phase" == full && -e "$IDENTITY" ]]; then
  echo "Fresh E51 identity required; file exists: $IDENTITY" >&2
  exit 1
fi
...
freeze_execution() {
  ...
  SOURCE_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e51_policy_entropy_adaptive_canonical_${SOURCE_HASH}/src"
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E51 source snapshot hash mismatch" >&2
    exit 1
  fi

  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e51-ops-input.XXXXXX")"
  ...
  OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e51_policy_entropy_adaptive_canonical_ops_${EXECUTION_HASH}/ops"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E51 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
export OAT_ZERO_ONLY_ARMS=grpo,online_canonical_policy_entropy
...
for flag in \
  ...
  ONLINE_CANONICAL_HAARNOJA; do
  export "OAT_ZERO_INCLUDE_${flag}_ARM=0"
done
export OAT_ZERO_INCLUDE_ONLINE_CANONICAL_POLICY_ENTROPY_ARM=1
...
export OAT_ZERO_ONLINE_CANONICAL_POLICY_ENTROPY_WARMUP_STEPS=64
export OAT_ZERO_ONLINE_CANONICAL_POLICY_ENTROPY_EMA_DECAY=0.90
...
payload = {
    "schema": "e51_policy_entropy_adaptive_canonical_05b_50ep_v2",
    ...
    "arms": ["grpo", "online_canonical_policy_entropy"],
    "seeds": [43, 44, 45],
    "prompt_epochs": 50,
    "num_samples": 16,
    "policy_entropy_controller": {
        "sensor": "train/entropy",
        "units": "policy_token_entropy_nats_masked_mean_v1",
        "rule": "unprojected_warmup_inverse_policy_entropy_alpha_v2",
...
submit_domain() {
  ...
  case "$domain" in
    graph_coloring)
      ...
      export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E51_TRAIN_NODELIST:-node302}"
      export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E51_TRAIN_GRES:-gpu:a100:1}"
      export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E51_TRAIN_PARTITION:-all}"
      export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E51_TRAIN_ACCOUNT:-mltheory}"
      ;;
    countdown)
      ...
      export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E51_COUNTDOWN_TRAIN_NODELIST:-node020,node022,node023,node024,node026}"
      export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E51_COUNTDOWN_TRAIN_GRES:-gpu:rtx_3090:1}"
      export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E51_COUNTDOWN_TRAIN_PARTITION:-lowprio}"
      export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E51_COUNTDOWN_TRAIN_ACCOUNT:-allcs}"
      ;;
    python_factor)
      ...
      export OAT_ZERO_RESUME_STEPS=384
      export OAT_ZERO_RESUME_FROM=384
      export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E51_PYTHON_TRAIN_NODELIST:-node020,node022,node023,node024,node026}"
      export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E51_PYTHON_TRAIN_GRES:-gpu:rtx_3090:1}"
      export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E51_PYTHON_TRAIN_PARTITION:-lowprio}"
      export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E51_PYTHON_TRAIN_ACCOUNT:-allcs}"
      ;;
  esac
...
if [[ "$phase" == config ]]; then
  ...
  echo "[e51] all three matched configurations passed; no jobs submitted"
  exit 0
fi
...
cleanup_partial_cohort() {
  ...
  if [[ "$COHORT_RELEASED" != "1" && "${#job_ids[@]}" -gt 0 ]]; then
    ...
    echo "[e51] cancelled incomplete held cohort: ${job_ids[*]}" >&2
  fi
  exit "$status"
}
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX" "$PYTHON_PREFIX"; do
  ...
  if [[ "${#task_jobs[@]}" -ne "$EXPECTED_JOBS_PER_DOMAIN" ]]; then
    echo "E51 ${prefix} has ${#task_jobs[@]} jobs; expected 6" >&2
    exit 1
  fi
  job_ids+=("${task_jobs[@]}")
...
expected = {
    (arm, str(seed))
    for arm in ("grpo", "online_canonical_policy_entropy")
    for seed in (43, 44, 45)
}
observed = {(row["arm"], row["seed"]) for row in rows}
if len(rows) != 6 or observed != expected:
    raise SystemExit(f"E51 held manifest mismatch: {observed!r}")
PY
done
if [[ "${#job_ids[@]}" -ne "$EXPECTED_JOBS" ]]; then
  echo "E51 cohort has ${#job_ids[@]} jobs; expected $EXPECTED_JOBS" >&2
  exit 1
fi
...
for job_id in "${job_ids[@]}"; do
  ...
  for required in \
    ...
    if [[ "$job_record" != *"$required"* ]]; then
      echo "E51 held-job audit failed for $job_id: missing $required" >&2
      exit 1
    fi
  done
  if [[ "$job_record" == *"OAT_ZERO_VARIANT=grpo"* ]]; then
    ...
    for required in \
      ...
      if [[ "$job_record" != *"$required"* ]]; then
        echo "E51 treatment audit failed for $job_id: missing $required" >&2
        exit 1
      fi
    done
  fi
...
COHORT_RELEASED=1
trap - EXIT
echo "[e51] released ${#job_ids[@]} matched jobs: ${job_ids[*]}"
echo "[e51] identity=$IDENTITY"
