#!/usr/bin/env bash
# Freeze and submit the AntMaze v9 fresh-map route admission gate.
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
source "$ROOT_DIR/ops/repo_env.sh"
PYTHON_BIN="$ROOT_DIR/var/seed_paper_eval/paper310/bin/python"
PROTOCOL="$ROOT_DIR/paper/preregistration/ant_maze_route_admission_v9_20260729.md"
SLURM_SCRIPT="$ROOT_DIR/ops/slurm/admit_ant_maze_modebench_v9.slurm"
TRAINING_IDENTITY="$ROOT_DIR/var/artifacts/ant_waypoint_controller_v9_identity.json"
IDENTITY="$ROOT_DIR/var/artifacts/ant_maze_modebench_v9_generation_identity.json"
...
FILES=(
  make_ant_maze_mode_data_v9.py
  make_ant_maze_mode_data.py
  audit_ant_maze_mode_data_v9.py
...
for required in "$PYTHON_BIN" "$PROTOCOL" "$SLURM_SCRIPT" \
  ...
  [[ -e "$required" ]] || {
    echo "Missing AntMaze v9 route prerequisite: $required" >&2
    exit 1
  }
done
for file in "${FILES[@]}"; do
  [[ -f "$ROOT_DIR/ops/$file" ]] || {
    echo "Missing AntMaze v9 route script: $file" >&2
    exit 1
  }
done
...
if [[ "$phase" == config ]]; then
  "$PYTHON_BIN" -m py_compile \
    ...
    "$ROOT_DIR/ops/make_ant_maze_mode_data_v9.py" \
    "$ROOT_DIR/ops/audit_ant_maze_mode_data.py" \
    "$ROOT_DIR/ops/audit_ant_maze_mode_data_v9.py" \
    "$ROOT_DIR/src/oat_drgrpo/ant_maze_worker_v9.py" \
  ...
  echo "[antmaze-v9-route] configuration passed; no v9 route map executed"
  exit 0
fi

for required in "$RECEIPT" "$MODEL"; do
  [[ -f "$required" ]] || {
    echo "Missing admitted AntMaze v9 controller artifact: $required" >&2
    exit 1
  }
done
...
if receipt.get("status") != "pass" or receipt.get("decision") != "admitted_to_fresh_maze_route_gate_v9":
    raise SystemExit("AntMaze v9 route gate requires the exact admitted controller")
if receipt.get("seed") != 73009 or receipt.get("timesteps") != 5_000_000:
    raise SystemExit("AntMaze v9 controller receipt differs from the frozen run")
checks=receipt.get("checks",{})
if not checks or not all(checks.values()):
    raise SystemExit("AntMaze v9 controller receipt has a failed or absent check")
if receipt.get("hashes",{}).get("model_sha256") != digest(sys.argv[2]):
    raise SystemExit("AntMaze v9 receipt does not bind the controller model")
if identity.get("job_id") != 30192731 or identity.get("seed") != 73009:
    raise SystemExit("AntMaze v9 training identity differs from the frozen run")
PYGATE

for fresh in "$IDENTITY" "$DATA_ROOT" "$AUDIT"; do
  if [[ -e "$fresh" ]]; then
    echo "Fresh AntMaze v9 route target required: $fresh" >&2
    exit 1
  fi
done

SOURCE_HASH="$(hash_tree "$ROOT_DIR/src")"
SOURCE_PARENT="$ROOT_DIR/var/artifacts/source_snapshots/ant_maze_v9_route_${SOURCE_HASH}"
...
OPS_INPUT="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.ant-maze-v9-route.XXXXXX")"
for file in "${FILES[@]}"; do
  cp "$ROOT_DIR/ops/$file" "$OPS_INPUT/$file"
done
cp "$SLURM_SCRIPT" "$OPS_INPUT/admit_ant_maze_modebench_v9.slurm"
OPS_HASH="$(hash_tree "$OPS_INPUT")"
OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/ant_maze_v9_route_ops_${OPS_HASH}"
if [[ ! -f "$OPS_ROOT/make_ant_maze_mode_data_v9.py" ]]; then
...
job_id="$(
  sbatch --parsable --hold \
    ...
    "$OPS_ROOT/admit_ant_maze_modebench_v9.slurm"
)"
job_id="${job_id%%;*}"
[[ "$job_id" =~ ^[0-9]+$ ]] || {
  echo "Invalid AntMaze v9 route job ID: $job_id" >&2
  exit 1
}
...
for required in "JobState=PENDING" "Reason=JobHeldUser" \
  ...
  [[ "$job_record" == *"$required"* ]] || {
    echo "Held v9 route job missing $required" >&2
    exit 1
  }
done
...
path=pathlib.Path(sys.argv[1]); payload={
 "schema_version":"ant-maze-v9-route-generation-identity-v1",
 ...
 "controller_training_job_id":30192731,"map_count":12,"map_size":9,
 "reset_seed_base":97300,"real_route_executions":48,
...
trap - EXIT
echo "[antmaze-v9-route] released admission job $job_id"
echo "[antmaze-v9-route] identity=$IDENTITY"
