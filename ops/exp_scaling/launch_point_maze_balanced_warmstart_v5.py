#!/usr/bin/env python3
"""Configure or launch the frozen orientation-balanced PointMaze v5 warm start."""
...
QUALIFIER = ROOT / "ops/qualify_point_maze_balanced_warmstart_v5.py"
BATCH = ROOT / "ops/slurm/train_point_maze_balanced_warmstart_v5.slurm"
...
MODEL = ROOT / "var/models/point_maze_interactive_warmstart_v5_balanced"
SFT_RECEIPT = ROOT / "var/artifacts/point_maze_balanced_warmstart_sft_v5.json"
VIABILITY = ROOT / "var/artifacts/point_maze_balanced_warmstart_v5_viability.json"
QUALIFICATION = (
...
SUBMISSION = (
    ROOT / "var/artifacts/point_maze_balanced_warmstart_v5_submission.json"
)
V3_ADMISSION = (
...
def validate() -> None:
    ...
    run(
        [
            ...
            "pytest",
            "-q",
            str(ROOT / "tests/test_point_maze_balanced_warmstart_v5.py"),
...
def validate_materialized() -> None:
    ...
    if (
        ...
        raise RuntimeError("PointMaze v5 balanced source identity drift")
    task_fingerprints = {
    ...
    if len(task_fingerprints) != 8:
        raise RuntimeError("PointMaze v5 train executable identity collision")
    if (
        ...
        raise RuntimeError("PointMaze v5 replayed warm-start identity drift")
...
def snapshot_source() -> tuple[Path, str]:
    ...
    parent = (
        ...
        f"point_balanced_warmstart_v5_source_{digest}"
    )
    target = parent / "src"
    ...
    if tree_hash(target) != digest:
        raise RuntimeError("PointMaze v5 source snapshot mismatch")
...
def snapshot_execution() -> tuple[Path, str]:
    staging = Path(
        tempfile.mkdtemp(
            prefix=".point-balanced-warmstart-v5.",
    ...
    target = (
        ROOT
        / "var/artifacts/source_snapshots/"
        f"point_balanced_warmstart_v5_ops_{digest}"
    )
    if target.exists():
    ...
    if tree_hash(target) != digest:
        raise RuntimeError("PointMaze v5 execution snapshot mismatch")
    return target, digest
...
def main() -> None:
    ...
    if args.phase == "config":
        print(
            "[point-balanced-warmstart-v5] configuration passed; "
    ...
    for path in (MODEL, SFT_RECEIPT, VIABILITY, QUALIFICATION, IDENTITY, SUBMISSION):
        if path.exists():
            raise FileExistsError(f"fresh PointMaze v5 artifact required: {path}")
    ...
    try:
        atomic(
            IDENTITY,
            {
                "schema_version": "point-maze-balanced-warmstart-v5-identity-v1",
                "job_id": job_id,
                ...
                "sft_seed": 76601,
                "development_seed": 76602,
                "sft_optimizer_steps": 276,
        ...
        for required in (
            ...
            if required not in record:
                raise RuntimeError(f"held PointMaze v5 job missing {required}")
        atomic(
            SUBMISSION,
            {
                "schema_version": "point-maze-balanced-warmstart-v5-submission-v1",
                "job_id": job_id,
        ...
        raise
    print(f"[point-balanced-warmstart-v5] released job {job_id}")
