#!/usr/bin/env python3
"""Audit terminal E49D eval responses under the exact execution contract."""
...
def _run_dir(prefix: str, arm: str) -> pathlib.Path:
    ...
    if len(complete) != 1:
        raise RuntimeError(
            f"expected one complete E49D {arm} run, found {len(complete)}"
        )
    return complete[0]
...
def _endpoint(
    ...
    if any(record.get(key) != value for key, value in expected.items()):
        raise RuntimeError("unexpected E49D judge endpoint")
...
def main() -> None:
    ...
    if args.output.exists():
        ...
        if existing.get("identity") != identity:
            raise RuntimeError("existing E49D eval audit identity drift")
        print(args.output)
        return
    ...
    report = {
        "schema": "e49d_terminal_execution_gated_eval_v1",
        "identity": identity,
