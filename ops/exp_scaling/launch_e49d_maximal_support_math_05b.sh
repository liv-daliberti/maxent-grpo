#!/usr/bin/env bash
# Launch E49D's matched finite-action MATH toy or full cohort.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e49d_maximal_support_math_haarnoja_05b.md"
FROZEN_SOURCE_HASH="27386bb32552105c2a073a1dfa3e3b3b31839eb78225bec1dbc05daebbdfabcd"
ENDPOINT_RECORD="${E49D_QWEN72_ENDPOINT_RECORD:-$ROOT_DIR/var/artifacts/e49c_math_strategy_qwen72_v1/qwen72_endpoint.json}"
...
TOY_ROOT="$ROOT_DIR/var/data/e49d_math_strategy_menu_toy"
FULL_ROOT="$ROOT_DIR/var/data/e49d_math_strategy_menu_full"
TOY_EVIDENCE="$ROOT_DIR/var/artifacts/e49d_math_strategy_menu_toy_v1"
FULL_EVIDENCE="$ROOT_DIR/var/artifacts/e49d_math_strategy_menu_full_v1"
MATERIALIZER="$ROOT_DIR/ops/math_strategy_calibration/materialize_e49d_strategy_menu_data.py"

if [[ ! -x "$PYTHON_BIN" ]] \
  || [[ ! -f "$PROTOCOL" ]] \
  || [[ ! -f "$MODEL_ROOT/model.safetensors" ]] \
  || ! grep -q '^\*\*Status: FROZEN BEFORE TRAINING; PREPROCESSING THROUGHPUT' "$PROTOCOL"; then
  echo "E49D frozen prerequisite is missing" >&2
  exit 1
fi
...
if [[ "$stage" == toy ]]; then
  ...
  EVAL_INTERVAL=50
  SAVE_INTERVAL=50
  PREFIX=e49d_maximal_support_math_toy_05b_v1
  ...
  EVAL_INTERVAL=384
  SAVE_INTERVAL=384
  PREFIX=e49d_maximal_support_math_full_05b_v1
else
  DATA_ROOT="$TOY_ROOT"
  EVIDENCE_ROOT="$TOY_EVIDENCE"
  SOURCE_DATA="$ROOT_DIR/var/data/e49b_math_strategy_toy"
  ...
  EVAL_INTERVAL=50
  SAVE_INTERVAL=50
  PREFIX=e49d_maximal_support_math_config_05b_v1
fi
...
if [[ "$stage" == full ]]; then
  TOY_DECISION="$ROOT_DIR/var/artifacts/e49d_maximal_support_math_toy_advancement.json"
  if [[ ! -f "$TOY_DECISION" ]] \
    || [[ "$("$PYTHON_BIN" -c 'import json,sys; print(json.load(open(sys.argv[1])).get("advance_to_full",False))' "$TOY_DECISION")" != True ]]; then
    echo "E49D hard-toy advancement gate has not passed" >&2
    exit 1
  fi
fi
...
if [[ "$stage" != config ]] \
  ...
  echo "E49D menu data artifact is incomplete: $DATA_ROOT" >&2
  exit 1
fi
...
if [[ "$stage" == config ]]; then
  ...
  read -r EXPECTED_TRAIN_HASH EXPECTED_EVAL_HASH EXPECTED_MENU_COUNT EXPECTED_MULTI_COUNT <<<"$(
    "$PYTHON_BIN" - "$DATA_ROOT/MATERIALIZATION_MANIFEST.json" <<'PY'
import json
import sys
manifest = json.load(open(sys.argv[1], encoding="utf-8"))
if (
    manifest.get("schema") != "e49d_strategy_menu_materialization_v1"
    or manifest.get("all_retained_strategies_double_audited") is not True
):
    raise SystemExit("invalid E49D menu manifest")
print(
    manifest["train_tree_sha256"],
    manifest["eval_tree_sha256"],
    manifest["menu_count"],
    manifest["multi_strategy_menu_count"],
)
PY
  )"
  if [[ "$TRAIN_DATA_HASH" != "$EXPECTED_TRAIN_HASH" ]] \
    || [[ "$EVAL_DATA_HASH" != "$EXPECTED_EVAL_HASH" ]]; then
    echo "E49D menu data identity mismatch" >&2
    exit 1
  fi
  EXPECTED_ROWS=100
  [[ "$stage" == full ]] && EXPECTED_ROWS=884
  if [[ "$EXPECTED_MENU_COUNT" != "$EXPECTED_ROWS" ]]; then
    echo "E49D menu evidence row-count mismatch" >&2
    exit 1
  fi
  MIN_MULTI=$(( (EXPECTED_ROWS + 4) / 5 ))
  if (( EXPECTED_MULTI_COUNT < MIN_MULTI )); then
    echo "E49D has too few double-certified multi-route menus: $EXPECTED_MULTI_COUNT < $MIN_MULTI" >&2
    exit 1
  fi
fi
...
if [[ ! -f "$SOURCE_ROOT/oat_drgrpo/__init__.py" ]]; then
  echo "E49D frozen source snapshot is missing: $SOURCE_ROOT" >&2
  exit 1
fi
if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
  echo "E49D source snapshot hash mismatch" >&2
  exit 1
fi

OPS_INPUT="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e49d-ops.XXXXXX")"
...
OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e49d_maximal_support_math_ops_${OPS_HASH}/ops"
...
if [[ "$stage" != config && -e "$MANIFEST" ]]; then
  echo "Fresh E49D stage required; manifest already exists: $MANIFEST" >&2
  exit 1
fi
...
if any(record.get(key) != value for key, value in expected.items()):
    raise SystemExit("unexpected E49D judge endpoint")
...
CORE_HASH="$(sha256sum \
  ...
  "$SOURCE_ROOT/oat_drgrpo/args.py" \
...
payload = {
    "schema": "e49d_maximal_support_math_haarnoja_05b_v1",
    "stage": sys.argv[2],
...
if [[ "$stage" == config ]]; then
  ...
  echo "[e49d] configuration passed; no jobs submitted"
  exit 0
fi
...
if [[ "$rows" != 2 ]]; then
  echo "E49D expected two matched jobs, observed $rows" >&2
  exit 1
fi
echo "[e49d] submitted matched ${stage} cohort"
