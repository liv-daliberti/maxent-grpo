#!/usr/bin/env bash
# Launch E50's fresh 50-pass matched Dr.GRPO/uncapped-Haarnoja cohort.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e50_uncapped_normalized_canonical_haarnoja_05b_50ep.md"
...
GRAPH_PREFIX=gce50_uncapped_normalized_canonical_haarnoja_05b_50ep_v1
COUNTDOWN_PREFIX=cde50_uncapped_normalized_canonical_haarnoja_05b_50ep_v1
IDENTITY="$ROOT_DIR/var/artifacts/e50_uncapped_normalized_canonical_haarnoja_05b_50ep_v1_identity.json"
...
for required in \
  ...
  if [[ ! -e "$required" ]]; then
    echo "Missing frozen E50 prerequisite: $required" >&2
    exit 1
  fi
done
if ! grep -q '^\*\*Status: FROZEN BEFORE LAUNCH' "$PROTOCOL"; then
  echo "E50 protocol is not frozen" >&2
  exit 1
fi
for contract in \
  ...
  if ! grep -q "$needle" "$ROOT_DIR/$path"; then
    echo "Current source lacks E50 contract: $path :: $needle" >&2
    exit 1
  fi
done
...
for manifest in \
  ...
  if [[ "$phase" == full && -e "$manifest" ]]; then
    echo "Fresh E50 prefix required; manifest exists: $manifest" >&2
    exit 1
  fi
done
if [[ "$phase" == full && -e "$IDENTITY" ]]; then
  echo "Fresh E50 identity required; file exists: $IDENTITY" >&2
  exit 1
fi
...
freeze_execution() {
  ...
  SOURCE_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e50_uncapped_normalized_canonical_haarnoja_05b_50ep_${SOURCE_HASH}/src"
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E50 source snapshot hash mismatch" >&2
    exit 1
  fi

  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e50-ops-input.XXXXXX")"
  ...
  OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e50_uncapped_normalized_canonical_haarnoja_05b_50ep_ops_${EXECUTION_HASH}/ops"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E50 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
export OAT_ZERO_TRAIN_NODELIST="${OAT_ZERO_E50_TRAIN_NODELIST:-node302}"
export OAT_ZERO_TRAIN_GRES="${OAT_ZERO_E50_TRAIN_GRES:-gpu:a100:1}"
export OAT_ZERO_TRAIN_CPUS_PER_TASK="${OAT_ZERO_E50_TRAIN_CPUS_PER_TASK:-8}"
export OAT_ZERO_TRAIN_PARTITION="${OAT_ZERO_E50_TRAIN_PARTITION:-all}"
export OAT_ZERO_TRAIN_ACCOUNT="${OAT_ZERO_E50_TRAIN_ACCOUNT:-mltheory}"
export OAT_ZERO_TRAIN_MEMORY="${OAT_ZERO_E50_TRAIN_MEMORY:-64G}"
export OAT_ZERO_TRAIN_TIME_LIMIT="${OAT_ZERO_E50_TRAIN_TIME_LIMIT:-7-00:00:00}"
...
payload = {
    "schema": "e50_uncapped_normalized_canonical_haarnoja_05b_50ep_v1",
...
if [[ "$phase" == config ]]; then
  ...
  echo "[e50] both 50-pass matched configurations passed; no jobs submitted"
  exit 0
fi
...
cleanup_partial_cohort() {
  ...
  if [[ "$COHORT_RELEASED" != "1" && "${#job_ids[@]}" -gt 0 ]]; then
    ...
    echo "[e50] cancelled incomplete held cohort: ${job_ids[*]}" >&2
  fi
  exit "$status"
}
...
for prefix in "$GRAPH_PREFIX" "$COUNTDOWN_PREFIX"; do
  ...
  if [[ "${#task_jobs[@]}" -ne "$EXPECTED_JOBS_PER_TASK" ]]; then
    echo "E50 ${prefix} has ${#task_jobs[@]} jobs; expected $EXPECTED_JOBS_PER_TASK" >&2
    exit 1
  fi
  job_ids+=("${task_jobs[@]}")
...
if len(rows) != 6 or observed != expected:
    raise SystemExit(f"E50 held manifest mismatch: {observed!r}")
PY
done
if [[ "${#job_ids[@]}" -ne "$EXPECTED_JOBS" ]]; then
  echo "E50 cohort has ${#job_ids[@]} jobs; expected $EXPECTED_JOBS" >&2
  exit 1
fi
  ...
  while IFS=$'\t' read -r arm seed job_id run_stamp; do
    ...
    for required in \
      ...
      if [[ "$job_record" != *"$required"* ]]; then
        echo "E50 held-job audit failed for $job_id: missing $required" >&2
        exit 1
      fi
    done
    if [[ "$arm" == grpo ]]; then
      for required in \
        ...
        if [[ "$job_record" != *"$required"* ]]; then
          echo "E50 Dr.GRPO audit failed for $job_id: missing $required" >&2
          exit 1
        fi
      done
    else
      for required in \
        ...
        if [[ "$job_record" != *"$required"* ]]; then
          echo "E50 Haarnoja audit failed for $job_id: missing $required" >&2
          exit 1
        fi
      done
    fi
    if [[ "$job_record" != *"--nodelist=node302"* ]] || \
      ...
      echo "E50 held job $job_id does not attest the frozen placement/time limit" >&2
      exit 1
    fi
  done < "$manifest"
done
...
COHORT_RELEASED=1
trap - EXIT
echo "[e50] released ${#job_ids[@]} matched 50-pass jobs: ${job_ids[*]}"
echo "[e50] identity=$IDENTITY"
