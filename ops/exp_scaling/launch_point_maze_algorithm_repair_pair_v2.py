#!/usr/bin/env python3
"""Launch the K=16-aligned PointMaze repair-v2 calibration pair."""
...
_snapshot_files = stage.snapshot_files


def snapshot_files(files, prefix):
    files = list(files)
    if any(Path(source) == stage.AUDITOR for source, _ in files):
...
stage.atomic = atomic
stage.snapshot_files = snapshot_files
