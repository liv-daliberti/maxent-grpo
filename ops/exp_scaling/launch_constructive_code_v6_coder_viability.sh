#!/usr/bin/env bash
# Conditionally snapshot and launch the frozen ConstructiveCode v6 Coder gate.
set -euo pipefail
...
EVALUATOR="$ROOT_DIR/ops/evaluate_constructive_code_v6_coder_viability.py"
...
SLURM_SCRIPT="$ROOT_DIR/ops/slurm/evaluate_constructive_code_v6_coder_viability.slurm"
PROTOCOL="$ROOT_DIR/paper/preregistration/constructive_code_v6_coder_05b_viability_20260730.md"
...
IDENTITY="$ROOT_DIR/var/artifacts/constructive_code_v6_coder_05b_viability_identity.json"
SUBMISSION="$ROOT_DIR/var/artifacts/constructive_code_v6_coder_05b_viability_submission.json"
RECEIPT="$ROOT_DIR/var/artifacts/constructive_code_v6_coder_05b_viability.json"

for required in "$PYTHON_BIN" "$EVALUATOR" "$GATE_AUDITOR" "$V4_EVALUATOR" "$BASE_EVALUATOR" "$SLURM_SCRIPT" \
  "$PROTOCOL" "$GATE_IDENTITY" "$V6_MANIFEST" "$MODEL/config.json" "$MODEL/model.safetensors" \
  "$MODEL/tokenizer.json" "$IMAGE"; do
  [[ -e "$required" ]] || { echo "Missing ConstructiveCode v6 viability prerequisite: $required" >&2; exit 1; }
done
...
if [[ "$phase" == config ]]; then
  PYTHONPATH="$ROOT_DIR/ops:$ROOT_DIR/src" "$PYTHON_BIN" -m py_compile "$EVALUATOR" "$GATE_AUDITOR" "$V4_EVALUATOR" "$BASE_EVALUATOR"
  ...
  bash -n "$SLURM_SCRIPT"
  echo "[constructive-v6-coder-viability] configuration passed; no model sampled"
  exit 0
fi

[[ -f "$GATE_AUDIT" ]] || { echo "ConstructiveCode v6 gate has not produced an audit; refusing to sample" >&2; exit 1; }
...
if gate.get("expected_replay_count") != 960 or gate.get("observed_replay_count") != 960: raise SystemExit("v6 gate replay count differs from 960/960")
if gate.get("violations") not in ([],None) or gate.get("checker_equivalence_violations") not in ([],None): raise SystemExit("v6 gate contains a hard violation")
tasks=gate.get("task_results")
if not isinstance(tasks,list) or len(tasks)!=10 or any(row.get("status")!="pass" for row in tasks): raise SystemExit("v6 gate did not admit all ten tasks")
if gate.get("evaluation_rows_loaded") is not False or gate.get("language_model_sampling") is not False: raise SystemExit("v6 gate crossed its pre-model boundary")
PYGATE

for fresh in "$IDENTITY" "$SUBMISSION" "$RECEIPT"; do
  [[ ! -e "$fresh" ]] || { echo "Fresh ConstructiveCode v6 viability target required: $fresh" >&2; exit 1; }
done
[[ -f "$DATA_ROOT/manifest.json" ]] || { echo "Missing admitted v6 data root" >&2; exit 1; }
...
[[ -f "$SOURCE_ROOT/oat_drgrpo/__init__.py" && -f "$GATE_OPS_ROOT/replay_constructive_code_v5.py" ]] || { echo "ConstructiveCode v5 antecedent snapshots are missing" >&2; exit 1; }
...
EXEC_INPUT="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.constructive-v6-coder-viability.XXXXXX")"
cp "$EVALUATOR" "$EXEC_INPUT/evaluate_constructive_code_v6_coder_viability.py"
cp "$GATE_AUDITOR" "$EXEC_INPUT/audit_constructive_code_v6_gate.py"
...
cp "$SLURM_SCRIPT" "$EXEC_INPUT/evaluate_constructive_code_v6_coder_viability.slurm"
cp "$PROTOCOL" "$EXEC_INPUT/constructive_code_v6_coder_05b_viability_20260730.md"
EXECUTION_HASH="$(hash_tree "$EXEC_INPUT")"
EXECUTION_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/constructive_code_v6_coder_viability_${EXECUTION_HASH}"
if [[ ! -f "$EXECUTION_ROOT/evaluate_constructive_code_v6_coder_viability.py" ]]; then
...
job_id="$(sbatch --parsable --hold --partition=all --account=allcs \
  ...
  "$EXECUTION_ROOT/evaluate_constructive_code_v6_coder_viability.slurm")"
job_id="${job_id%%;*}"
[[ "$job_id" =~ ^[0-9]+$ ]] || { echo "Invalid ConstructiveCode v6 viability job ID" >&2; exit 1; }
...
"$PYTHON_BIN" - "$IDENTITY" "$job_id" "$SOURCE_HASH" "$GATE_EXEC_HASH" "$EXECUTION_HASH" \
  "$PROTOCOL" "$0" "$GATE_AUDIT" "$GATE_IDENTITY" "$MODEL/config.json" "$MODEL/tokenizer.json" "$DATA_ROOT/manifest.json" "$V6_MANIFEST" <<'PYID'
import hashlib,json,os,pathlib,sys,tempfile
def digest(path): return hashlib.sha256(pathlib.Path(path).read_bytes()).hexdigest()
path=pathlib.Path(sys.argv[1])
payload={
 "schema_version":"constructive-code-v6-coder-05b-viability-identity-v1",
 ...
 "v5_source_manifest_sha256":digest(sys.argv[12]),"v6_manifest_sha256":digest(sys.argv[13]),"model":"Qwen2.5-Coder-0.5B-Instruct",
 "model_snapshot":"ea3f2471cf1b1f0db85067f1ef93848e38e88c25",
 "development_problem_ids":["359_B","988_A","1399_D"],"evaluation_problem_ids":["361_B","1294_C","149_C"],"evaluation_rows_loaded":False,
...
for required in 'JobState=PENDING' 'Reason=JobHeldUser' 'gres/gpu:a5000:1' 'NumCPUs=16' 'MinMemoryNode=64G' 'TimeLimit=04:00:00'; do
  [[ "$record" == *"$required"* ]] || { echo "Held ConstructiveCode v6 viability job missing $required" >&2; exit 1; }
done
[[ "$record" == *'Partition=all'* || "$record" == *'Partition=cs'* || "$record" == *'Partition=mltheory'* ]] || { echo "Held ConstructiveCode v6 viability job has unexpected partition" >&2; exit 1; }
"$PYTHON_BIN" - "$SUBMISSION" "$job_id" "$IDENTITY" "$record" <<'PYSUB'
import hashlib,json,os,pathlib,sys,tempfile
path=pathlib.Path(sys.argv[1]); identity=pathlib.Path(sys.argv[3])
payload={"schema_version":"constructive-code-v6-coder-05b-viability-submission-v1","job_id":int(sys.argv[2]),"identity_sha256":hashlib.sha256(identity.read_bytes()).hexdigest(),"held_job_audit":"pass","released":True,"scheduler_record":sys.argv[4]}
path.parent.mkdir(parents=True,exist_ok=True); fd,tmp=tempfile.mkstemp(prefix=f".{path.name}.",dir=path.parent)
with os.fdopen(fd,"w",encoding="utf-8") as handle: json.dump(payload,handle,indent=2,sort_keys=True); handle.write("\n")
os.replace(tmp,path)
PYSUB
...
trap - EXIT
echo "[constructive-v6-coder-viability] released job $job_id"
echo "[constructive-v6-coder-viability] identity=$IDENTITY"
