#!/usr/bin/env bash
# Configure or atomically submit E62-R9's same-seed Python mechanism pilot.
set -euo pipefail
...
PROTOCOL="$ROOT_DIR/paper/preregistration/e62_verified_counterfactual_proposals_05b.md"
AUDITOR="$ROOT_DIR/ops/exp_scaling/audit_e62_python_pilot.py"
PLOTTER="$ROOT_DIR/ops/exp_scaling/plot_e62_python_pilot.py"
WATCHER="$ROOT_DIR/ops/exp_scaling/watch_e62_python_pilot.sh"
...
IDENTITY="$ROOT_DIR/var/artifacts/e62r9_open_set_sweep_python_pilot_identity.json"
AUDIT_OUT="$ROOT_DIR/var/artifacts/e62r9_python_pilot_audit_latest.json"
...
freeze_execution() {
  ...
  SOURCE_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e62r9_open_set_sweep_${SOURCE_HASH}/src"
  ...
  if [[ "$(hash_tree "$SOURCE_ROOT")" != "$SOURCE_HASH" ]]; then
    echo "E62-R9 source snapshot hash mismatch" >&2
    exit 1
  fi

  ops_input="$(mktemp -d "$ROOT_DIR/var/artifacts/source_snapshots/.e62r9-ops-input.XXXXXX")"
  ...
  OPS_ROOT="$ROOT_DIR/var/artifacts/source_snapshots/e62r9_open_set_sweep_ops_${EXECUTION_HASH}/ops"
  ...
  if [[ "$(hash_tree "$OPS_ROOT")" != "$EXECUTION_HASH" ]]; then
    echo "E62-R9 execution snapshot hash mismatch" >&2
    exit 1
  fi
}
...
payload = {
    "schema": "e62r9_open_set_sweep_python_pilot_v1",
    ...
    "proposal_compute": {
        "primary_actuator": (
            "validator_preserving_transform_of_model_verified_response"
        ),
...
released=1
trap - EXIT
echo "[e62r9] released Python mechanism pilot jobs ${job_ids[*]}"
echo "[e62r9] identity=$IDENTITY"
