#!/usr/bin/env bash
# Launch the frozen E23 canonical Countdown 7B Standard-MaxEnt cohort.
set -euo pipefail
...
STAMP=cde23_canonical_maxent_7b_v4_4xa100
...
export OAT_ZERO_SYNC_PARAMS_EVERY=96
...
if [[ "$PHASE" == config ]]; then
  ...
  echo "[e23] frozen canonical Countdown-7B configuration passed; no jobs submitted"
  exit 0
fi
...
echo "[e23] released complete canonical Countdown-7B cohort: ${job_ids[*]}"
echo "[e23] manifest=$MANIFEST"
