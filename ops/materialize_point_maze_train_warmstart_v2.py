#!/usr/bin/env python3
"""Replay certified train-only PointMaze v2 routes into state-action examples.

Identical to the v1 builder except for two cardinality assumptions that were
specific to the v1 cohort: it fixed the training split at eight maps and every
map at exactly two certified routes. v2 has 384 maps carrying 3-6 routes each,
so both checks are generalised rather than dropped -- the split must still be
non-empty with unique map ids, and every map must still carry at least two
certified routes (one route cannot demonstrate alternative solutions).
"""
...
def main() -> None:
    ...
    if not rows:
        raise ValueError("PointMaze warm start requires a non-empty train split")
    ...
    if any(
        ...
        raise ValueError("every train map needs at least two certified routes")
    ...
    try:
        # The worker enforces a 60s alarm per request and builds one MuJoCo env
        # per distinct spec. v1 replayed all 16 episodes at once because they
        # spanned only 8 small maps; v2's grids are larger and there are 384 of
        # them, so batch strictly per map -- every request then constructs one
        # env and carries only that map's 3-6 certified routes.
        by_map: dict[int, list] = {}
        for episode in episodes:
            by_map.setdefault(int(episode["row_index"]), []).append(episode)
        for done_maps, (row_index, chunk) in enumerate(sorted(by_map.items())):
            if done_maps % 32 == 0:
                print(f'  replaying map {done_maps}/{len(by_map)} '
                      f'({len(episodes)} episodes total)', flush=True)
            ...
            while active:
                ...
                for (episode, action), transition in zip(choices, transitions):
                    ...
                    if transition["done"]:
                        # v1 required the program to be consumed exactly. v2
                        # programs were calibrated by a batch verifier that runs
                        # them to completion, so a trailing leg can overshoot a
                        # goal the interactive worker terminates on. The consumed
                        # prefix is itself a certified solution reaching the same
                        # gate, so truncate rather than reject -- but still demand
                        # the canonical key match, which is what identifies the
                        # route.
                        episode["truncated_actions"] = len(episode["actions"])
                        episode["actions"] = []
