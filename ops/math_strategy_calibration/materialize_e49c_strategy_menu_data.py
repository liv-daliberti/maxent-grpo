...
def _endpoint(record_path: pathlib.Path) -> tuple[str, str]:
    ...
    host = os.environ.get("E49C_QWEN_HOST_OVERRIDE", "").strip()
    if not host:
    ...
    if isinstance(raw_strategies, list):
        observed = [
            (
                ...
                tuple(strategy.get("action_ids") or ()),
            )
            for strategy in raw_strategies
            if isinstance(strategy, dict)
        ]
        combos = [combo for _, combo in observed]
        if len(set(combos)) != len(combos):
            rendered = ", ".join(
                f"{strategy_id}={'>'.join(combo)}"
                for strategy_id, combo in observed
            )
            raise ValueError(
                "strategy action combos must be distinct; the prior menu "
                f"returned {rendered}. Add route-specific mathematical "
                "actions and assign each strategy a genuinely different "
                "ordered action-ID sequence."
            )
    embedded = (
        f"x\n{MENU_START}\n"
...
def main() -> None:
    ...
    if args.audit_only:
        ...
        if len(records) != manifest["menu_count"] or not all(
            _record_passes_contract(record) for record in records.values()
        ):
    ...
    pending = [
        ...
        for row_id, problem, reference_answer in work
        if not (
            _record_passes_contract(existing.get(row_id, {}))
        )
    ]
    print(
    ...
    failures = [
        ...
        if not (
            _record_passes_contract(existing[row_id])
        )
    ]
    if failures:
        _write_json(
