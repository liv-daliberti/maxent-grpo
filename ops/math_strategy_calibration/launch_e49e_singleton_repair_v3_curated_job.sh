#!/usr/bin/env bash
# Submit the precommitted curated-contract audit for any post-V2 gaps.
set -euo pipefail
...
V2B_EVIDENCE="$ROOT_DIR/var/artifacts/e49e_trace_bank_math_toy_repair_v2b"
V2B_RECORDS="$V2B_EVIDENCE/repair_records.jsonl"
V2B_JOB_RECORD="$V2B_EVIDENCE/repair_job.json"
V2B_IDENTITY="$V2B_EVIDENCE/frozen_identity.json"
...
AMENDMENT="$ROOT_DIR/paper/preregistration/e49e_curated_singleton_contingency_amendment_20260724.md"
OBJECTIVE_AMENDMENT="$ROOT_DIR/paper/preregistration/e49e_curated_objective_dual_audit_amendment_20260724.md"
WORKTREE_CONTRACTS="$ROOT_DIR/ops/math_strategy_calibration/e49e_curated_singleton_contracts_toy.json"
...
SLURM="$ROOT_DIR/ops/slurm/e49e_repair_singletons_v3_curated_node915.slurm"
...
for required in \
  ...
  "$V2B_JOB_RECORD" \
  "$V2B_IDENTITY" \
  "$V2B_EVIDENCE/frozen_e49d_input/menu_records.jsonl" \
  "$V2B_EVIDENCE/frozen_e49e_input/trace_bank_records.jsonl" \
  "$V2B_EVIDENCE/frozen_kernel_augmentation/augmentation_records.jsonl" \
  "$V2B_EVIDENCE/qwen72_endpoint.json" \
  "$AMENDMENT" \
  "$OBJECTIVE_AMENDMENT" \
  "$WORKTREE_CONTRACTS" \
  "$INVALID_SOURCE" \
  ...
  "$SLURM"; do
  if [[ ! -e "$required" ]]; then
    echo "missing V3 repair prerequisite: $required" >&2
    exit 2
  fi
done
if [[ -e "$RECORD" || -e "$OUTPUT" ]]; then
  echo "fresh V3 curated repair required" >&2
  exit 2
fi
...
v2b_state="$(
  sacct -X -n -j "$v2b_job_id" --format=State -P \
    | head -n 1 | cut -d'|' -f1
)"
case "$v2b_state" in
  COMPLETED*|FAILED*|CANCELLED*|TIMEOUT*|OUT_OF_MEMORY*) ;;
  *)
    echo "V2b repair $v2b_job_id is not terminal: $v2b_state" >&2
    exit 2
    ;;
esac
"$PYTHON_BIN" - "$V2B_RECORDS" <<'PY'
import json
import sys
rows = [json.loads(line) for line in open(sys.argv[1], encoding="utf-8") if line.strip()]
ids = [row.get("row_id") for row in rows]
if len(rows) != 11 or len(set(ids)) != 11 or None in ids:
    raise SystemExit(f"terminal V2b repair evidence is incomplete: {len(set(ids))}/11")
PY

readarray -t frozen_fields < <(
  "$PYTHON_BIN" - "$V2B_IDENTITY" <<'PY'
import json
import sys
record = json.load(open(sys.argv[1], encoding="utf-8"))
if record.get("schema") != "e49e_singleton_repair_v2b_frozen_identity_v1":
    raise SystemExit("unexpected V2b frozen identity")
for key in (
    "snapshot_root",
...
V2B_SNAPSHOT_ROOT="${frozen_fields[0]}"
V2B_SNAPSHOT_HASH="${frozen_fields[1]}"
FROZEN_CONTRACTS_HASH="${frozen_fields[2]}"
FROZEN_AMENDMENT_HASH="${frozen_fields[3]}"
...
if [[ "$(hash_tree "$V2B_SNAPSHOT_ROOT")" != "$V2B_SNAPSHOT_HASH" ]]; then
  echo "pre-V2b source snapshot changed" >&2
  exit 2
fi
if [[ "$(sha256sum "$WORKTREE_CONTRACTS" | cut -d' ' -f1)" != "$FROZEN_CONTRACTS_HASH" ]] \
  || [[ "$(sha256sum "$AMENDMENT" | cut -d' ' -f1)" != "$FROZEN_AMENDMENT_HASH" ]]; then
  echo "V3 contingency differs from the pre-V2b frozen version" >&2
  exit 2
fi
if ! grep -q \
  '^\*\*Status: FROZEN BEFORE ANY CURATED-CONTRACT AUDIT REQUEST' \
  "$OBJECTIVE_AMENDMENT"; then
  echo "V3 objective dual-audit amendment is not frozen" >&2
  exit 2
fi

mkdir -p "$EVIDENCE" "$SNAPSHOT_PARENT"
snapshot_staging="$(mktemp -d "$SNAPSHOT_PARENT/.e49e-repair-v3.XXXXXX")"
mkdir -p \
...
SNAPSHOT_ROOT="$SNAPSHOT_PARENT/e49e_singleton_repair_v3_${SNAPSHOT_HASH}"
if [[ ! -e "$SNAPSHOT_ROOT" ]]; then
  mv "$snapshot_staging" "$SNAPSHOT_ROOT"
else
  if [[ "$(hash_tree "$SNAPSHOT_ROOT")" != "$SNAPSHOT_HASH" ]]; then
    echo "existing V3 repair snapshot hash mismatch" >&2
    exit 2
  fi
...
CONTRACTS="$SNAPSHOT_ROOT/ops/math_strategy_calibration/e49e_curated_singleton_contracts_toy.json"
...
if [[ "$(sha256sum "$CONTRACTS" | cut -d' ' -f1)" != "$FROZEN_CONTRACTS_HASH" ]]; then
  echo "V3 snapshot changed the precommitted contracts" >&2
  exit 2
fi
...
cp "$V2B_EVIDENCE/frozen_e49d_input/"* "$FROZEN_E49D/"
cp "$V2B_EVIDENCE/frozen_e49e_input/"* "$FROZEN_E49E/"
cp "$V2B_EVIDENCE/frozen_kernel_augmentation/"* "$FROZEN_AUGMENT/"
FROZEN_ENDPOINT="$EVIDENCE/qwen72_endpoint.json"
FROZEN_PRIOR="$EVIDENCE/frozen_v2b_repair_records.jsonl"
cp "$V2B_EVIDENCE/qwen72_endpoint.json" "$FROZEN_ENDPOINT"
cp "$V2B_RECORDS" "$FROZEN_PRIOR"

PREFLIGHT="$EVIDENCE/preflight.json"
E49E_PRIOR_REPAIR_V2B_RECORDS="$FROZEN_PRIOR" \
E49E_CURATED_CONTRACTS="$CONTRACTS" \
...
if (
    ...
    raise SystemExit("V3 curated-repair preflight failed")
PY
...
keys = (
    ...
    "snapshot_tree_sha256",
    "v2b_snapshot_root",
    "v2b_snapshot_tree_sha256",
    "prior_v2b_repair_records_sha256",
    "curated_contracts_sha256",
    "curated_contingency_amendment_sha256",
...
payload = {
    "schema": "e49e_singleton_repair_v3_frozen_identity_v1",
    **dict(zip(keys, sys.argv[2:], strict=True)),
    "curated_audit_requests_at_freeze": 0,
}
...
path = pathlib.Path(sys.argv[1])
payload = {
    "schema": "e49e_singleton_repair_v3_job_v1",
    "job_id": sys.argv[2],
    "stage": "toy",
...
os.replace(temporary, path)
PY
echo "submitted E49E toy precommitted curated-repair V3 job $job_id"
