#!/usr/bin/env python3
"""Export the exact admitted v10 Arrow rows for networkless node replay."""
...
VERSION = "v10"
DATA_SCHEMA = "ant-maze-modebench-data-v10"
AUDIT_DECISION = "admitted_to_v10_cross_node_route_determinism_gate"
ROUTE_SCHEMA = "ant-maze-v10-route-generation-identity-v1"
EXPORT_SCHEMA = "ant-maze-v10-cross-node-spec-export-v1"
CONTROLLER_IDENTITY_FIELD = "controller_receipt_sha256"
...
def main() -> None:
    ...
    if args.output.exists():
        raise FileExistsError(f"fresh v10 cross-node export required: {args.output}")
    ...
    if data_identity.get("schema_version") != DATA_SCHEMA:
        raise ValueError(f"{VERSION} cross-node export received the wrong data schema")
    if (
        route_audit.get("status") != "pass"
        or route_audit.get("decision")
        != AUDIT_DECISION
    ):
        raise ValueError(f"{VERSION} route audit is not admitted to cross-node replay")
    if route_identity.get("schema_version") != (
        ROUTE_SCHEMA
    ):
        raise ValueError(f"{VERSION} route generation identity schema mismatch")
    certification = {
        str(row["map_id"]): row for row in data_identity.get("certification", [])
    }
    if len(certification) != 12:
        raise ValueError("v10 data identity does not certify 12 maps")
    rows = []
    for split, dataset_split in (
        ...
        if set(dataset) != {dataset_split}:
            raise ValueError(f"v10 {split} dataset split mismatch")
        for row in dataset[dataset_split].to_list():
            ...
            if record is None or record.get("spec_sha256") != spec.get("spec_sha256"):
                raise ValueError(f"v10 certification mismatch for {map_id}")
            routes = [
                {
            ...
            if len(routes) != 2 or len({item["canonical_key"] for item in routes}) != 2:
                raise ValueError(f"v10 route identity mismatch for {map_id}")
            rows.append({"split": split, "spec": spec, "routes": routes})
    if len(rows) != 12 or len({row["spec"]["spec_sha256"] for row in rows}) != 12:
        raise ValueError("v10 cross-node export rows are not 12 unique specs")
    ...
    payload = {
        ...
        "controller_receipt_sha256": route_identity[CONTROLLER_IDENTITY_FIELD],
    ...
    print(f"[ant-{VERSION}-cross-node-export] maps=12 routes=24 output={args.output}")
