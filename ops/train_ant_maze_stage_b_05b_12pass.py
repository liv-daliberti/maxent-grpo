#!/usr/bin/env python3
"""Train one frozen AntMaze Stage-B arm/seed cell for 12 prompt passes."""
...
import train_ant_maze_interactive_paired_smoke_v13 as ant_base  # noqa: E402
...
base = ant_base.base
CONTROL = ant_base.CONTROL
TREATMENT = ant_base.TREATMENT
ARMS = ant_base.ARMS
...
EXPECTED_MAP_IDS = (
    "ant_v12_admission_eval_08",
    "ant_v12_admission_eval_09",
    "ant_v12_admission_eval_10",
    "ant_v12_admission_eval_11",
)
...
def _restricted_mixed_actions(
    *,
    ...
    if not (len(prompts) == len(greedy) == len(request_seeds)):
        raise ValueError("AntMaze evaluation selection vectors differ")
    ...
    if any(not row or len(row) > max_length for row in encoded):
        raise ValueError("AntMaze evaluation prompt left its frozen token bound")
    selected: list[int] = []
    ...
    *,
    ...
    for row in rows:
        ...
        if (
            ...
            or spec.max_actions != ant_base.HORIZON
            or spec.action_repeat != 400
        ):
            raise ValueError("AntMaze evaluation action/controller contract drift")
    ...
    if len(sessions) != EVAL_TRAJECTORIES:
        raise RuntimeError("AntMaze evaluation trajectory count changed")
    ...
    for decision_round in range(1, ant_base.HORIZON + 1):
        prompts = []
        request_seeds = []
        greedy_flags = []
        ...
        if len(transitions) != len(active_indices):
            raise RuntimeError("AntMaze evaluation worker batch changed")
        ...
        for index, transition in zip(active_indices, transitions):
            ...
            if transition["session_id"] != session_id:
                raise RuntimeError("AntMaze evaluation worker reordered sessions")
    ...
    if any(active):
        raise RuntimeError("AntMaze evaluation exceeded the frozen horizon")
    ...
    result: dict[str, Any] = {
        "schema": "ant-maze-stage-b-evaluation-v1", "split": "multi_answer",
    ...
    for row_index, map_id in enumerate(EXPECTED_MAP_IDS):
        ...
        for draw in range(EVAL_DRAWS):
            ...
            result[f"eval/{map_id}/mean8_draw_{draw}"] = mean_value
            result[f"eval/{map_id}/pass8_draw_{draw}"] = pass_value
            result[f"eval/{map_id}/distinct8_draw_{draw}"] = distinct_value
    ...
    if any(isinstance(value, float) and not math.isfinite(value) for value in result.values()):
        raise RuntimeError("nonfinite AntMaze evaluation metric")
    return result
...
def main() -> None:
    ...
    if args.seed not in SEEDS or args.learning_rate != 2e-7 or args.max_length != 1536:
        raise ValueError("AntMaze Stage-B frozen cell contract drift")
    if args.microbatch_size != 16 or args.evaluation_batch_size != 16:
        raise ValueError("AntMaze Stage-B batch contract drift")
    for fresh in (args.output_receipt, args.output_metrics, args.output_replay):
        if fresh.exists():
            raise FileExistsError(f"fresh AntMaze Stage-B output required: {fresh}")
    for required in (
    ...
    if not torch.cuda.is_available() or not torch.cuda.is_bf16_supported():
        raise RuntimeError("AntMaze Stage B requires a BF16 GPU")
    ...
    if (
        ...
        or qualification.get("decision") != "eligible_for_ten_ant_maze_stage_b_jobs"
        or identity.get("jobs", {}).get(label) != int(args.job_id)
    ):
        raise ValueError("AntMaze Stage-B qualification or job identity drift")
    ...
    if set(train_dict) != {"train"} or set(evaluation_dict) != {"multi_answer"}:
        raise ValueError("AntMaze Stage-B split keys changed")
    ...
    if len(train_rows) != TRAIN_PROMPTS or len(evaluation_rows) != EVAL_PROMPTS:
        raise ValueError("AntMaze Stage-B split cardinality changed")
    evaluation_map_ids = []
    for row in evaluation_rows:
        raw_spec = row["answer"]
        if isinstance(raw_spec, str):
            raw_spec = json.loads(raw_spec)
        evaluation_map_ids.append(parse_maze_action_spec(raw_spec).map_id)
    if tuple(evaluation_map_ids) != EXPECTED_MAP_IDS:
        raise ValueError("AntMaze Stage-B evaluation map order changed")
    ...
    action_token_ids = []
    for label_token in ANT_POLICY_ACTIONS:
        ...
        if len(token_ids) != 1 or tokenizer.decode(token_ids) != label_token:
            raise RuntimeError(f"AntMaze label is not one exact token: {label_token}")
    ...
    with AntMazeInteractiveProcess(worker_python=args.worker_python) as worker:
        ...
        for update_index in range(1, UPDATES + 1):
            ...
            episodes, policy_slots, rollout = ant_base.rollout_group(
            ...
            active_mask = [True] * ant_base.SAMPLES
            semantic_raw, semantic_diag = semantic.score_success_conditioned_signed_advantages_and_update(
                ...
                active_mask=active_mask, num_samples=ant_base.SAMPLES,
            )
            ...
            for slot in policy_slots:
                if slot["active"]:
                    ...
                    slot["weight"] = float(advantages[episode_index].item()) / decision_counts[episode_index] / ant_base.SAMPLES
            ...
            replay_slots, replay_diag = base._replay_slots(
                ...
                horizon=ant_base.HORIZON, replay_capacity=ant_base.REPLAY_CAPACITY,
                samples=ant_base.SAMPLES,
            )
            ...
            if not math.isfinite(float(grad_norm.detach().item())):
                raise RuntimeError("nonfinite AntMaze Stage-B gradient")
            optimizer.step()
            metric = {
                "schema": "ant-maze-stage-b-training-metric-v1", "split": "train",
            ...
            if any(isinstance(value, float) and not math.isfinite(value) for value in metric.values()):
                raise RuntimeError("nonfinite AntMaze Stage-B training metric")
            training_metrics.append(metric)
            append_jsonl(args.output_metrics, [metric])
            append_jsonl(args.output_replay, [{
                "schema": "ant-maze-stage-b-state-replay-v1", "arm": args.arm,
    ...
    payload = {
        ...
        "counts": {
            ...
            "training_rollouts": UPDATES * ant_base.SAMPLES,
            "training_fixed_policy_slots": UPDATES * ant_base.SAMPLES * ant_base.HORIZON,
            "training_fixed_replay_decision_slots": UPDATES * ant_base.REPLAY_CAPACITY * ant_base.HORIZON,
        ...
        "information_boundary": {
            ...
            "fixed_public_action_mask": True, "controller_feedback_in_prompt": False,
            "fixed_v11_low_level_controller": True,
        },
    }
    atomic(args.output_receipt, payload)
    print(f"[ant-stage-b] complete arm={args.arm} seed={args.seed}", flush=True)
