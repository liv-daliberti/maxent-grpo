...
# Revert to the sharper fixed-tau recipe. In this hybrid, learned tau quickly
# softened the auxiliary and erased the early-training gains we were after.
...
# Keep the sequence-side listwise auxiliary length-independent. The primary
# update now comes from the true Dr.GRPO token PPO surrogate, while the
# listwise term acts as a smaller prompt-local shaping loss over normalized
# sequence scores instead of raw summed support.
...
# Skip prompt groups whose reward targets collapse to uniform mass. Otherwise
# the listwise CE term still backpropagates a "match uniform" gradient on
# all-wrong/all-right groups, which can swamp the sparse informative signal in
# verifiable-math training.
