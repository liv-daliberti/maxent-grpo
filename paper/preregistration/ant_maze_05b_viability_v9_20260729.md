# AntMaze v9 Qwen2.5-0.5B development viability

**Status: FROZEN DURING V9 CONTROLLER TRAINING, BEFORE ITS OUTCOME OR ANY V9 MODEL COMPLETION — 2026-07-29**

This gate is conditional on passes from the frozen v9 controller, 9x9 route
admission, and three-node exact-slate replay. It may not run if any antecedent
is missing or failed.

## Immutable sampling contract

- exact four-row `dev/multi_answer` split from the admitted
  `var/data/ant_maze_modebench_v9` slate;
- pinned Qwen2.5-0.5B-Instruct snapshot
  `7ae557604adf67be50417f59c2c2f167def9a775`;
- temperature 1.0, top-p 1.0, seed `97309`;
...
- assistant prefill `\boxed{` and the fixed `ant_maze_v9` grammar reminder;
...
positive is executed by the exact admitted v9 controller and topology
verifier.
...
stops AntMaze v9 before language-model training; no prompt, seed, sample count,
or threshold repair is allowed.
