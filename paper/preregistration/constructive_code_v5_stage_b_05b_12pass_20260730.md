# ConstructiveCode v5 Stage B: clean 0.5B verified MaxEnt versus compute-matched Dr.GRPO

**Status: FROZEN BEFORE THE V5 EXECUTABLE-GATE, CODER-VIABILITY, AND PAIRED-SMOKE OUTCOMES AND BEFORE STAGE-B SUBMISSION — 2026-07-30**

## Conditional authorization

This protocol may launch only if the exact v5 executable gate and Qwen2.5-Coder-0.5B viability gate pass, followed by a passing immutable paired online smoke with decision `eligible_for_ten_constructive_code_stage_b_jobs`. A failed or incomplete antecedent stops ConstructiveCode; no final cell is submitted. Development weights and seed 78101 are never reused.
...
- training tasks, in order: 327B (`ordered_sequence`), 659C (`unordered_set`), 1208C (`assignment`), and 1102B (`unordered_partition`);
- evaluation tasks, in order: 361B, 1294C, 1408A, and 149C, one per frozen witness family;
...
The four evaluation tasks are loaded only inside final paper jobs. Evaluation occurs before training and after every update, giving all 49 quarter-pass coordinates from pass 0 through pass 12. At each coordinate, each task receives one greedy program plus four deterministic temperature-one K=8 replicates: 132 programs total. Every program executes once without feedback in the same frozen worker. Evaluation updates no optimizer, tracker, bank, prompt order, request namespace, or training schedule.
...
It also binds the v5 gate, Coder viability, paired smoke, source, operations, model, data, runtime image, sandbox launcher, checker builds, protocol, manifest, scheduler, metrics, and candidate ledgers. Missing or failed cells remain visible and prevent ConstructiveCode from becoming a terminal paper row.
