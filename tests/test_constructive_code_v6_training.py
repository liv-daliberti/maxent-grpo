...
import audit_constructive_code_v6 as audit
import launch_constructive_code_v6_experiment as launch
import train_constructive_code_v6 as train


def test_frozen_task_schedule_and_compute_rectangle() -> None:
    assert train.DEVELOPMENT_PROBLEMS == ("359_B", "988_A", "1399_D")
    assert train.TRAIN_PROBLEMS == ("327_B", "659_C", "1283_C", "1102_B")
    assert train.EVALUATION_PROBLEMS == ("361_B", "1294_C", "149_C")
    assert train.DEVELOPMENT_FAMILIES == (
        "ordered_sequence",
        "unordered_set",
        "unordered_partition",
    )
    assert train.TRAIN_FAMILIES == (
        "ordered_sequence",
        "unordered_set",
        "assignment",
        "unordered_partition",
...
def test_shell_tree_hash_matches_frozen_gate_identity() -> None:
    identity = json.loads(
        (ROOT / "var/artifacts/constructive_code_v6_gate_identity.json").read_text()
    )
...
def test_dependency_chain_is_afterok_and_fail_closed() -> None:
    launcher = (
        ROOT / "ops/exp_scaling/launch_constructive_code_v6_experiment.py"
    ).read_text()
    ...
    assert "OAT_ZERO_EXPECTED_LAUNCHER_SHA256" in after_smoke
    assert '"$LAUNCHER" stage_b run' in after_smoke


def test_v6_evaluation_coordinate_is_three_tasks_and_99_programs() -> None:
    assert len(train.EVALUATION_PROBLEMS) * (1 + 4 * 8) == 99
    source = (ROOT / "ops/train_constructive_code_v6.py").read_text()
    assert '"evaluation_programs": evaluation_program_count' in source


def test_v6_emits_exact_online_mean_support_for_the_shared_graph_panel() -> None:
    source = (ROOT / "ops/train_constructive_code_v6.py").read_text()
    assert '"online_canonical_mean_support_per_prompt"' in source
    assert "canonical.mean_support_per_prompt" in source
