...
from maxent_helpers.run_training_pipeline import (  # noqa: E402  import after stubbing torch
...
def _patch_helpers(monkeypatch):
    ...
    monkeypatch.setattr(
        "maxent_helpers.run_training_pipeline.build_score_batch",
        _fake_build_score_batch,
    )
    monkeypatch.setattr(
        "maxent_helpers.run_training_pipeline.compute_weight_stats",
        _fake_weight_stats,
    )
    monkeypatch.setattr(
        "maxent_helpers.run_training_pipeline.summarize_completion_lengths",
        _fake_lengths,
    )
...
def test_collect_batch_stats_counts_prompt_tokens(monkeypatch):
    ...
    monkeypatch.setattr(
        "maxent_helpers.run_training_pipeline.gather_reference_logprobs",
        _fake_gather,
    )
...
def test_prepare_training_batch_success(monkeypatch):
    ...
    def _fake_prepare_generation_batch(batch, generator, stats, num_generations, max_retry_rounds):
    ...
    monkeypatch.setattr(
        "maxent_helpers.run_training_pipeline.prepare_generation_batch",
        _fake_prepare_generation_batch,
    )
    monkeypatch.setattr(
        "maxent_helpers.run_training_pipeline.compute_reward_statistics",
        _fake_reward_stats,
    )
    monkeypatch.setattr(
        "maxent_helpers.run_training_pipeline.gather_reference_logprobs",
    ...
    monkeypatch.setattr(
        "maxent_helpers.run_training_pipeline.score_model_outputs",
        _fake_score_model,
    )
    monkeypatch.setattr(
        "maxent_helpers.run_training_pipeline.build_sequence_scores",
