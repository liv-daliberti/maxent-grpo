...
def test_prompt_char_limit_prefers_ctx_override():
    helper = _PromptLimitHelper(SimpleNamespace(prompt_char_limit=123, max_prompt_len=10))
...
def test_prompt_char_limit_handles_negative_constant(monkeypatch):
    ...
    monkeypatch.setattr(modules["maxent_grpo.generation"], "vllm", dummy_vllm, raising=False)
    helper = _PromptLimitHelper(SimpleNamespace(prompt_char_limit=None, max_prompt_len=6))
...
def test_prompt_char_limit_uses_constant_when_no_max_len(monkeypatch):
    ...
    monkeypatch.setitem(modules, dummy_vllm.__name__, dummy_vllm)
    monkeypatch.setattr(modules["maxent_grpo.generation"], "vllm", dummy_vllm, raising=False)
    helper = _PromptLimitHelper(SimpleNamespace(prompt_char_limit=None, max_prompt_len=None))
