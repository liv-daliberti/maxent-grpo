"""Tests for maxent_helpers.run_training_rewards."""
...
import maxent_helpers.run_training_rewards as rr  # noqa: E402
from maxent_helpers.run_training_rewards import (  # noqa: E402
    RewardComputation,
    RewardSpec,
    prepare_generation_batch,
)
from maxent_helpers.run_training_types import GenerationBatch  # noqa: E402
from utils.vllm_patch import VLLMLogprobResult  # noqa: E402
...
def test_prepare_generation_batch_keeps_partial_and_drops_empty():
    ...
    gen_batch = prepare_generation_batch(batch, generator, stats, expected_generations=2)
...
def test_prepare_generation_batch_retries_until_prompt_fills():
    ...
    def generator(prompts, expected_generations, per_prompt_counts=None):
        calls.append((list(prompts), None if per_prompt_counts is None else list(per_prompt_counts)))
        if len(calls) == 1:
            return ([["p1-c1"], []], None)
        assert prompts == ["p2"]
    ...
    gen_batch = prepare_generation_batch(batch, generator, stats, expected_generations=1)
...
def test_flatten_ref_metadata_emits_trl_payload():
    ...
    flat = rr._flatten_ref_metadata([["comp"]], [[meta]])
    assert flat == [
        {
            "logprob_sum": -1.0,
            "token_count": 2,
