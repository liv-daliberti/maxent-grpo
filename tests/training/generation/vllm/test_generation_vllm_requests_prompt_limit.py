...
import maxent_grpo.training.generation.vllm_requests as vr
...
def test_prompt_char_limit_handles_negative_constant(monkeypatch):
    dummy_vllm = ModuleType("maxent_grpo.training.generation.vllm")
    dummy_vllm.PROMPT_CHAR_LIMIT = -1
    modules = __import__("sys").modules
    monkeypatch.setitem(modules, dummy_vllm.__name__, dummy_vllm)
    monkeypatch.setattr(
        modules["maxent_grpo.training.generation"], "vllm", dummy_vllm, raising=False
    )
    helper = _PromptLimitHelper(
