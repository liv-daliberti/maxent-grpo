...
def _load_validator():
    return runpy.run_path(str(_resolve_tool("validate_logs.py")))
...
def test_validate_file_flags_nan(tmp_path: Path):
    ...
    errors, _hits = mod["validate_file"](log, require_keys=[])
    assert errors
...
def test_run_validation_handles_multiple_files(tmp_path: Path):
    ...
    errors = mod["run_validation"]([good, bad], require_keys=["train/loss"])
