...
LAUNCHER = (
    ROOT
    / "ops/exp_scaling/launch_e42_quality_gated_semantic_novelty_05b.sh"
)
PROTOCOL = (
    ROOT
    / "paper/preregistration/e42_quality_gated_semantic_novelty_05b.md"
)
...
def test_e42_is_treatment_only_three_domain_nine_run_cohort():
    ...
    for literal in (
        ...
        "OAT_ZERO_INCLUDE_QUALITY_GATED_SEMANTIC_NOVELTY_ARM=1",
...
def test_e42_audits_and_hashes_frozen_e37_e38_e39_e41_comparators():
    text = LAUNCHER.read_text(encoding="utf-8")
    for literal in (
...
def test_e42_freezes_source_execution_data_and_held_release_gate():
    text = LAUNCHER.read_text(encoding="utf-8")
    for literal in (
        'cd "$tree"',
        "find . -type f -print0",
        "e42_quality_gated_semantic_novelty_05b_${SOURCE_HASH}",
        "e42_quality_gated_semantic_novelty_05b_ops_${EXECUTION_HASH}",
...
def test_e42_matches_e41_modebench_and_math_evaluation_contracts():
    text = LAUNCHER.read_text(encoding="utf-8")
    for literal in (
...
def test_e42_held_audit_pins_quality_gate_and_excludes_other_mechanisms():
    text = LAUNCHER.read_text(encoding="utf-8")
    for literal in (
...
def test_e42_identity_records_success_only_support_and_capped_transform():
    text = LAUNCHER.read_text(encoding="utf-8")
    for literal in (
...
def test_shared_shell_stack_has_e42_variant_and_explicit_false_paths():
    ...
    for literal in (
        ...
        'elif [[ "$variant" == "quality_gated_semantic_novelty" ]]',
        'if [[ "$variant" == "quality_gated_semantic_novelty" ]]',
        ...
        "submit_arm quality_gated_semantic_novelty quality_gated_semantic_novelty",
    ):
    ...
    for literal in (
        ...
        "Frozen source lacks quality-gated semantic-Shannon advantage support",
    ):
        assert literal in train


def test_e42_protocol_freezes_quality_gated_capped_advantage():
    text = PROTOCOL.read_text(encoding="utf-8")
    for literal in (
        "**Status: FROZEN BEFORE LAUNCH",
        "quality-gated semantic novelty",
