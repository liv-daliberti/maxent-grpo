...
def test_main_with_explicit_args_calls_training(monkeypatch):
    ...
    monkeypatch.setitem(
        ...
        SimpleNamespace(run_baseline_training=_run_baseline),
    )
...
def test_main_parses_args_when_missing(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules,
        "maxent_grpo.training.baseline",
        SimpleNamespace(run_baseline_training=lambda *a: called.setdefault("args", a)),
    )
    entry.main()
...
def test_main_falls_back_to_hydra_on_parse_error(monkeypatch):
    ...
    entry.hydra_cli = SimpleNamespace(
        _maybe_insert_command=lambda cmd: called.setdefault("command", cmd),
        hydra_entry=lambda: called.setdefault("hydra", True),
    )
    result = entry.main()
    assert result is True
    assert called.get("command") == "train-maxent"
